...
void WorldSession::HandleSendDuelRequest(WorldPacket& recvPacket)
{
    ...
    recvPacket.ReadBitSeq<6, 2, 4, 1, 5, 3, 0, 7>(guid);
    recvPacket.ReadByteSeq<5, 2, 3, 6, 1, 0, 4, 7>(guid);
...
void WorldSession::HandleDuelResponseOpcode(WorldPacket& recvPacket)
{
    ...
    ObjectGuid guid;
    recvPacket.ReadBitSeq<5, 0, 1, 7, 6>(guid);
    bool accept = recvPacket.ReadBit();
    recvPacket.ReadBitSeq<3, 2, 4>(guid);
    recvPacket.ReadByteSeq<4, 6, 0, 3, 5, 1, 2, 7>(guid);
