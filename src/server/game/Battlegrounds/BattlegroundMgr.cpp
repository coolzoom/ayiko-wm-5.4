...
void BattlegroundMgr::Update(uint32 diff)
{
    ...
    if (sWorld->getIntConfig(CONFIG_ARENA_MAX_RATING_DIFFERENCE) && sWorld->getIntConfig(CONFIG_ARENA_RATED_UPDATE_TIMER))
    {
        ...
        if (m_NextRatedArenaUpdate < diff)
        {
            ...
            sLog->outDebug("bg.battleground", "BattlegroundMgr: UPDATING ARENA QUEUES");
    ...
    switch (StatusID)
    {
        case STATUS_NONE:
        {
            ...
            *data << uint32(Time1);                    // Join Time
            ...
            data->WriteBitSeq<1, 2, 4, 6, 5, 7, 0, 3>(player_guid);
            data->FlushBits();

            data->WriteByteSeq<7, 6, 4, 3, 0, 1, 2, 5>(player_guid);
            break;
        }
        case STATUS_WAIT_QUEUE:
        {
            ...
            data->WriteBitSeq<5, 1, 3>(player_guid);
            data->WriteBitSeq<6>(bg_guid);
            data->WriteBitSeq<4, 0>(player_guid);
            ...
            *data << uint32(Time2); //Time of the join
            data->WriteByteSeq<2, 3>(player_guid);
            ...
            *data << uint32(bg->isArena() ? bg->GetMaxPlayersPerTeam() : 0);
            data->WriteByteSeq<1, 0, 4>(player_guid);
            data->WriteByteSeq<0, 6, 7>(bg_guid);
            data->WriteByteSeq<6, 7>(player_guid);
            break;
        }
        case STATUS_WAIT_JOIN:
        {
            ...
            data->WriteBitSeq<2, 3>(player_guid);
            data->WriteBitSeq<2>(bg_guid);
            data->WriteBitSeq<5>(player_guid);
            data->WriteBit(role);
            data->WriteBitSeq<0, 7, 5>(bg_guid);
            data->WriteBitSeq<7, 0, 6>(player_guid);
            data->WriteBit(bg->isRated()); // byte38
            data->WriteBitSeq<1, 4>(player_guid);
            data->FlushBits();
            ...
            *data << uint32(bg->GetClientInstanceID());
            data->WriteByteSeq<0, 6>(player_guid);
            *data << uint32(GetMSTimeDiffToNow(Time2));
            *data << uint8(bg->GetMinLevel());
            data->WriteByteSeq<1, 5, 2>(player_guid);
            data->WriteByteSeq<4>(bg_guid);
            ...
            *data << uint8(0); // byte30
            data->WriteByteSeq<3, 4>(player_guid);
        ...
        case STATUS_IN_PROGRESS:
        {
            ...
            data->WriteBitSeq<0, 5, 3>(player_guid);
            data->WriteBitSeq<0>(bg_guid);
            data->WriteBitSeq<2, 7>(player_guid);
            data->WriteBitSeq<4>(bg_guid);
            data->WriteBit(bg->isRated()); // byte38
            data->WriteBitSeq<6>(player_guid);
            data->WriteBitSeq<2, 5, 6, 3>(bg_guid);
            data->WriteBitSeq<4, 1>(player_guid);
            data->FlushBits();

            *data << uint8(0); // byte30
            *data << uint8(bg->GetMinLevel());          // Min Level
            *data << uint8(0); // byte32
            data->WriteByteSeq<7, 0, 1, 2>(player_guid);
        ...
        case STATUS_WAIT_LEAVE:
        {
          /*  data->Initialize(SMSG_BATTLEFIELD_STATUS_WAIT_FOR_GROUPS, 48);
            ...
            *data << uint8(0);                          // unk
            ...
            data->WriteBitSeq<0, 1, 7>(bg_guid);
            data->WriteBitSeq<7, 0>(player_guid);
            data->WriteBitSeq<4>(bg_guid);
            data->WriteBitSeq<6, 2, 3>(player_guid);
            data->WriteBitSeq<3>(bg_guid);
            data->WriteBitSeq<4>(player_guid);
            data->WriteBitSeq<5>(bg_guid);
            data->WriteBitSeq<5>(player_guid);
            data->WriteBitSeq<2>(bg_guid);
            data->WriteBit(bg->isRated());              // Is Rated
            data->WriteBitSeq<1>(player_guid);
            data->WriteBitSeq<6>(bg_guid);
            ...
            data->WriteByteSeq<1, 0, 2>(bg_guid);
            data->WriteByteSeq<2>(player_guid);
            data->WriteByteSeq<7>(bg_guid);
            data->WriteByteSeq<1, 6>(player_guid);
            data->WriteByteSeq<6, 5>(bg_guid);
            data->WriteByteSeq<5, 4, 7>(player_guid);
            data->WriteByteSeq<3>(bg_guid);*/
            break;
        }
    }
...
void BattlegroundMgr::BuildPvpLogDataPacket(WorldPacket* data, Battleground* bg)
{
    ...
    for (Battleground::BattlegroundScoreMap::const_iterator itr = itr2; itr != bg->GetPlayerScoresEnd();)
    {
        ...
        {
            sLog->outError("bg.battleground", "Player " UI64FMTD " has scoreboard entry for battleground %u but is not in battleground!", itr->first, bg->GetTypeID(true));
            continue;
        }
    ...
    *data << uint8(counth2);
    ...
    if (HaveArenaData)                                          // arena TODO : Fix Order on Rated Implementation
    {
        ...
        for (int8 i = BG_TEAMS_COUNT - 1; i >= 0; --i)
        {
            ...
            sLog->outDebug("bg.battleground", "rating change: %d", rating_change);
        }
    }
...
void BattlegroundMgr::BuildPlayerLeftBattlegroundPacket(WorldPacket* data, uint64 guid)
{
    ...
    data->WriteBitSeq<7, 6, 2, 4, 5, 1, 3, 0>(guidBytes);
    data->WriteByteSeq<4, 2, 5, 7, 0, 6, 1, 3>(guidBytes);
}
...
Battleground* BattlegroundMgr::CreateNewBattleground(BattlegroundTypeId bgTypeId, PvPDifficultyEntry const* bracketEntry, uint8 arenaType, bool isRated)
{
    ...
    if (!bg_template)
    {
        sLog->outError("bg.battleground", "Battleground: CreateNewBattleground - bg template not found for %u", bgTypeId);
        return NULL;
    }
...
void BattlegroundMgr::CreateInitialBattlegrounds()
{
    ...
    if (!result)
    {
        sLog->outError("server.loading", ">> Loaded 0 battlegrounds. DB table `battleground_template` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!bl)
        {
            sLog->outError("bg.battleground", "Battleground ID %u not found in BattlemasterList.dbc. Battleground not created.", bgTypeID_);
            continue;
        }
        ...
        {
            sLog->outError("sql.sql", "Table `battleground_template` for id %u has bad values for MinPlayersPerTeam (%u) and MaxPlayersPerTeam(%u)",
        ...
        {
            sLog->outError("sql.sql", "Table `battleground_template` for id %u has bad values for LevelMin (%u) and LevelMax(%u)",
        ...
        {
            sLog->outError("sql.sql", "Table `battleground_template` for id %u have non-existed WorldSafeLocs.dbc id %u in field `AllianceStartLoc`. BG not created.", data.bgTypeId, startId);
            continue;
        }
        ...
        {
            sLog->outError("sql.sql", "Table `battleground_template` for id %u have non-existed WorldSafeLocs.dbc id %u in field `HordeStartLoc`. BG not created.", data.bgTypeId, startId);
            continue;
        }
        ...
        for (int i = 0; i < 11; ++i)
            if (rated_bl->mapid[i] == bl->mapid[0] && bl->mapid[1] == uint32(-1))
    ...
    sLog->outInfo("server.loading", ">> Loaded %u battlegrounds in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void BattlegroundMgr::SendToBattleground(Player* player, uint32 instanceId, BattlegroundTypeId bgTypeId)
{
    ...
    if (bg)
    {
        ...
        sLog->outInfo("bg.battleground", "BATTLEGROUND: Sending %s to map %u, X %f, Y %f, Z %f, O %f", player->GetName(), mapid, x, y, z, O);
        player->TeleportTo(mapid, x, y, z, O);
    }
    else
    {
        sLog->outError("bg.battleground", "player %u is trying to port to non-existent bg instance %u", player->GetGUIDLow(), instanceId);
    }
}
...
void BattlegroundMgr::LoadBattleMastersEntry()
{
    ...
    if (!result)
    {
        sLog->outInfo("server.loading", ">> Loaded 0 battlemaster entries. DB table `battlemaster_entry` is empty!");
        return;
    }
    ...
    do
    {
        ...
        {
            sLog->outError("sql.sql", "Table `battlemaster_entry` contain entry %u for not existed battleground type %u, ignored.", entry, bgTypeId);
            continue;
        }
    ...
    sLog->outInfo("server.loading", ">> Loaded %u battlemaster entries in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
