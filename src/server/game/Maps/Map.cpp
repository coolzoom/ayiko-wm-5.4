...
bool Map::ExistMap(uint32 mapid, int gx, int gy)
{
    ...
    if (!pf)
        sLog->outError("maps", "Map file '%s': does not exist!", tmp);
    else
    {
        ...
        if (fread(&header, sizeof(header), 1, pf) == 1)
        {
            if (header.mapMagic != MapMagic.asUInt || header.versionMagic != MapVersionMagic.asUInt)
                sLog->outError("maps", "Map file '%s' is from an incompatible clientversion. Please recreate using the mapextractor.", tmp);
            else
                ret = true;
        }
        fclose(pf);
    ...
    if (VMAP::IVMapManager* vmgr = VMAP::VMapFactory::createOrGetVMapManager())
    {
        if (vmgr->isMapLoadingEnabled())
        {
            ...
            if (!exists)
            {
                ...
                sLog->outError("maps", "VMap file '%s' is missing or points to wrong version of vmap file. Redo vmaps with latest version of vmap_assembler.exe.", (sWorld->GetDataPath()+"vmaps/"+name).c_str());
                return false;
            }
        }
    }
...
void Map::LoadVMap(int gx, int gy)
{
    ...
    switch (vmapLoadResult)
    {
        case VMAP::VMAP_LOAD_RESULT_OK:
            sLog->outInfo("maps", "VMAP loaded name:%s, id:%d, x:%d, y:%d (vmap rep.: x:%d, y:%d)", GetMapName(), GetId(), gx, gy, gx, gy);
            break;
        case VMAP::VMAP_LOAD_RESULT_ERROR:
            sLog->outInfo("maps", "Could not load VMAP name:%s, id:%d, x:%d, y:%d (vmap rep.: x:%d, y:%d)", GetMapName(), GetId(), gx, gy, gx, gy);
            break;
        case VMAP::VMAP_LOAD_RESULT_IGNORED:
            sLog->outDebug("maps", "Ignored VMAP name:%s, id:%d, x:%d, y:%d (vmap rep.: x:%d, y:%d)", GetMapName(), GetId(), gx, gy, gx, gy);
            break;
    }
}
...
void Map::LoadMap(int gx, int gy, bool reload)
{
    ...
    if (GridMaps[gx][gy])
    {
        sLog->outInfo("maps", "Unloading previously loaded map %u before reloading.", GetId());
    ...
    sLog->outInfo("maps", "Loading map %s", tmp);
    // loading data
    GridMaps[gx][gy] = new GridMap();
    if (!GridMaps[gx][gy]->loadData(tmp))
    {
        sLog->outError("maps", "Error loading map file: \n %s\n", tmp);
    }
    delete [] tmp;
...
void Map::SwitchGridContainers(Creature* obj, bool on)
{
    ...
    if (!p.IsCoordValid())
    {
        sLog->outError("maps", "Map::SwitchGridContainers: Object " UI64FMTD " has invalid coordinates X:%f Y:%f grid cell [%u:%u]", obj->GetGUID(), obj->GetPositionX(), obj->GetPositionY(), p.x_coord, p.y_coord);
        return;
    }
    ...
    sLog->outDebug("maps", "Switch object " UI64FMTD " from grid[%u, %u] %u", obj->GetGUID(), cell.data.Part.grid_x, cell.data.Part.grid_y, on);
...
void Map::EnsureGridCreated(const GridCoord &p)
{
    if (!getNGrid(p.x_coord, p.y_coord))
    {
        ...
        if (!getNGrid(p.x_coord, p.y_coord))
        {
            sLog->outDebug("maps", "Creating grid[%u, %u] for map %u instance %u", p.x_coord, p.y_coord, GetId(), i_InstanceId);
...
void Map::EnsureGridLoadedForActiveObject(const Cell &cell, WorldObject* object)
{
    ...
    {
        sLog->outDebug("maps", "Active object " UI64FMTD " triggers loading of grid [%u, %u] on map %u", object->GetGUID(), cell.GridX(), cell.GridY(), GetId());
...
bool Map::EnsureGridLoaded(const Cell &cell)
{
    ...
    {
        sLog->outDebug("maps", "Loading grid[%u, %u] for map %u instance %u", cell.GridX(), cell.GridY(), GetId(), i_InstanceId);
...
bool Map::AddPlayerToMap(Player* player)
{
    ...
    {
        sLog->outError("maps", "Map::Add: Player (GUID: %u) has invalid coordinates X:%f Y:%f grid cell [%u:%u]", player->GetGUIDLow(), player->GetPositionX(), player->GetPositionY(), cellCoord.x_coord, cellCoord.y_coord);
        return false;
    }
...
bool Map::AddToMap(T *obj)
{
    ...
    if (!cellCoord.IsCoordValid())
    {
        sLog->outError("maps", "Map::Add: Object " UI64FMTD " has invalid coordinates X:%f Y:%f grid cell [%u:%u]", obj->GetGUID(), obj->GetPositionX(), obj->GetPositionY(), cellCoord.x_coord, cellCoord.y_coord);
    ...
    if (obj->isActiveObject())
        ...
        EnsureGridCreated(GridCoord(cell.GridX(), cell.GridY()));
    AddToGrid(obj, cell);
    sLog->outDebug("maps", "Object %u enters grid[%u, %u]", GUID_LOPART(obj->GetGUID()), cell.GridX(), cell.GridY());
...
void Map::PlayerRelocation(Player* player, float x, float y, float z, float orientation)
{
    ...
    {
        sLog->outDebug("maps", "Player %s relocation grid[%u, %u]cell[%u, %u]->grid[%u, %u]cell[%u, %u]", player->GetName(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY(), new_cell.GridX(), new_cell.GridY(), new_cell.CellX(), new_cell.CellY());
...
void Map::CreatureRelocation(Creature* creature, float x, float y, float z, float ang, bool respawnRelocationOnFail)
{
    ...
    {
        #ifdef TRINITY_DEBUG
            sLog->outDebug("maps", "Creature (GUID: %u Entry: %u) added to moving list from grid[%u, %u]cell[%u, %u] to grid[%u, %u]cell[%u, %u].", creature->GetGUIDLow(), creature->GetEntry(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY(), new_cell.GridX(), new_cell.GridY(), new_cell.CellX(), new_cell.CellY());
        #endif
    ...
    for (std::vector<Creature*>::iterator itr = _creaturesToMove.begin(); itr != _creaturesToMove.end(); ++itr)
    {
        ...
        else
        {
            ...
            if (!CreatureRespawnRelocation(c, false))
            {
                ...
                #ifdef TRINITY_DEBUG
                    sLog->outDebug("maps", "Creature (GUID: %u Entry: %u) cannot be move to unloaded respawn grid.", c->GetGUIDLow(), c->GetEntry());
                #endif
...
bool Map::CreatureCellRelocation(Creature* c, Cell new_cell)
{
    ...
    if (!old_cell.DiffGrid(new_cell))                       // in same grid
    {
        ...
        {
            #ifdef TRINITY_DEBUG
                sLog->outDebug("maps", "Creature (GUID: %u Entry: %u) moved in grid[%u, %u] from cell[%u, %u] to cell[%u, %u].", c->GetGUIDLow(), c->GetEntry(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY(), new_cell.CellX(), new_cell.CellY());
            #endif
        ...
        {
            #ifdef TRINITY_DEBUG
                sLog->outDebug("maps", "Creature (GUID: %u Entry: %u) moved in same grid[%u, %u]cell[%u, %u].", c->GetGUIDLow(), c->GetEntry(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY());
            #endif
        }
    ...
    if (c->isActiveObject())
    {
        ...
        #ifdef TRINITY_DEBUG
            sLog->outDebug("maps", "Active creature (GUID: %u Entry: %u) moved from grid[%u, %u]cell[%u, %u] to grid[%u, %u]cell[%u, %u].", c->GetGUIDLow(), c->GetEntry(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY(), new_cell.GridX(), new_cell.GridY(), new_cell.CellX(), new_cell.CellY());
        #endif
    ...
    {
        #ifdef TRINITY_DEBUG
            sLog->outDebug("maps", "Creature (GUID: %u Entry: %u) moved from grid[%u, %u]cell[%u, %u] to grid[%u, %u]cell[%u, %u].", c->GetGUIDLow(), c->GetEntry(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY(), new_cell.GridX(), new_cell.GridY(), new_cell.CellX(), new_cell.CellY());
        #endif
    ...
    #ifdef TRINITY_DEBUG
        sLog->outDebug("maps", "Creature (GUID: %u Entry: %u) attempted to move from grid[%u, %u]cell[%u, %u] to unloaded grid[%u, %u]cell[%u, %u].", c->GetGUIDLow(), c->GetEntry(), old_cell.GridX(), old_cell.GridY(), old_cell.CellX(), old_cell.CellY(), new_cell.GridX(), new_cell.GridY(), new_cell.CellX(), new_cell.CellY());
    #endif
    return false;
}
...
bool Map::CreatureRespawnRelocation(Creature* c, bool diffGridOnly)
{
    ...
    #ifdef TRINITY_DEBUG
        sLog->outDebug("maps", "Creature (GUID: %u Entry: %u) moved from grid[%u, %u]cell[%u, %u] to respawn grid[%u, %u]cell[%u, %u].", c->GetGUIDLow(), c->GetEntry(), c->GetCurrentCell().GridX(), c->GetCurrentCell().GridY(), c->GetCurrentCell().CellX(), c->GetCurrentCell().CellY(), resp_cell.GridX(), resp_cell.GridY(), resp_cell.CellX(), resp_cell.CellY());
    #endif
...
bool Map::UnloadGrid(NGridType& ngrid, bool unloadAll)
{
...

    {
        ...
        sLog->outDebug("maps", "Unloading grid[%u, %u] for map %u", x, y, GetId());
    ...
    sLog->outDebug("maps", "Unloading grid[%u, %u] for map %u finished", x, y, GetId());
    return true;
}
    ...
    if (HavePlayers())
    {
        for (MapRefManager::iterator itr = m_mapRefManager.begin(); itr != m_mapRefManager.end(); ++itr)
        {
            ...
            if (!player->IsBeingTeleportedFar())
            {
                ...
                sLog->outError("maps", "Map::UnloadAll: player %s is still in map %u during unload, this should not happen!", player->GetName(), GetId());
...
bool GridMap::loadData(char *filename)
{
    ...
    if (header.mapMagic == MapMagic.asUInt && header.versionMagic == MapVersionMagic.asUInt)
    {
        ...
        {
            sLog->outError("maps", "Error loading map area data\n");
            fclose(in);
            return false;
        }
        // loadup height data
        ...
        {
            sLog->outError("maps", "Error loading map height data\n");
            fclose(in);
            return false;
        }
        // loadup liquid data
        ...
        {
            sLog->outError("maps", "Error loading map liquids data\n");
            fclose(in);
            return false;
        }
        fclose(in);
    ...
    sLog->outError("maps", "Map file '%s' is from an incompatible clientversion. Please recreate using the mapextractor.", filename);
    fclose(in);
    return false;
}
...
bool Map::IsOutdoors(float x, float y, float z) const
{
    ...
    if (wmoEntry)
    {
        sLog->outDebug("maps", "Got WMOAreaTableEntry! flag %u, areaid %u", wmoEntry->Flags, wmoEntry->areaId);
...
ZLiquidStatus Map::getLiquidStatus(float x, float y, float z, uint8 ReqLiquidType, LiquidData* data) const
{
    ...
    {
        sLog->outDebug("maps", "getLiquidStatus(): vmap liquid level: %f ground: %f type: %u", liquid_level, ground_level, liquid_type);
        // Check water level and ground level
        if (liquid_level > ground_level && z > ground_level - 2)
        {
            // All ok in water -> store data
...
bool Map::CheckGridIntegrity(Creature* c, bool moved) const
{
    ...
    if (xy_cell != cur_cell)
    {
        sLog->outDebug("maps", "Creature (GUID: %u) X: %f Y: %f (%s) is in grid[%u, %u]cell[%u, %u] instead of grid[%u, %u]cell[%u, %u]",
            c->GetGUIDLow(),
...
{
    sLog->outInfo("maps", "Creating player data for himself %u", player->GetGUIDLow());
...
inline void Map::setNGrid(NGridType *grid, uint32 x, uint32 y)
{
    ...
    {
        sLog->outError("maps", "map::setNGrid() Invalid grid coordinates found: %d, %d!", x, y);
        ASSERT(false);
    }
    i_grids[x][y] = grid;
}
...
void Map::AddObjectToRemoveList(WorldObject* obj)
{
    ...
    //sLog->outDebug("maps", "Object (GUID: %u TypeId: %u) added to removing list.", obj->GetGUIDLow(), obj->GetTypeId());
}
...
void Map::RemoveAllObjectsInRemoveList()
{
    ...
    //sLog->outDebug("maps", "Object remover 1 check.");
    while (!i_objectsToRemove.empty())
    {
        ...
        switch (obj->GetTypeId())
        {
            case TYPEID_CORPSE:
            {
                ...
                if (!corpse)
                    sLog->outError("maps", "Tried to delete corpse/bones %u that is not in map.", obj->GetGUIDLow());
                else
        ...
        case TYPEID_UNIT:
            ...
            break;
        default:
            sLog->outError("maps", "Non-grid object (TypeId: %u) is in grid object remove list, ignored.", obj->GetTypeId());
            break;
        }
    ...
    //sLog->outDebug("maps", "Object remover 2 check.");
}
...
void Map::AddToActive(Creature* c)
{
    ...
    if (!c->isPet() && c->GetDBTableGUIDLow())
    {
        ...
        else
        {
            ...
            sLog->outError("maps", "Active creature (GUID: %u Entry: %u) added to grid[%u, %u] but spawn grid[%u, %u] was not loaded.",
...
void Map::RemoveFromActive(Creature* c)
{
    ...
    if (!c->isPet() && c->GetDBTableGUIDLow())
    {
        ...
        {
            GridCoord p2 = Trinity::ComputeGridCoord(c->GetPositionX(), c->GetPositionY());
            sLog->outError("maps", "Active creature (GUID: %u Entry: %u) removed from grid[%u, %u] but spawn grid[%u, %u] was not loaded.",
                c->GetGUIDLow(), c->GetEntry(), p.x_coord, p.y_coord, p2.x_coord, p2.y_coord);
        }
    }
}
...
bool InstanceMap::CanEnter(Player* player)
{
    ...
    {
        sLog->outError("maps", "InstanceMap::CanEnter - player %s(%u) already in map %d, %d, %d!", player->GetName(), player->GetGUIDLow(), GetId(), GetInstanceId(), GetSpawnMode());
        ASSERT(false);
        return false;
    }
    ...
    {
        sLog->outInfo("maps", "MAP: Instance '%u' of map '%s' cannot have more than '%u' players. Player '%s' rejected", GetInstanceId(), GetMapName(), maxPlayers, player->GetName());
...
bool InstanceMap::AddPlayerToMap(Player* player)
{
...

    {
        ...
        if (IsDungeon())
        {
            ...
            if (!mapSave)
            {
                sLog->outInfo("maps", "InstanceMap::Add: creating instance save for map %d spawnmode %d with instance id %d", GetId(), GetSpawnMode(), GetInstanceId());
            ...
            else
            {
                if (group)
                {
                    ...
                    if (playerBind && playerBind->save != mapSave)
                    {
                        ...
                        if (groupBind)
                            sLog->outError("maps", "InstanceMap::Add: the group is bound to the instance %d, %d, %d, %d, %d, %d", groupBind->save->GetMapId(), groupBind->save->GetInstanceId(), groupBind->save->GetDifficulty(), groupBind->save->GetPlayerCount(), groupBind->save->GetGroupCount(), groupBind->save->CanReset());
                        //ASSERT(false);
                        return false;
                    }
                    ...
                    else
                    {
                        ...
                        if (groupBind->save != mapSave)
                        {
                            ...
                            if (mapSave)
                                sLog->outError("maps", "MapSave players: %d, group count: %d", mapSave->GetPlayerCount(), mapSave->GetGroupCount());
                            else
                                sLog->outError("maps", "MapSave NULL");
                            if (groupBind->save)
                                sLog->outError("maps", "GroupBind save players: %d, group count: %d", groupBind->save->GetPlayerCount(), groupBind->save->GetGroupCount());
                            else
                                sLog->outError("maps", "GroupBind save NULL");
                            return false;
                        }
        ...
        sLog->outInfo("maps", "MAP: Player '%s' entered instance '%u' of map '%s'", player->GetName(), GetInstanceId(), GetMapName());
...
{
    sLog->outInfo("maps", "MAP: Removing player '%s' from instance '%u' of map '%s' before relocating to another map", player->GetName(), GetInstanceId(), GetMapName());
...
void InstanceMap::CreateInstanceData(bool load)
{
    ...
    if (load)
    {
        ...
        if (result)
        {
            ...
            if (data != "")
            {
                sLog->outDebug("maps", "Loading instance data for `%s` with id %u", sObjectMgr->GetScriptName(i_script_id), i_InstanceId);
...
void InstanceMap::PermBindAllPlayers(Player* source)
{
    ...
    if (!save)
    {
        sLog->outError("maps", "Cannot bind player (GUID: %u, Name: %s), because no instance save is available for instance map (Name: %s, Entry: %u, InstanceId: %u)!", source->GetGUIDLow(), source->GetName(), source->GetMap()->GetMapName(), source->GetMapId(), GetInstanceId());
        return;
    }
...
void InstanceMap::SetResetSchedule(bool on)
{
    ...
    if (IsDungeon() && !HavePlayers() && !IsRaidOrHeroicDungeon())
    {
        if (InstanceSave* save = sInstanceSaveMgr->GetInstanceSave(GetInstanceId()))
            ...
            sLog->outError("maps", "InstanceMap::SetResetSchedule: cannot turn schedule %s, there is no save information for instance (map [id: %u, name: %s], instance id: %u, difficulty: %u)",
...
bool BattlegroundMap::CanEnter(Player* player)
{
    if (player->GetMapRef().getTarget() == this)
    {
        sLog->outError("maps", "BGMap::CanEnter - player %u is already in map!", player->GetGUIDLow());
        ASSERT(false);
        return false;
    }
...
{
    sLog->outInfo("maps", "MAP: Removing player '%s' from bg '%u' of map '%s' before relocating to another map", player->GetName(), GetInstanceId(), GetMapName());
    Map::RemovePlayerFromMap(player, remove);
}
