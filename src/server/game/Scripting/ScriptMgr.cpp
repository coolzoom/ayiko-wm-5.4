...
void DoScriptText(int32 iTextEntry, WorldObject* pSource, Unit* target)
{
    ...
    if (pData->uiSoundId)
    {
        if (sSoundEntriesStore.LookupEntry(pData->uiSoundId))
            ...
            TC_LOG_ERROR("scripts", "DoScriptText entry %i tried to process invalid sound id %u.", iTextEntry, pData->uiSoundId);
    }
...
void ScriptMgr::FillSpellSummary()
{
    ...
    for (uint32 i = 0; i < sSpellMgr->GetSpellInfoStoreSize(); ++i)
    {
        ...
        for (auto const &spellEffect : pTempSpell->Effects)
        {
            // Spell targets self.
            if (spellEffect.TargetA.GetTarget() == TARGET_UNIT_CASTER)
                SpellSummary[i].Targets |= 1 << (SELECT_TARGET_SELF-1);

            // Spell targets a single enemy.
            if (spellEffect.TargetA.GetTarget() == TARGET_UNIT_TARGET_ENEMY
                    || spellEffect.TargetA.GetTarget() == TARGET_DEST_TARGET_ENEMY)
            ...
            if (spellEffect.TargetA.GetTarget() == TARGET_UNIT_SRC_AREA_ENEMY
                    ...
                    || spellEffect.TargetA.GetTarget() == TARGET_SRC_CASTER
            ...
            if (spellEffect.TargetA.GetTarget() == TARGET_UNIT_TARGET_ENEMY
                    || spellEffect.TargetA.GetTarget() == TARGET_DEST_TARGET_ENEMY
                    || spellEffect.TargetA.GetTarget() == TARGET_UNIT_SRC_AREA_ENEMY
                    || spellEffect.TargetA.GetTarget() == TARGET_UNIT_DEST_AREA_ENEMY
                    || spellEffect.TargetA.GetTarget() == TARGET_SRC_CASTER
                    || spellEffect.TargetA.GetTarget() == TARGET_DEST_DYNOBJ_ENEMY)
            ...
            if (spellEffect.TargetA.GetTarget() == TARGET_UNIT_CASTER
                    ...
                    || spellEffect.TargetA.GetTarget() == TARGET_UNIT_TARGET_PARTY)
                SpellSummary[i].Targets |= 1 << (SELECT_TARGET_SINGLE_FRIEND-1);

            // Spell targets AoE friends.
            if (spellEffect.TargetA.GetTarget() == TARGET_UNIT_CASTER_AREA_PARTY
                    || spellEffect.TargetA.GetTarget() == TARGET_UNIT_LASTTARGET_AREA_PARTY
                    || spellEffect.TargetA.GetTarget() == TARGET_SRC_CASTER)
            ...
            if (spellEffect.TargetA.GetTarget() == TARGET_UNIT_CASTER
                    || spellEffect.TargetA.GetTarget() == TARGET_UNIT_TARGET_ALLY
                    || spellEffect.TargetA.GetTarget() == TARGET_UNIT_TARGET_PARTY
                    || spellEffect.TargetA.GetTarget() == TARGET_UNIT_CASTER_AREA_PARTY
                    || spellEffect.TargetA.GetTarget() == TARGET_UNIT_LASTTARGET_AREA_PARTY
                    || spellEffect.TargetA.GetTarget() == TARGET_SRC_CASTER)
            ...
            if (spellEffect.Effect == SPELL_EFFECT_HEAL
                    ...
                    || (spellEffect.Effect == SPELL_EFFECT_APPLY_AURA && spellEffect.ApplyAuraName == 8))
