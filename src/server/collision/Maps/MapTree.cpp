...
namespace VMAP
{
    ...
    */
    ...
    {
        sLog->outDebug("maps", "StaticMapTree::InitMap() : initializing StaticMapTree '%s'", fname.c_str());
        bool success = false;
        ...
        if (!iIsTiled && ModelSpawn::readFromFile(rf, spawn))
        {
            ...
            sLog->outDebug("maps", "StaticMapTree::InitMap() : loading %s", spawn.name.c_str());
            if (model)
            {
            ...
            {
                success = false;
                sLog->outDebug("maps", "StaticMapTree::InitMap() : could not acquire WorldModel pointer for '%s'", spawn.name.c_str());
            }
        }
    ...
    bool StaticMapTree::LoadMapTile(uint32 tileX, uint32 tileY, VMapManager2* vm)
    {
        ...
        if (!iTreeValues)
        {
            sLog->outDebug("maps", "StaticMapTree::LoadMapTile() : tree has not been initialized [%u, %u]", tileX, tileY);
            return false;
        }
        bool result = true;
        ...
        if (tf)
        {
            ...
            for (uint32 i=0; i<numSpawns && result; ++i)
            {
                ...
                if (result)
                {
                    ...
                    WorldModel* model = vm->acquireModelInstance(iBasePath, spawn.name);
                    if (!model)
                        sLog->outDebug("maps", "StaticMapTree::LoadMapTile() : could not acquire WorldModel pointer [%u, %u]", tileX, tileY);
    ...
    void StaticMapTree::UnloadMapTile(uint32 tileX, uint32 tileY, VMapManager2* vm)
    {
        ...
        {
            sLog->outDebug("maps", "StaticMapTree::UnloadMapTile() : trying to unload non-loaded tile - Map:%u X:%u Y:%u", iMapID, tileX, tileY);
            return;
        }
                    ...
                    if (result)
                    {
                        ...
                        else
                        {
                            if (!iLoadedSpawns.count(referencedNode))
                                sLog->outDebug("maps", "StaticMapTree::UnloadMapTile() : trying to unload non-referenced model '%s' (ID:%u)", spawn.name.c_str(), spawn.ID);
