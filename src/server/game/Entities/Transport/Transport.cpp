...
bool Transport::GenerateWaypoints(uint32 pathid, std::set<uint32> &mapids)
{
    ...
    //        TC_LOG_INFO("entities.transport", "%f, %f, %f, %f, %f, %f, %f", keyFrames[i].x, keyFrames[i].y, keyFrames[i].distUntilStop, keyFrames[i].distSinceStop, keyFrames[i].distFromPrev, keyFrames[i].tFrom, keyFrames[i].tTo);
    //    }
    ...
    for (size_t i = 0; i < keyFrames.size() - 1; ++i)
    {
        ...
        if (((d < keyFrames[i + 1].distFromPrev) && (tTo > 0)))
        {
            while ((d < keyFrames[i + 1].distFromPrev) && (tTo > 0))
            {
                ...
                if (d > 0)
                {
                    ...
                    //                    TC_LOG_INFO("entities.transport", "T: %d, D: %f, x: %f, y: %f, z: %f", t, d, newX, newY, newZ);
                    if (teleport)
        ...
        //        TC_LOG_INFO("entities.transport", "T: %d, x: %f, y: %f, z: %f, t:%d", t, pos.x, pos.y, pos.z, teleport);
    ...
    //    TC_LOG_INFO("entities.transport", "    Generated %lu waypoints, total time %u.", (unsigned long)m_WayPoints.size(), timer);
...
bool Transport::AddPassenger(Player* passenger)
{
    if (m_passengers.insert(passenger).second)
        TC_LOG_INFO("entities.transport", "Player %s boarded transport %s.", passenger->GetName(), GetName());
...
bool Transport::RemovePassenger(Player* passenger)
{
    if (m_passengers.erase(passenger))
        TC_LOG_INFO("entities.transport", "Player %s removed from transport %s.", passenger->GetName(), GetName());
...
void Transport::DoEventIfAny(WayPointMap::value_type const& node, bool departure)
{
    ...
    {
        TC_LOG_DEBUG("maps.scripts", "Taxi %s event %u of node %u of %s path", departure ? "departure" : "arrival", eventid, node.first, GetName());
...
Creature * Transport::AddNPCPassengerCreature(uint32 tguid, uint32 entry, float x, float y, float z, float o, uint32 anim)
{
    ...
    {
        TC_LOG_ERROR("entities.transport", "Creature (guidlow %d, entry %d) not created. Suggested coordinates isn't valid (X: %f Y: %f)", pCreature->GetGUIDLow(), pCreature->GetEntry(), pCreature->GetPositionX(), pCreature->GetPositionY());
        delete pCreature;
        return NULL;
    }
