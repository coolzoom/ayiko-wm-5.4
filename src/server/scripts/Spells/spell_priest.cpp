        ...
        class spell_pri_halo_damage_SpellScript : public SpellScript
        {
            ...
            void HandleDamage(SpellEffIndex /*eff*/)
            {
                if (Player* player = GetCaster()->ToPlayer())
                {
                    ...
                    {
                        int32 damage = GetHitDamage();
                        damage += int32(player->SpellBaseDamageBonusDone(GetSpellInfo()->GetSchoolMask()) * 1.95f);
                ...
                if (Player* player = GetCaster()->ToPlayer())
                {
                    if (Unit* target = GetHitUnit())
                    {
                        if (GetSpellInfo()->Id == 120517)
                            player->CastSpell(target, PRIEST_SPELL_HALO_HEAL_SHADOW, true);
                        else
                            player->CastSpell(target, PRIEST_SPELL_HALO_HEAL_HOLY, true);
                    }
                }
            }
