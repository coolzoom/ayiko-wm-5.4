...
{
    sLog->outInfo("server.loading", "Loading guild finder guild-related settings...");
    //                                                           0                1             2                  3             4           5             6         7
    ...
    if (!result)
    {
        sLog->outInfo("server.loading", ">> Loaded 0 guild finder guild-related settings. Table `guild_finder_guild_settings` is empty.");
        return;
    }
    ...
    sLog->outInfo("server.loading", ">> Loaded %u guild finder guild-related settings in %u ms.", count, GetMSTimeDiffToNow(oldMSTime));
}

void GuildFinderMgr::LoadMembershipRequests()
{
    sLog->outInfo("server.loading", "Loading guild finder membership requests...");
    //                                                      0         1           2            3           4         5         6
    ...
    if (!result)
    {
        sLog->outInfo("server.loading", ">> Loaded 0 guild finder membership requests. Table `guild_finder_applicant` is empty.");
        return;
    }
    ...
    sLog->outInfo("server.loading", ">> Loaded %u guild finder membership requests in %u ms.", count, GetMSTimeDiffToNow(oldMSTime));
}
...
{
    std::vector<MembershipRequest> recruitsList = GetAllMembershipRequestsForGuild(guild.GetId());
...
{
    std::list<MembershipRequest> applicatedGuilds = GetAllMembershipRequestsForPlayer(player.GetGUIDLow());
    uint32 applicationsCount = applicatedGuilds.size();

    WorldPacket data(SMSG_LF_GUILD_MEMBERSHIP_LIST_UPDATED);

    data << uint32(10 - CountRequestsFromPlayer(player.GetGUIDLow())); // Applications count left
    ...
    if (applicationsCount > 0)
    {
        ...
        for (std::list<MembershipRequest>::const_iterator itr = applicatedGuilds.begin(); itr != applicatedGuilds.end(); ++itr)
        {
            ...
            LFGuildSettings guildSettings = GetGuildSettings(itr->GetGuildId());
            MembershipRequest request = *itr;
