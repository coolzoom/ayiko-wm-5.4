...
void WorldSession::SendPacket(WorldPacket const* packet, bool forced /*= false*/)
{
    ...
    {
        sLog->outError("network.opcode", "Prevented sending of NULL_OPCODE to %s", GetPlayerName(false).c_str());
        return;
    }
    else if (packet->GetOpcode() == UNKNOWN_OPCODE && !forced)
    {
        sLog->outError("network.opcode", "Prevented sending of UNKNOWN_OPCODE to %s", GetPlayerName(false).c_str());
        return;
    }
    ...
    if (!forced)
    {
        ...
        {
            sLog->outError("network.opcode", "Prevented sending disabled opcode %s to %s", GetOpcodeNameForLogging(packet->GetOpcode(), WOW_SERVER).c_str(), GetPlayerName(false).c_str());
            return;
        }
    }
    ...
    else
    {
        ...
        sLog->outInfo("misc", "Send all time packets count: " UI64FMTD " bytes: " UI64FMTD " avr.count/sec: %f avr.bytes/sec: %f time: %u", sendPacketCount, sendPacketBytes, float(sendPacketCount)/fullTime, float(sendPacketBytes)/fullTime, uint32(fullTime));
        sLog->outInfo("misc", "Send last min packets count: " UI64FMTD " bytes: " UI64FMTD " avr.count/sec: %f avr.bytes/sec: %f", sendLastPacketCount, sendLastPacketBytes, float(sendLastPacketCount)/minTime, float(sendLastPacketBytes)/minTime);
...
{
    sLog->outError("network.opcode", "Received unexpected opcode %s Status: %s Reason: %s from %s",
...
{
    sLog->outError("network.opcode", "Unprocessed tail data (read stop at %u from %u) Opcode %s from %s",
...
bool WorldSession::Update(uint32 diff, PacketFilter& updater)
{
            ...
            _recvQueue.next(packet, updater))
    {
        ...
        try
        {
            switch (opHandle->status)
            {
                case STATUS_LOGGEDIN:
                    if (!_player)
                    {
                        ...
                        if (!m_playerRecentlyLogout)
                        {
                            ...
                            QueuePacket(packet);
                            //! Log
                                sLog->outDebug("network", "Re-enqueueing packet with opcode %s with with status STATUS_LOGGEDIN. "
                ...
                case STATUS_AUTHED:
                    ...
                    break;
                case STATUS_NEVER:
                        sLog->outError("network.opcode", "Received not allowed opcode %s from %s", GetOpcodeNameForLogging(packet->GetOpcode(), WOW_CLIENT).c_str()
                            , GetPlayerName(false).c_str());
                    break;
                case STATUS_UNHANDLED:
                        sLog->outError("network.opcode", "Received not handled opcode %s from %s", GetOpcodeNameForLogging(packet->GetOpcode(), WOW_CLIENT).c_str()
                            , GetPlayerName(false).c_str());
                    break;
            }
        }
        ...
        {
            sLog->outError("network", "WorldSession::Update ByteBufferException occured while parsing a packet (opcode: %u) from client %s, accountid=%i. Skipped packet.",
...
void WorldSession::LogoutPlayer(bool Save)
{
    ...
    if (_player)
    {
        ...
        sLog->outInfo("entities.player.character", "Account: %d (IP: %s) Logout Character:[%s] (GUID: %u) Level: %d", GetAccountId(), GetRemoteAddress().c_str(), _player->GetName(), _player->GetGUIDLow(), _player->getLevel());
        ...
        SendPacket(&data);
        sLog->outDebug("network", "SESSION: Sent SMSG_LOGOUT_COMPLETE Message");
...
{
    sLog->outError("network.opcode", "Received unhandled opcode %s from %s"
        , GetOpcodeNameForLogging(recvPacket.GetOpcode(), WOW_CLIENT).c_str(), GetPlayerName(false).c_str());
}

void WorldSession::Handle_EarlyProccess(WorldPacket& recvPacket)
{
    sLog->outError("network.opcode", "Received opcode %s that must be processed in WorldSocket::OnRead from %s"
        , GetOpcodeNameForLogging(recvPacket.GetOpcode(), WOW_CLIENT).c_str(), GetPlayerName(false).c_str());
}

void WorldSession::Handle_ServerSide(WorldPacket& recvPacket)
{
    sLog->outError("network.opcode", "Received server-side opcode %s from %s"
        , GetOpcodeNameForLogging(recvPacket.GetOpcode(), WOW_CLIENT).c_str(), GetPlayerName(false).c_str());
}

void WorldSession::Handle_Deprecated(WorldPacket& recvPacket)
{
    sLog->outError("network.opcode", "Received deprecated opcode %s from %s"
        , GetOpcodeNameForLogging(recvPacket.GetOpcode(), WOW_CLIENT).c_str(), GetPlayerName(false).c_str());
}
...
void WorldSession::LoadAccountData(PreparedQueryResult result, uint32 mask)
{
    ...
    do
    {
        ...
        {
            sLog->outError("misc", "Table `%s` have invalid account data type (%u), ignore.",
        ...
        if ((mask & (1 << type)) == 0)
        {
            sLog->outError("misc", "Table `%s` have non appropriate for table  account data type (%u), ignore.",
                mask == GLOBAL_CACHE_MASK ? "account_data" : "character_account_data", type);
            continue;
        }
...
void WorldSession::ReadAddonsInfo(WorldPacket &data)
{
    ...
    if (size > 0xFFFFF)
    {
        sLog->outError("misc", "WorldSession::ReadAddonsInfo addon info too big, size %u", size);
        return;
    }
    ...
    if (uncompress(const_cast<uint8*>(addonInfo.contents()), &uSize, const_cast<uint8*>(data.contents() + pos), data.size() - pos) == Z_OK)
    {
        ...
        for (uint32 i = 0; i < addonsCount; ++i)
        {
            ...
            sLog->outInfo("misc", "ADDON: Name: %s, Enabled: 0x%x, CRC: 0x%x, Unknown2: 0x%x", addonName.c_str(), enabled, crc, unk1);
            ...
            if (savedAddon)
            {
                ...
                if (!match)
                    sLog->outInfo("misc", "ADDON: %s was known, but didn't match known CRC (0x%x)!", addon.Name.c_str(), savedAddon->CRC);
                else
                    sLog->outInfo("misc", "ADDON: %s was known, CRC is correct (0x%x)", addon.Name.c_str(), savedAddon->CRC);
            }
            else
            {
                AddonMgr::SaveAddon(addon);

                sLog->outInfo("misc", "ADDON: %s (0x%x) was not known, saving...", addon.Name.c_str(), addon.CRC);
            }
        ...
        sLog->outDebug("network", "ADDON: CurrentTime: %u", currentTime);

        if (addonInfo.rpos() != addonInfo.size())
            sLog->outDebug("network", "packet under-read!");
    }
    else
        sLog->outError("misc", "Addon packet uncompress error!");
}
...
void WorldSession::SendAddonsInfo()
{
    ...
    for (AddonsList::iterator itr = m_addonsList.begin(); itr != m_addonsList.end(); ++itr)
    {
        ...
        if (usePublicKey)
        {
            sLog->outInfo("misc", "ADDON: CRC (0x%x) for addon %s is wrong (does not match expected 0x%x), sending pubkey",
...
{
    sLog->outDebug("network", "WORLD: Received CMSG_UNREGISTER_ALL_ADDON_PREFIXES");
...
{
    sLog->outDebug("network", "WORLD: Received CMSG_ADDON_REGISTERED_PREFIXES");
