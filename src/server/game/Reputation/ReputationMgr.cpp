...
bool ReputationMgr::IsAtWar(uint32 faction_id) const
{
    ...
    if (!factionEntry)
    {
        sLog->outError("misc", "ReputationMgr::IsAtWar: Can't get AtWar flag of %s for unknown faction (faction id) #%u.", _player->GetName(), faction_id);
        return 0;
    }
...
int32 ReputationMgr::GetReputation(uint32 faction_id) const
{
    ...
    if (!factionEntry)
    {
        sLog->outError("misc", "ReputationMgr::GetReputation: Can't get reputation of %s for unknown faction (faction id) #%u.", _player->GetName(), faction_id);
        return 0;
    }
