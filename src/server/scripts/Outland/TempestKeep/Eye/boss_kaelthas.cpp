    ...
    public:
        ...
        struct boss_kaelthasAI : public ScriptedAI
        {
            ...
            void StartEvent()
            {
                ...
                {
                    sLog->outError("scripts", "Kael'Thas One or more advisors missing, Skipping Phases 1-3");
                    ...
                    case 3:
                    {
                        if (PhaseSubphase == 0)
                        {
                            ...
                            for (uint8 i = 0; i < MAX_ADVISORS; ++i)
                            {
                                ...
                                if (!Advisor)
                                    sLog->outError("scripts", "SD2: Kael'Thas Advisor %u does not exist. Possibly despawned? Incorrectly Killed?", i);
                                else
                    ...
                    case 6:
                    {
                        ...
                        if (!InGravityLapse && !ChainPyros && Phase != 5)
                        {
                            ...
                            if (MindControl_Timer <= diff)
                            {
                                ...
                                {
                                    sLog->outDebug("scripts", "SD2: Kael'Thas mind control not supported.");
                                    ...
                                    case 1:
                                        ...
                                        for (i = me->getThreatManager().getThreatList().begin(); i != me->getThreatManager().getThreatList().end(); ++i)
                                        {
                                            if (Unit* unit = Unit::GetUnit(*me, (*i)->getUnitGuid()))
                                            {
                                                ...
                                                data.WriteBitSeq<0, 1, 6, 5, 7, 2, 3, 4>(guid);
                                                data << uint32(0);          //! movement counter
                                                data.WriteByteSeq<4, 3, 2, 0, 1, 5, 7, 6>(guid);
