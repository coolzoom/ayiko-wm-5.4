...
void npc_escortAI::AttackStart(Unit* who)
{
    ...
    {
        TC_LOG_ERROR("network.opcode", "npc_escortAI::AttackStart - attacker " UI64FMTD " (entry %u), attacking target " UI64FMTD " (entry %u)", me->GetGUID(), me->GetEntry(), who->GetGUID(), who->GetEntry());
...
void npc_escortAI::Start(bool isActiveAttacker /* = true*/, bool run /* = false */, uint64 playerGUID /* = 0 */, Quest const* quest /* = NULL */, bool instantRespawn /* = false */, bool canLoopPath /* = false */, bool resetWaypoints /* = true */)
{
    ...
    if (WaypointList.empty())
    {
        TC_LOG_ERROR("sql.sql", "TSCR: EscortAI (script: %s, creature entry: %u) starts with 0 waypoints (possible missing entry in script_waypoint. Quest: %u).",
