...
bool AddonHandler::BuildAddonPacket(WorldPacket* source, WorldPacket* target)
{
    ...
    *source >> tempValue;                                   // get real size of the packed structure
    ...
    ByteBuffer addonPacked(addonRealSize);                  // resize target for zlib action

    if (uncompress(const_cast<uint8*>(addonPacked.contents()), &addonRealSize, source->contents() + currentPosition, source->size() - currentPosition) == Z_OK)
    {
        ...
        for (uint32 i = 0; i < addonsCount; ++i)
        {
            ...
            sLog->outDebug("network", "ADDON: Name: %s, Enabled: 0x%x, CRC: 0x%x, Unknown2: 0x%x", addonName.c_str(), enabled, crc, unk2);
    ...
    {
        sLog->outError("network", "Addon packet uncompress error :(");
        return false;
    }
    return true;
}
