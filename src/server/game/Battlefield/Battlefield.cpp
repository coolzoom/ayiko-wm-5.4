...
void Battlefield::InitStalker(uint32 entry, float x, float y, float z, float o)
{
    if (Creature* creature = SpawnCreature(entry, x, y, z, o, TEAM_NEUTRAL))
        ...
        sLog->outError("bg.battlefield", "Battlefield::InitStalker: could not spawn Stalker (Creature entry %u), zone messeges will be un-available", entry);
}
...
BfGraveyard* Battlefield::GetGraveyardById(uint32 id)
{
    if (id < m_GraveyardList.size())
    {
        if (m_GraveyardList[id])
            ...
            sLog->outError("bg.battlefield", "Battlefield::GetGraveyardById Id:%u not existed", id);
    }
    else
        sLog->outError("bg.battlefield", "Battlefield::GetGraveyardById Id:%u cant be found", id);
...
void BfGraveyard::SetSpirit(Creature* spirit, TeamId team)
{
    if (!spirit)
    {
        sLog->outError("bg.battlefield", "BfGraveyard::SetSpirit: Invalid Spirit.");
        return;
    }
...
Creature* Battlefield::SpawnCreature(uint32 entry, float x, float y, float z, float o, TeamId team)
{
    ...
    if (!map)
    {
        sLog->outError("bg.battlefield", "Battlefield::SpawnCreature: Can't create creature entry: %u map not found", entry);
        return 0;
    }
    ...
    {
        sLog->outError("bg.battlefield", "Battlefield::SpawnCreature: Can't create creature entry: %u", entry);
        delete creature;
        return NULL;
    }
    ...
    if (!cinfo)
    {
        sLog->outError("bg.battlefield", "Battlefield::SpawnCreature: entry %u does not exist.", entry);
        return NULL;
    }
...
GameObject* Battlefield::SpawnGameObject(uint32 entry, float x, float y, float z, float o)
{
    ...
    {
        sLog->outError("bg.battlefield", "Battlefield::SpawnGameObject: Gameobject template %u not found in database! Battlefield not created!", entry);
        sLog->outError("bg.battlefield", "Battlefield::SpawnGameObject: Cannot create gameobject template %u! Battlefield not created!", entry);
        delete go;
        return NULL;
    }
...
bool BfCapturePoint::SetCapturePointData(GameObject* capturePoint)
{
    ...
    sLog->outDebug("bg.battlefield", "Creating capture point %u", capturePoint->GetEntry());
    ...
    {
        sLog->outError("misc", "OutdoorPvP: GO %u is not capture point!", capturePoint->GetEntry());
        return false;
    }
...
bool BfCapturePoint::Update(uint32 diff)
{
    ...
    {
        //sLog->outError("bg.battlefield", "%u->%u", m_OldState, m_State);
        if (oldTeam != m_team)
            ChangeTeam(oldTeam);
        return true;
    }
