...
class WorldSession
{
    public:
        ...
        static void WriteMovementInfo(WorldPacket& data, MovementInfo const *mi);
        ...
        void SendSetPhaseShift(PhaseShiftSet const &phaseIds, PhaseShiftSet const &terrainswaps, PhaseShiftSet const& worldMapAreaIds);
        ...
        void SendAuctionBidderNotification(uint32 location, uint32 auctionId, uint64 bidder, uint64 bidSum, uint64 diff, uint32 item_template);
    ...
    private:
        ...
        // EnumData helpers
        bool IsLegitCharacterForAccount(uint32 lowGUID) const
        {
        ...
        std::unordered_set<uint32> _legitCharacters;
