            ...
            switch (creature->GetEntry())
            {
                ...
                case 18796:
                    if (creature->isAlive())
                    {
                        ...
                        sLog->outDebug("scripts", "Shadow Labyrinth: counting %u Fel Overseers.", m_uiFelOverseerCount);
                    }
                    break;
            }
        }
        ...
        void SetData(uint32 type, uint32 uiData)
        {
            switch (type)
            {
                ...
                case TYPE_OVERSEER:
                    if (uiData != DONE)
                    {
                        sLog->outError("scripts", "Shadow Labyrinth: TYPE_OVERSEER did not expect other data than DONE");
                        return;
                    }
                    if (m_uiFelOverseerCount)
                    {
                        ...
                        if (m_uiFelOverseerCount)
                            sLog->outDebug("scripts", "Shadow Labyrinth: %u Fel Overseers left to kill.", m_uiFelOverseerCount);
                        else
                        {
                            m_auiEncounter[1] = DONE;
                            sLog->outDebug("scripts", "Shadow Labyrinth: TYPE_OVERSEER == DONE");
                        }
                    }
                    break;
