...
void MotionMaster::MoveRandom(float spawndist)
{
    if (_owner->GetTypeId() == TYPEID_UNIT)
    {
        sLog->outDebug("misc", "Creature (GUID: %u) start moving random", _owner->GetGUIDLow());
...
void MotionMaster::MoveTargetedHome()
{
    ...
    {
        sLog->outDebug("misc", "Creature (Entry: %u GUID: %u) targeted home", _owner->GetEntry(), _owner->GetGUIDLow());
        Mutate(new HomeMovementGenerator<Creature>(), MOTION_SLOT_ACTIVE);
    }
    else if (_owner->GetTypeId()==TYPEID_UNIT && ((Creature*)_owner)->GetCharmerOrOwnerGUID())
    {
        sLog->outDebug("misc", "Pet or controlled creature (Entry: %u GUID: %u) targeting home", _owner->GetEntry(), _owner->GetGUIDLow());
        Unit *target = ((Creature*)_owner)->GetCharmerOrOwner();
        if (target)
        {
            sLog->outDebug("misc", "Following %s (GUID: %u)", target->GetTypeId() == TYPEID_PLAYER ? "player" : "creature", target->GetTypeId() == TYPEID_PLAYER ? target->GetGUIDLow() : ((Creature*)target)->GetDBTableGUIDLow());
            Mutate(new FollowMovementGenerator<Creature>(*target,PET_FOLLOW_DIST,PET_FOLLOW_ANGLE), MOTION_SLOT_ACTIVE);
        }
    }
    else
    {
        sLog->outError("misc", "Player (GUID: %u) attempt targeted home", _owner->GetGUIDLow());
    }
}
...
void MotionMaster::MoveConfused()
{
    ...
    {
        sLog->outDebug("misc", "Player (GUID: %u) move confused", _owner->GetGUIDLow());
        Mutate(new ConfusedMovementGenerator<Player>(), MOTION_SLOT_CONTROLLED);
    }
    else
    {
        sLog->outDebug("misc", "Creature (Entry: %u GUID: %u) move confused",
...
void MotionMaster::MoveChase(Unit* target, float dist, float angle)
{
    ...
    if (_owner->GetTypeId() == TYPEID_PLAYER)
    {
        sLog->outDebug("misc", "Player (GUID: %u) chase to %s (GUID: %u)",
            _owner->GetGUIDLow(),
    ...
    {
        sLog->outDebug("misc", "Creature (Entry: %u GUID: %u) chase to %s (GUID: %u)",
            _owner->GetEntry(), _owner->GetGUIDLow(),
            target->GetTypeId() == TYPEID_PLAYER ? "player" : "creature",
            target->GetTypeId() == TYPEID_PLAYER ? target->GetGUIDLow() : target->ToCreature()->GetDBTableGUIDLow());
...
void MotionMaster::MoveFollow(Unit* target, float dist, float angle, MovementSlot slot)
{
    ...
    if (_owner->GetTypeId() == TYPEID_PLAYER)
    {
        sLog->outDebug("misc", "Player (GUID: %u) follow to %s (GUID: %u)", _owner->GetGUIDLow(),
            target->GetTypeId() == TYPEID_PLAYER ? "player" : "creature",
            target->GetTypeId() == TYPEID_PLAYER ? target->GetGUIDLow() : target->ToCreature()->GetDBTableGUIDLow());
    ...
    {
        sLog->outDebug("misc", "Creature (Entry: %u GUID: %u) follow to %s (GUID: %u)",
            _owner->GetEntry(), _owner->GetGUIDLow(),
            target->GetTypeId() == TYPEID_PLAYER ? "player" : "creature",
            target->GetTypeId() == TYPEID_PLAYER ? target->GetGUIDLow() : target->ToCreature()->GetDBTableGUIDLow());
...
void MotionMaster::MovePoint(uint32 id, float x, float y, float z)
{
    if (_owner->GetTypeId() == TYPEID_PLAYER)
    {
        sLog->outDebug("misc", "Player (GUID: %u) targeted point (Id: %u X: %f Y: %f Z: %f)", _owner->GetGUIDLow(), id, x, y, z);
        Mutate(new PointMovementGenerator<Player>(id, x, y, z), MOTION_SLOT_ACTIVE);
    }
    else
    {
        sLog->outDebug("misc", "Creature (Entry: %u GUID: %u) targeted point (ID: %u X: %f Y: %f Z: %f)",
...
void MotionMaster::MoveLand(uint32 id, Position const& pos)
{
    ...
    sLog->outDebug("misc", "Creature (Entry: %u) landing point (ID: %u X: %f Y: %f Z: %f)", _owner->GetEntry(), id, x, y, z);
...
{
    sLog->outDebug("misc", "Creature (Entry: %u) take off point (ID: %u X: %f Y: %f Z: %f)", _owner->GetEntry(), id, x, y, z);
...
{
    sLog->outDebug("misc", "Unit (GUID: %u) jump to point (X: %f Y: %f Z: %f)", _owner->GetGUIDLow(), x, y, z);
...
void MotionMaster::MoveFall(uint32 id/*=0*/)
{
    ...
    {
        sLog->outDebug("misc", "MotionMaster::MoveFall: unable retrive a proper height at map %u (x: %f, y: %f, z: %f).",
...
void MotionMaster::MoveCharge(float x, float y, float z, float speed, uint32 id)
{
    ...
    if (_owner->GetTypeId() == TYPEID_PLAYER)
    {
        sLog->outDebug("misc", "Player (GUID: %u) charge point (X: %f Y: %f Z: %f)", _owner->GetGUIDLow(), x, y, z);
        Mutate(new PointMovementGenerator<Player>(id, x, y, z, speed), MOTION_SLOT_CONTROLLED);
    }
    else
    {
        sLog->outDebug("misc", "Creature (Entry: %u GUID: %u) charge point (X: %f Y: %f Z: %f)",
...
void MotionMaster::MoveSeekAssistance(float x, float y, float z)
{
    if (_owner->GetTypeId() == TYPEID_PLAYER)
    {
        sLog->outError("misc", "Player (GUID: %u) attempt to seek assistance", _owner->GetGUIDLow());
    }
    else
    {
        sLog->outDebug("misc", "Creature (Entry: %u GUID: %u) seek assistance (X: %f Y: %f Z: %f)",
            _owner->GetEntry(), _owner->GetGUIDLow(), x, y, z);
        _owner->AttackStop();
...
void MotionMaster::MoveSeekAssistanceDistract(uint32 time)
{
    if (_owner->GetTypeId() == TYPEID_PLAYER)
    {
        sLog->outError("misc", "Player (GUID: %u) attempt to call distract after assistance", _owner->GetGUIDLow());
    }
    else
    {
        sLog->outDebug("misc", "Creature (Entry: %u GUID: %u) is distracted after assistance call (Time: %u)",
...
void MotionMaster::MoveFleeing(Unit* enemy, uint32 time)
{
    ...
    if (_owner->GetTypeId() == TYPEID_PLAYER)
    {
        sLog->outDebug("misc", "Player (GUID: %u) flee from %s (GUID: %u)", _owner->GetGUIDLow(),
    ...
    {
        sLog->outDebug("misc", "Creature (Entry: %u GUID: %u) flee from %s (GUID: %u)%s",
            _owner->GetEntry(), _owner->GetGUIDLow(),
            enemy->GetTypeId() == TYPEID_PLAYER ? "player" : "creature",
            enemy->GetTypeId() == TYPEID_PLAYER ? enemy->GetGUIDLow() : enemy->ToCreature()->GetDBTableGUIDLow(),
...
void MotionMaster::MoveTaxiFlight(uint32 path, uint32 pathnode)
{
    if (_owner->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        {
            sLog->outError("misc", "%s attempt taxi to (not existed Path %u node %u)",
            _owner->GetName(), path, pathnode);
        }
    }
    else
    {
        sLog->outError("misc", "Creature (Entry: %u GUID: %u) attempt taxi to (Path %u node %u)",
...
void MotionMaster::MoveDistract(uint32 timer)
{
    ...
    if (_owner->GetTypeId() == TYPEID_PLAYER)
    {
        sLog->outDebug("misc", "Player (GUID: %u) distracted (timer: %u)", _owner->GetGUIDLow(), timer);
    }
    else
    {
        sLog->outDebug("misc", "Creature (Entry: %u GUID: %u) (timer: %u)",
...
void MotionMaster::MovePath(uint32 path_id, bool repeatable)
{
    ...
    sLog->outDebug("misc", "%s (GUID: %u) start moving over path(Id:%u, repeatable: %s)",
...
{
    sLog->outFatal("misc", "Unit (Entry %u) is trying to delete its updating MG (Type %u)!", _owner->GetEntry(), curr->GetMovementGeneratorType());
    if (isStatic(curr))
        return;
    if (!_expList)
