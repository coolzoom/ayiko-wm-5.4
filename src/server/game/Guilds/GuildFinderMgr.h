...
struct MembershipRequest
{
    public:
        ...
        MembershipRequest(uint32 playerGUID, uint32 guildId, uint32 availability,
                          uint32 classRoles, uint32 interests,
                          std::string const &comment, time_t submitTime)
        ...
        std::string const& GetName() const
        {
            ...
            auto const nameData = sWorld->GetCharacterNameData(GetPlayerGUID());
            return nameData ? nameData->m_name : defaultName;
        }
...
struct LFGuildPlayer
{
    public:
        ...
        LFGuildPlayer(uint32 guid, uint8 role, uint8 availability, uint8 interests,
                      uint8 level, std::string const &comment)
            : _comment(comment)
        {
            _guid = guid;
            _roles = role;
...
struct LFGuildSettings : public LFGuildPlayer
{
    public:
        ...
        LFGuildSettings(bool listed, TeamId team, uint32 guid, uint8 role,
                        uint8 availability, uint8 interests, uint8 level)
            : LFGuildPlayer(guid, role, availability, interests, level)
            , _listed(listed)
            , _team(team)
        { }
        ...
        LFGuildSettings(bool listed, TeamId team, uint32 guid, uint8 role,
            ...
            : LFGuildPlayer(guid, role, availability, interests, level, comment)
            , _listed(listed)
            , _team(team)
        { }
