...
public:
    ...
    ChatCommand* GetCommands() const override
    {
        ...
        static ChatCommand modifyCommandTable[] =
        {
            ...
            { "rating",       rbac::RBAC_PERM_COMMAND_MODIFY_RATING,       false, &HandleModifyRatingCommand,        "", NULL },
            { NULL,           0,                                     false, NULL,                              "", NULL }
        };
    ...
    static bool HandleModifyRatingCommand(ChatHandler* handler, char const* args)
    {
        Player* player = handler->GetSession()->GetPlayer();
        if (!player)
            return false;

        char* rating = strtok((char*)args, " ");
        if (!rating)
            return false;

        CombatRating ratingId = (CombatRating)atoi(rating);

        if (ratingId > CR_MASTERY)
            return false;

        char* value = strtok(NULL, " ");
        if (!value)
            return false;
        int32 val = (int32)atoi(value);

        if (!val)
            return false;

        player->ApplyRatingMod(ratingId, val, true);
        return true;
    }
    ...
    static bool HandleModifyMoneyCommand(ChatHandler* handler, const char* args)
    {
        ...
        auto moneyToAdd = Trinity::lexicalCast<int64>(args);
