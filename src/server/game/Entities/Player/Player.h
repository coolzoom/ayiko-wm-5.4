...
#ifndef TRINITY_GAME_PLAYER_H
#define TRINITY_GAME_PLAYER_H
...
struct PlayerSpell
{
    ...
    bool active;                                // show in spellbook
    bool dependent;                             // learned as result another spell learn, skill grow, quest reward, etc
    bool disabled;                              // first rank has been learned in result talent learn but currently talent unlearned, save max learned ranks
};
...
#define MAX_MONEY_AMOUNT               (UI64LIT(10000000000)) // @TODO: Move this restriction to worldserver.conf, default to this value, hardcap at uint64.max
...
enum DungeonStatusFlag
{
    ...
    RAID_STATUSFLAG_RAID_TOOL = 0x10
};
...
{
    auraEffectData(uint8 slot, uint8 effect, int32 amount, int32 baseamount)
...
class Player final : public Unit, public GridObject<Player>
{
    ...
    public:
        ...
        Pet* GetPet() const;
        Pet* SummonPet(uint32 entry, float x, float y, float z, float ang, uint32 despwtime, bool stampeded = false);

        void RemovePet(PetRemoveMode mode, uint8 removeFlags = PET_REMOVE_FLAG_NONE);

        PhaseMgr& GetPhaseMgr() { return m_phaseMgr; }
        ...
        */
        ...
        void SetQuestSlot(uint16 slot, uint32 quest_id, uint32 timer = 0)
        {
            ...
            SetUInt64Value(PLAYER_QUEST_LOG_1_1 + slot * MAX_QUEST_OFFSET + QUEST_COUNTS_OFFSET, 0);
        ...
        uint32 GetRemainingSpellCooldown(uint32 spellId) const;

        void AddSpellAndCategoryCooldowns(SpellInfo const* spellInfo, uint32 itemId, Spell* spell = NULL, bool infinityCooldown = false);
        void AddSpellCooldown(uint32 spell_id, uint32 itemid, uint32 delay, bool sendCooldownPacket = false);
        ...
        void ReduceSpellCooldown(uint32 spellId, int32 reduction);
        ...
        // Arena
        uint32 GetArenaPersonalRating(uint8 slot) const { return m_ArenaPersonalRating[slot]; }
        uint32 GetBestRatingOfWeek(uint8 slot) const { return m_BestRatingOfWeek[slot]; }
        uint32 GetBestRatingOfSeason(uint8 slot) const { return m_BestRatingOfSeason[slot]; }
        uint32 GetWeekWins(uint8 slot) const { return m_WeekWins[slot]; }
        uint32 GetPrevWeekWins(uint8 slot) const { return m_PrevWeekWins[slot]; }
        uint32 GetSeasonWins(uint8 slot) const { return m_SeasonWins[slot]; }
        uint32 GetWeekGames(uint8 slot) const { return m_WeekGames[slot]; }
        ...
        void IncrementWeekGames(uint8 slot) { ++m_WeekGames[slot]; }
        void IncrementWeekWins(uint8 slot) { ++m_WeekWins[slot]; }
        void IncrementSeasonGames(uint8 slot) { ++m_SeasonGames[slot]; }
        void IncrementSeasonWins(uint8 slot) { ++m_SeasonWins[slot]; }
        void FinishWeek();
        ...
        void UpdateHasteAffectedPowerRegeneration(CombatRating cr, float value, bool apply);
        ...
        void SetPetSlotUsed(uint32 petId, uint8 slot) { m_petsBySlot.at(slot) = petId; }

        void SetPetSlotFree(uint32 petId)
        {
            for (PetSlotVector::iterator i = m_petsBySlot.begin(); i != m_petsBySlot.end(); ++i)
            {
                if (*i == petId)
                {
                    *i = 0;
        ...
        void SwapPetSlots(uint8 first, uint8 second)
        {
            std::swap(m_petsBySlot.at(first), m_petsBySlot.at(second));
        }

        uint32 GetPetIdBySlot(uint8 slot) const { return m_petsBySlot.at(slot); }

        Pet* CreateTamedPetFrom(Creature* creatureTarget, uint32 spellId = 0);
        Pet* CreateTamedPetFrom(uint32 creatureEntry, uint32 spellId = 0);
        bool InitTamedPet(Pet* pet, uint8 level, uint32 spellId);

        PetAuraSet const & GetPetAuras() const { return m_petAuras; }
        ...
        uint32 GetInvitingGroupId() const { return m_invitingGroupId; }
        void SetInvitingGroupId(uint32 groupId) { m_invitingGroupId = groupId; }
    ...
    private:
        ...
        typedef std::vector<uint32> PetSlotVector;
        PetSlotVector m_petsBySlot;
