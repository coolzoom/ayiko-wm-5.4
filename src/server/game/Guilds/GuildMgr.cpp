...
uint32 GuildMgr::GenerateGuildId()
{
    ...
    {
        sLog->outError("guild", "Guild ids overflow!! Can't continue, shutting down server. ");
...
{
    // 1. Load all guilds
    sLog->outInfo("server.loading", "Loading guilds definitions...");
    {
        ...
        if (!result)
        {
            sLog->outInfo("server.loading", ">> Loaded 0 guild definitions. DB table `guild` is empty.");
            return;
        }
        else
        {
            ...
            sLog->outInfo("server.loading", ">> Loaded %u guild definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // 2. Load all guild ranks
    sLog->outInfo("server.loading", "Loading guild ranks...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            sLog->outInfo("server.loading", ">> Loaded 0 guild ranks. DB table `guild_rank` is empty.");
        }
        else
        {
            uint32 count = 0;
            ...
            sLog->outInfo("server.loading", ">> Loaded %u guild ranks in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // 3. Load all guild members
    sLog->outInfo("server.loading", "Loading guild members...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            sLog->outInfo("server.loading", ">> Loaded 0 guild members. DB table `guild_member` is empty.");
        }
        else
        {
            uint32 count = 0;
            ...
            sLog->outInfo("server.loading", ">> Loaded %u guild members int %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // 4. Load all guild bank tab rights
    sLog->outInfo("server.loading", "Loading bank tab rights...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            sLog->outInfo("server.loading", ">> Loaded 0 guild bank tab rights. DB table `guild_bank_right` is empty.");
        }
        else
        {
            uint32 count = 0;
            ...
            sLog->outInfo("server.loading", ">> Loaded %u bank tab rights in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // 5. Load all event logs
    sLog->outInfo("server.loading", "Loading guild event logs...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            sLog->outInfo("server.loading", ">> Loaded 0 guild event logs. DB table `guild_eventlog` is empty.");
        }
        else
        {
            uint32 count = 0;
            ...
            sLog->outInfo("server.loading", ">> Loaded %u guild event logs in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // 6. Load all bank event logs
    sLog->outInfo("server.loading", "Loading guild bank event logs...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            sLog->outInfo("server.loading", ">> Loaded 0 guild bank event logs. DB table `guild_bank_eventlog` is empty.");
        }
        else
        {
            uint32 count = 0;
            ...
            sLog->outInfo("server.loading", ">> Loaded %u guild bank event logs in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // 7. Load all guild bank tabs
    sLog->outInfo("server.loading", "Loading guild bank tabs...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            sLog->outInfo("server.loading", ">> Loaded 0 guild bank tabs. DB table `guild_bank_tab` is empty.");
        }
        else
        {
            uint32 count = 0;
            ...
            sLog->outInfo("server.loading", ">> Loaded %u guild bank tabs in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // 8. Fill all guild bank tabs
    sLog->outInfo("guild", "Filling bank tabs with items...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            sLog->outInfo("server.loading", ">> Loaded 0 guild bank tab items. DB table `guild_bank_item` or `item_instance` is empty.");
        }
        else
        {
            uint32 count = 0;
            ...
            sLog->outInfo("server.loading", ">> Loaded %u guild bank tab items in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }
    ...
    sLog->outInfo("misc", "Loading Guild News");
    {
        for (GuildContainer::const_iterator itr = GuildStore.begin(); itr != GuildStore.end(); ++itr)
        {
    ...
    sLog->outInfo("misc", "Validating data of loaded guilds...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        sLog->outInfo("server.loading", ">> Validated data of loaded guilds in %u ms", GetMSTimeDiffToNow(oldMSTime));
    }
}
...
void GuildMgr::LoadGuildXpForLevel()
{
    ...
    if (!result)
    {
        sLog->outError("server.loading", ">> Loaded 0 xp for guild level definitions. DB table `guild_xp_for_level` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            sLog->outInfo("misc", "Unused (> Guild.MaxLevel in worldserver.conf) level %u in `guild_xp_for_level` table, ignoring.", uint32(level));
            continue;
        }
    ...
    for (uint8 level = 1; level < sWorld->getIntConfig(CONFIG_GUILD_MAX_LEVEL); ++level)
    {
        ...
        {
            sLog->outError("sql.sql", "Level %i does not have XP for guild level data. Using data of level [%i] + 1660000.", level+1, level);
            GuildXPperLevel[level] = GuildXPperLevel[level - 1] + 1660000;
        }
    }

    sLog->outInfo("server.loading", ">> Loaded %u xp for guild level definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void GuildMgr::LoadGuildRewards()
{
    ...
    if (!result)
    {
        sLog->outError("server.loading", ">> Loaded 0 guild reward definitions. DB table `guild_rewards` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            sLog->outError("server.loading", "Guild rewards contains not existing item entry %u", reward.Entry);
            continue;
        }

        if (reward.AchievementId != 0 && (!sAchievementStore.LookupEntry(reward.AchievementId)))
        {
            sLog->outError("server.loading", "Guild rewards contains not existing achievement entry %u", reward.AchievementId);
            continue;
        }

        if (reward.Standing >= MAX_REPUTATION_RANK)
        {
            sLog->outError("server.loading", "Guild rewards contains wrong reputation standing %u, max is %u", uint32(reward.Standing), MAX_REPUTATION_RANK - 1);
            continue;
        }
    ...
    sLog->outInfo("server.loading", ">> Loaded %u guild reward definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
