...
{
    return m_petBattle->Teams[!m_caster->GetTeamIndex()]->GetActivePet();
}

void BattlePetAbilityEffect::AddTargets()
{
    auto casterTeam = m_petBattle->Teams[m_caster->GetTeamIndex()];
    ...
    switch (Handlers[m_effectEntry->EffectProperty].Target)
    {
        ...
        {
            for (auto &team : m_petBattle->Teams)
...
uint32 BattlePetAbilityEffect::CalculateDamage(uint32 damage)
{
    ...
    m_flags |= modPetTypePct > 0 ? PET_BATTLE_EFFECT_FLAG_STRONG | PET_BATTLE_EFFECT_FLAG_KILL
        : (modPetTypePct < 0 ? PET_BATTLE_EFFECT_FLAG_WEAK | PET_BATTLE_EFFECT_FLAG_KILL : 0);
...
void BattlePetAbilityEffect::SetHealth(BattlePet* target, uint32 value)
{
    ...
    if (!(m_flags & FailFlags) && value == target->GetCurrentHealth())
        m_flags |= PET_BATTLE_EFFECT_FLAG_MISS;
