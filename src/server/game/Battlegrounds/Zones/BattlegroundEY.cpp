    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        ...
        if (obj)
        {
            ...
            while (j < m_PlayersNearPoint[EY_POINTS_MAX].size())
            {
                ...
                if (!player)
                {
                    sLog->outError("bg.battleground", "BattlegroundEY:CheckSomeoneJoinedPoint: Player (GUID: %u) not found!", GUID_LOPART(m_PlayersNearPoint[EY_POINTS_MAX][j]));
                    ++j;
                    continue;
                }
    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        ...
        if (obj)
        {
            ...
            while (j < m_PlayersNearPoint[i].size())
            {
                ...
                if (!player)
                {
                    sLog->outError("bg.battleground", "BattlegroundEY:CheckSomeoneLeftPoint Player (GUID: %u) not found!", GUID_LOPART(m_PlayersNearPoint[i][j]));
...
bool BattlegroundEY::SetupBattleground()
{
    ...
    {
        sLog->outError("sql.sql", "BatteGroundEY: Failed to spawn some object Battleground not created!");
        return false;
    }
    ...
    for (int i = 0; i < EY_POINTS_MAX; ++i)
    {
        ...
        if (!at)
        {
            sLog->outError("bg.battleground", "BattlegroundEY: Unknown trigger: %u", m_Points_Trigger[i]);
            continue;
        }
        if (!AddObject(BG_EY_OBJECT_SPEEDBUFF_FEL_REAVER + i * 3, Buff_Entries[0], at->x, at->y, at->z, 0.907571f, 0, 0, 0.438371f, 0.898794f, RESPAWN_ONE_DAY)
            ...
            sLog->outError("bg.battleground", "BattlegroundEY: Cannot spawn buff");
    }
    ...
    {
        sLog->outError("sql.sql", "BatteGroundEY: Failed to spawn spirit guide! Battleground not created!");
        return false;
    }
...
void BattlegroundEY::RespawnFlagAfterDrop()
{
    ...
    if (obj)
        ...
        sLog->outError("bg.battleground", "BattlegroundEY: Unknown dropped flag guid: %u", GUID_LOPART(GetDroppedFlagGUID()));
...
void BattlegroundEY::EventTeamCapturedPoint(Player* Source, uint32 Point)
{
    ...
    if (!sg || !AddSpiritGuide(Point, sg->x, sg->y, sg->z, 3.124139f, Team))
        sLog->outError("bg.battleground", "BatteGroundEY: Failed to spawn spirit guide! point: %u, team: %u, graveyard_id: %u",
...
WorldSafeLocsEntry const* BattlegroundEY::GetClosestGraveYard(Player* player)
{
    ...
    if (!entry)
    {
        sLog->outError("bg.battleground", "BattlegroundEY: Not found the main team graveyard. Graveyard system isn't working!");
        return NULL;
    }
    ...
    for (uint8 i = 0; i < EY_POINTS_MAX; ++i)
    {
        if (m_PointOwnedByTeam[i] == player->GetBGTeam() && m_PointState[i] == EY_POINT_UNDER_CONTROL)
        {
            ...
            if (!entry)
                sLog->outError("bg.battleground", "BattlegroundEY: Not found graveyard: %u", m_CapturingPointTypes[i].GraveYardId);
            else
            {
                distance = (entry->x - plr_x)*(entry->x - plr_x) + (entry->y - plr_y)*(entry->y - plr_y) + (entry->z - plr_z)*(entry->z - plr_z);
