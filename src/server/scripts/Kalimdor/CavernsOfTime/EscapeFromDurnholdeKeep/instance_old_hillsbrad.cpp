...
public:
    ...
    struct instance_old_hillsbrad_InstanceMapScript : public InstanceScript
    {
        ...
        Player* GetPlayerInMap()
        {
            ...
            sLog->outDebug("scripts", "Instance Old Hillsbrad: GetPlayerInMap, but PlayerList is empty!");
            return NULL;
        }
        ...
        void SetData(uint32 type, uint32 data)
        {
            ...
            if (!player)
            {
                sLog->outDebug("scripts", "Instance Old Hillsbrad: SetData (Type: %u Data %u) cannot find any player.", type, data);
                return;
            }
            ...
            switch (type)
            {
                case TYPE_BARREL_DIVERSION:
                {
                    if (data == IN_PROGRESS)
                    {
                        ...
                        sLog->outDebug("scripts", "Instance Old Hillsbrad: go_barrel_old_hillsbrad count %u", mBarrelCount);
                ...
                case TYPE_THRALL_EVENT:
                {
                    if (data == FAIL)
                    {
                        if (mThrallEventCount <= 20)
                        {
                            ...
                            sLog->outDebug("scripts", "Instance Old Hillsbrad: Thrall event failed %u times. Resetting all sub-events.", mThrallEventCount);
                        ...
                        m_auiEncounter[1] = data;
                    sLog->outDebug("scripts", "Instance Old Hillsbrad: Thrall escort event adjusted to data %u.", data);
                    break;
                }
                case TYPE_THRALL_PART1:
                    m_auiEncounter[2] = data;
                    sLog->outDebug("scripts", "Instance Old Hillsbrad: Thrall event part I adjusted to data %u.", data);
                    break;
                case TYPE_THRALL_PART2:
                    m_auiEncounter[3] = data;
                    sLog->outDebug("scripts", "Instance Old Hillsbrad: Thrall event part II adjusted to data %u.", data);
                    break;
                case TYPE_THRALL_PART3:
                    m_auiEncounter[4] = data;
                    sLog->outDebug("scripts", "Instance Old Hillsbrad: Thrall event part III adjusted to data %u.", data);
                    break;
                case TYPE_THRALL_PART4:
                    m_auiEncounter[5] = data;
                     sLog->outDebug("scripts", "Instance Old Hillsbrad: Thrall event part IV adjusted to data %u.", data);
                    break;
            }
        }
