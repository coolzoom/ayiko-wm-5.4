...
void MapManager::LoadTransports()
{
    ...
    if (!result)
    {
        sLog->outInfo("server.loading", ">> Loaded 0 transports. DB table `transports` is empty!");
        return;
    }
    ...
    do
    {
        ...
        if (!goinfo)
        {
            sLog->outError("sql.sql", "Transport ID:%u, Name: %s, will not be loaded, gameobject_template missing", entry, name.c_str());
            continue;
        }

        if (goinfo->type != GAMEOBJECT_TYPE_MO_TRANSPORT)
        {
            sLog->outError("sql.sql", "Transport ID:%u, Name: %s, will not be loaded, gameobject_template type wrong", entry, name.c_str());
            continue;
        }

        // sLog->outInfo("server.loading", "Loading transport %d between %s, %s", entry, name.c_str(), goinfo->name);
        ...
        {
            sLog->outError("sql.sql", "Transport (path id %u) path size = 0. Transport ignored, check DBC files or transport GO data0 field.", goinfo->moTransport.taxiPathId);
            delete t;
            continue;
        }
    ...
    if (result)                                              // wrong data found
    {
        do
        {
            ...
            uint32 entry = fields[1].GetUInt32();
            std::string name = fields[2].GetString();
            sLog->outError("sql.sql", "Transport %u '%s' have record (GUID: %u) in `gameobject`. Transports must not have any records in `gameobject` or its behavior will be unpredictable/bugged.", entry, name.c_str(), guid);
        }
        while (result->NextRow());
    }

    sLog->outInfo("server.loading", ">> Loaded %u transports in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void MapManager::LoadTransportNPCs()
{
    ...
    if (!result)
    {
        sLog->outInfo("server.loading", ">> Loaded 0 transport NPCs. DB table `creature_transport` is empty!");
        return;
    }
    ...
    sLog->outInfo("server.loading", ">> Loaded %u transport npcs in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
bool Transport::Create(uint32 guidlow, uint32 entry, uint32 mapid, float x, float y, float z, float ang, uint32 animprogress, uint32 dynflags)
{
    ...
    if (!IsPositionValid())
    {
        sLog->outError("entities.transport", "Transport (GUID: %u) not created. Suggested coordinates isn't valid (X: %f Y: %f)",
            guidlow, x, y);
        return false;
    }
    ...
    if (!goinfo)
    {
        sLog->outError("sql.sql", "Transport not created: entry in `gameobject_template` not found, guidlow: %u map: %u  (X: %f Y: %f Z: %f) ang: %f", guidlow, mapid, x, y, z, ang);
        return false;
    }
...
bool Transport::GenerateWaypoints(uint32 pathid, std::set<uint32> &mapids)
{
    ...
    //        sLog->outInfo("entities.transport", "%f, %f, %f, %f, %f, %f, %f", keyFrames[i].x, keyFrames[i].y, keyFrames[i].distUntilStop, keyFrames[i].distSinceStop, keyFrames[i].distFromPrev, keyFrames[i].tFrom, keyFrames[i].tTo);
    //    }
    ...
    for (size_t i = 0; i < keyFrames.size() - 1; ++i)
    {
        ...
        if (((d < keyFrames[i + 1].distFromPrev) && (tTo > 0)))
        {
            while ((d < keyFrames[i + 1].distFromPrev) && (tTo > 0))
            {
                ...
                if (d > 0)
                {
                    ...
                    //                    sLog->outInfo("entities.transport", "T: %d, D: %f, x: %f, y: %f, z: %f", t, d, newX, newY, newZ);
                    if (teleport)
        ...
        //        sLog->outInfo("entities.transport", "T: %d, x: %f, y: %f, z: %f, t:%d", t, pos.x, pos.y, pos.z, teleport);
    ...
    //    sLog->outInfo("entities.transport", "    Generated %lu waypoints, total time %u.", (unsigned long)m_WayPoints.size(), timer);
...
bool Transport::AddPassenger(Player* passenger)
{
    if (m_passengers.insert(passenger).second)
        sLog->outInfo("entities.transport", "Player %s boarded transport %s.", passenger->GetName(), GetName());
...
bool Transport::RemovePassenger(Player* passenger)
{
    if (m_passengers.erase(passenger))
        sLog->outInfo("entities.transport", "Player %s removed from transport %s.", passenger->GetName(), GetName());
...
void Transport::Update(uint32 p_diff)
{
    ...
    {
        if (!AIM_Initialize())
            sLog->outError("entities.transport", "Could not initialize GameObjectAI for Transport");
    }
    else
        AI()->UpdateAI(p_diff);
    ...
    while (((m_timer - m_curr->first) % m_pathTime) > ((m_next->first - m_curr->first) % m_pathTime))
    {
        ...
        if (m_curr == m_WayPoints.begin())
            sLog->outDebug("entities.transport", " ************ BEGIN ************** %s", m_name.c_str());

        sLog->outDebug("entities.transport", "%s moved to %d %f %f %f %d", m_name.c_str(), m_curr->second.id, m_curr->second.x, m_curr->second.y, m_curr->second.z, m_curr->second.mapid);
    }
...
void Transport::DoEventIfAny(WayPointMap::value_type const& node, bool departure)
{
    ...
    {
        sLog->outDebug("maps.scripts", "Taxi %s event %u of node %u of %s path", departure ? "departure" : "arrival", eventid, node.first, GetName());
...
Creature * Transport::AddNPCPassengerCreature(uint32 tguid, uint32 entry, float x, float y, float z, float o, uint32 anim)
{
    ...
    {
        sLog->outError("entities.transport", "Creature (guidlow %d, entry %d) not created. Suggested coordinates isn't valid (X: %f Y: %f)", pCreature->GetGUIDLow(), pCreature->GetEntry(), pCreature->GetPositionX(), pCreature->GetPositionY());
        delete pCreature;
        return NULL;
    }
...
uint32 Transport::AddNPCPassenger(uint32 tguid, uint32 entry, float x, float y, float z, float o, uint32 anim)
{
    ...
    {
        sLog->outError("entities.transport", "Creature (guidlow %d, entry %d) not created. Suggested coordinates isn't valid (X: %f Y: %f)", creature->GetGUIDLow(), creature->GetEntry(), creature->GetPositionX(), creature->GetPositionY());
        delete creature;
        return 0;
    }
