...
pAuraEffectHandler AuraEffectHandler[TOTAL_AURAS]=
{
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //412 SPELL_AURA_412
    ...
    if (caster && caster->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        GetAuraType() == SPELL_AURA_PERIODIC_HEAL)
        {
            if (GetBase()->GetType() == UNIT_AURA_TYPE)
            {
                ...
                GetFixedDamageInfo().SetFixedDamage(temp_damage);
                GetFixedDamageInfo().SetCriticalChance(temp_crit);
                ...
                if (GetSpellInfo()->Id == 27243 || GetSpellInfo()->Id == 114790)
                    GetFixedDamageInfo().SetFixedTotalDamage(temp_damage * (GetBase()->GetMaxDuration() / GetAmplitude()));
...
void AuraEffect::HandleEffect(AuraApplication * aurApp, uint8 mode, bool apply)
{
    ...
    auto auraBase = GetBase();
    bool prevented = false;

    if (auraBase)
    {
        if (apply)
            prevented = auraBase->CallScriptEffectApplyHandlers(this, aurApp, (AuraEffectHandleModes)mode);
        else
            prevented = auraBase->CallScriptEffectRemoveHandlers(this, aurApp, (AuraEffectHandleModes)mode);
    }
    ...
    {
        if (apply)
            auraBase->CallScriptAfterEffectApplyHandlers(this, aurApp, (AuraEffectHandleModes)mode);
        else
            auraBase->CallScriptAfterEffectRemoveHandlers(this, aurApp, (AuraEffectHandleModes)mode);
    }
}
...
void AuraEffect::HandleShapeshiftBoosts(Unit* target, bool apply) const
{
    ...
    if (apply)
    {
        ...
        if (auto const plrTarget = target->ToPlayer())
        {
            for (auto const &kvPair : plrTarget->GetSpellMap())
            {
                auto const &playerSpell = kvPair.second;
                if (playerSpell.state == PLAYERSPELL_REMOVED || playerSpell.disabled)
                    continue;

                if (kvPair.first == spellId || kvPair.first == spellId2 || kvPair.first == spellId3 || kvPair.first == spellId4)
                    continue;

                SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(kvPair.first);
                if (!spellInfo || !(spellInfo->Attributes & (SPELL_ATTR0_PASSIVE | SPELL_ATTR0_HIDDEN_CLIENTSIDE)))
                    continue;

                if (spellInfo->Stances & (1 << (GetMiscValue() - 1)))
                    target->CastSpell(target, kvPair.first, true, NULL, this);
            }
            ...
            switch (GetMiscValue())
            {
                ...
                {
                    // Savage Roar
                    if (target->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_DRUID, Trinity::Flag128(0, 0x10000000)))
...
void AuraEffect::HandleModInvisibility(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        // apply glow vision
        //if (target->GetTypeId() == TYPEID_PLAYER)
            //target->SetByteFlag(PLAYER_BYTES2, 3, PLAYER_FIELD_BYTE2_INVISIBILITY_GLOW); // TODO : Check PLAYER_FIELD_AURA_VISION
    ...
    else
    {
        if (!target->HasAuraType(SPELL_AURA_MOD_INVISIBILITY))
        {
            ...
            // remove glow vision
            //if (target->GetTypeId() == TYPEID_PLAYER)
                //target->RemoveByteFlag(PLAYER_BYTES2, 3, PLAYER_FIELD_BYTE2_INVISIBILITY_GLOW); // TODO : Check PLAYER_FIELD_AURA_VISION
...
void AuraEffect::HandleModStealth(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    else
    {
        ...
        if (!target->HasAuraType(SPELL_AURA_MOD_STEALTH) && !target->HasAura(115192)) // if last SPELL_AURA_MOD_STEALTH
        {
            ...
            //if (target->GetTypeId() == TYPEID_PLAYER)
                //target->RemoveByteFlag(PLAYER_BYTES2, 3, PLAYER_FIELD_BYTE2_STEALTH); // TODO : Check PLAYER_FIELD_AURA_VISION
        }
    }
...
void AuraEffect::HandleAuraModShapeshift(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (Player * playerTarget = target->ToPlayer())
    {
        ...
        {
            // Dash
            if (AuraEffect *aurEff = target->GetAuraEffect(SPELL_AURA_MOD_INCREASE_SPEED, SPELLFAMILY_DRUID, Trinity::Flag128(0, 0, 0x8)))
                aurEff->RecalculateAmount();
...
void AuraEffect::HandleModFixateTarget(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (!(caster && target) || !(target->isAlive() && caster->isAlive()) || !caster->CanHaveThreatList())
        return;
...
void AuraEffect::HandleModPowerRegen(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    Player * const target = aurApp->GetTarget()->ToPlayer();
    if (!target)
        return;
    ...
    switch (GetMiscValue())
    {
        ...
        case POWER_ENERGY:
            ...
            break;
        case POWER_FOCUS:
            target->UpdateHasteAffectedPowerRegeneration(CR_HASTE_RANGED, GetAmount(), apply);
            break;
    }
...
void AuraEffect::HandleModMeleeRangedSpeedPct(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (GetAuraType() != SPELL_AURA_MOD_MELEE_RANGED_HASTE)
        return;

    if (GetAmount() > 0.0f && target->GetTypeId() == TYPEID_PLAYER)
    {
        target->ToPlayer()->UpdateHasteAffectedPowerRegeneration(CR_HASTE_MELEE, GetAmount(), apply);
        target->ToPlayer()->UpdateHasteAffectedPowerRegeneration(CR_HASTE_RANGED, GetAmount(), apply);
    }
}
...
void AuraEffect::HandleModCombatSpeedPct(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (GetAuraType() != SPELL_AURA_MELEE_SLOW)
        return;
...
void AuraEffect::HandleModMeleeSpeedPct(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (GetAuraType() != SPELL_AURA_MOD_MELEE_HASTE)
        return;
...
void AuraEffect::HandleModRatingPct(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    auto const target = aurApp->GetTarget()->ToPlayer();
    if (!target)
        return;
...
void AuraEffect::HandleNoReagentUseAura(AuraApplication const* aurApp, uint8 mode, bool /*apply*/) const
{
    ...
    for (auto i = 0; i < 4; ++i)
        target->SetUInt32Value(PLAYER_NO_REAGENT_COST_1 + i, mask[i]);
}
...
void AuraEffect::HandleAuraOpenStable(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
        target->GetSession()->SendPetList(0, PET_SLOT_ACTIVE_FIRST, PET_SLOT_STABLE_LAST);
            ...
            switch (GetSpellInfo()->Id)
            {
                ...
                case 51690:
                {
                    ...
                    UnitList targets;
                    {
                        ...
                        TypeContainerVisitor<Trinity::UnitListSearcher<Trinity::AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck>, Grid::GridObjectMap> grid_object_checker(checker);
                        TypeContainerVisitor<Trinity::UnitListSearcher<Trinity::AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck>, Grid::WorldObjectMap> world_object_checker(checker);
...
void AuraEffect::HandlePeriodicDamageAurasTick(Unit* target, Unit* caster) const
{
    ...
    if (GetFixedDamageInfo().HasCritChance())
        crit = roll_chance_f(GetFixedDamageInfo().GetCriticalChance());
    else
...
void AuraEffect::HandleProcTriggerSpellWithValueAuraProc(AuraApplication* aurApp, ProcEventInfo& eventInfo)
{
    ...
    uint32 triggerSpellId = GetSpellInfo()->Effects[m_effIndex].TriggerSpell;
    if (sSpellMgr->GetSpellInfo(triggerSpellId))
    {
        int32 basepoints0 = GetAmount();
