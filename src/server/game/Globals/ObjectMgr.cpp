...
void ObjectMgr::LoadQuestObjectivesLocale()
{
    ...
    //                                                0             1          2
    QueryResult result = WorldDatabase.Query("SELECT objectiveId, locale, Description FROM quest_objectives_locale");
    if (!result)
        return;
...
void ObjectMgr::LoadQuestLocales()
{
    ...
    QueryResult result = WorldDatabase.Query("SELECT Id, locale, "
        ...
        "Title, Description, Objectives, OfferRewardText, RequestItemsText, CompletedText, EndText, PortraitGiverText, PortraitGiverName, PortraitTurnInText, PortraitTurnInName"
    ...
    do
    {
        ...
        std::string Objectives = fields[4].GetString();
        std::string OfferRewardText = fields[5].GetString();
        std::string RequestItemsText = fields[6].GetString();
        std::string CompletedText = fields[7].GetString();
        std::string EndText = fields[8].GetString();
        std::string portraitGiverText = fields[9].GetString();
        std::string portraitGiverName = fields[10].GetString();
        std::string portraitTurnInText = fields[12].GetString();
        std::string portraitTurnInName = fields[10].GetString();

        QuestLocale& data = _questLocaleStore[id];
        LocaleConstant locale = GetLocaleByName(localeName);

        AddLocaleString(Title, locale, data.Title);
        AddLocaleString(Description, locale, data.Details);
        AddLocaleString(Objectives, locale, data.Objectives);
        AddLocaleString(OfferRewardText, locale, data.OfferRewardText);
        AddLocaleString(RequestItemsText, locale, data.RequestItemsText);
        AddLocaleString(CompletedText, locale, data.CompletedText);
        AddLocaleString(EndText, locale, data.EndText);
        AddLocaleString(portraitGiverText, locale, data.QuestGiverTextWindow);
        AddLocaleString(portraitGiverName, locale, data.QuestGiverTargetName);
        AddLocaleString(portraitTurnInText, locale, data.QuestTurnTextWindow);
        AddLocaleString(portraitTurnInName, locale, data.QuestTurnTargetName);
