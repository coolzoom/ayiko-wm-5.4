                ...
                if (!UpdateVictim())
                {
                    if (WarpGuid && check_Timer <= diff)
                    {
                        if (Unit* Warp = Unit::GetUnit(*me, WarpGuid))
                        {
                            ...
                            {
                                Warp->CastCustomSpell(SPELL_HEAL_FATHER, SPELLVALUE_BASE_POINT0, (int32)me->GetHealth(), Warp, true, 0, 0, me->GetGUID());
...
class boss_warp_splinter : public CreatureScript
{
    public:
        ...
        {
            boss_warp_splinterAI(Creature* creature) : BossAI(creature, 5)
            {
