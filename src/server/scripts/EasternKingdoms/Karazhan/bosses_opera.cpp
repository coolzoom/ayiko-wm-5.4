...
void boss_julianne::boss_julianneAI::DamageTaken(Unit* /*done_by*/, uint32 &damage)
{
    ...
    if (Phase == PHASE_ROMULO)
    {
        sLog->outError("scripts", "boss_julianneAI: cannot take damage in PHASE_ROMULO, why was i here?");
        damage = 0;
        return;
    }
    ...
    sLog->outError("scripts", "boss_julianneAI: DamageTaken reach end of code, that should not happen.");
}
