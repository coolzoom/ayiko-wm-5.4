...
struct SpellProcEventEntry
{
    ...
    Trinity::Flag128 spellFamilyMask;                       // if nonzero - for matching proc condition based on candidate spell's SpellFamilyFlags  (like auras 107 and 108 do)
...
struct SpellProcEntry
{
    ...
    Trinity::Flag128 spellFamilyMask;                       // if nonzero - bitmask for matching proc condition based on candidate spell's SpellFamilyFlags
...
typedef std::unordered_map<uint32, SpellGroupStackRule> SpellGroupStackMap;
...
typedef std::unordered_set<PetAura const*> PetAuraSet;
typedef std::unordered_map<uint32, PetAura> SpellPetAuraMap;
...
typedef std::vector<std::vector<uint32>> SpellClassVector;
...
class SpellMgr
{
    ...
    public:
        ...
        bool IsSpellValid(SpellInfo const* spellInfo, Player* player = NULL, bool msg = true);
        ...
        SpellClassVector::value_type const & GetSpellClassList(uint8 classId) const { return mSpellClassInfo[classId]; }

        SpellPowerVector::value_type const & GetSpellPowerList(uint32 spellId) const { return mSpellPowerInfo[spellId]; }

        bool IsTalent(uint32 spellId) const { return mTalentSpellInfo.find(spellId) != mTalentSpellInfo.end(); }
        ...
        SpellsByCategoryRange getSpellsWithCategory(uint32 categoryId) const
        {
