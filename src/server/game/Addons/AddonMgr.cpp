...
void LoadFromDB()
{
    ...
    if (!result)
    {
        sLog->outInfo("server.loading", ">> Loaded 0 known addons. DB table `addons` is empty!");
    ...
    sLog->outInfo("server.loading", ">> Loaded %u known addons in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
