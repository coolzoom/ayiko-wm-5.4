...
void LoadSkillDiscoveryTable()
{
    ...
    if (!result)
    {
        sLog->outError("server.loading", ">> Loaded 0 skill discovery definitions. DB table `skill_discovery_template` is empty.");
    ...
    do
    {
        ...
        if (reqSkillOrSpell > 0)                            // spell case
        {
            ...
            if (!reqSpellInfo)
            {
                ...
                {
                    sLog->outError("sql.sql", "Spell (ID: %u) have not existed spell (ID: %i) in `reqSpell` field in `skill_discovery_template` table", spellId, reqSkillOrSpell);
                ...
                !reqSpellInfo->IsExplicitDiscovery())
            {
                if (reportedReqSpells.find(absReqSkillOrSpell) == reportedReqSpells.end())
                {
                    sLog->outError("sql.sql", "Spell (ID: %u) not have MECHANIC_DISCOVERY (28) value in Mechanic field in spell.dbc"
        ...
        else if (reqSkillOrSpell == 0)                      // skill case
        {
            ...
            {
                sLog->outError("sql.sql", "Spell (ID: %u) not listed in `SkillLineAbility.dbc` but listed with `reqSpell`=0 in `skill_discovery_template` table", spellId);
                continue;
            }
        ...
        {
            sLog->outError("sql.sql", "Spell (ID: %u) have negative value in `reqSpell` field in `skill_discovery_template` table", spellId);
            continue;
        }
    ...
    if (!ssNonDiscoverableEntries.str().empty())
        sLog->outError("sql.sql", "Some items can't be successfully discovered: have in chance field value < 0.000001 in `skill_discovery_template` DB table . List:\n%s", ssNonDiscoverableEntries.str().c_str());
    ...
    for (uint32 spell_id = 1; spell_id < sSpellMgr->GetSpellInfoStoreSize(); ++spell_id)
    {
        ...
        if (SkillDiscoveryStore.find(int32(spell_id)) == SkillDiscoveryStore.end())
            sLog->outError("sql.sql", "Spell (ID: %u) is 100%% chance random discovery ability but not have data in `skill_discovery_template` table", spell_id);
    }

    sLog->outInfo("server.loading", ">> Loaded %u skill discovery definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
