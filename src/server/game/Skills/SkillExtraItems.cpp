...
typedef std::unordered_map<uint32, SkillExtraItemEntry> SkillExtraItemMap;
...
void LoadSkillExtraItemTable()
{
    ...
    if (!result)
    {
        sLog->outError("server.loading", ">> Loaded 0 spell specialization definitions. DB table `skill_extra_item_template` is empty.");
    ...
    do
    {
        ...
        {
            sLog->outError("misc", "Skill specialization %u has non-existent spell id in `skill_extra_item_template`!", spellId);
            continue;
        }
        ...
        {
            sLog->outError("misc", "Skill specialization %u have not existed required specialization spell id %u in `skill_extra_item_template`!", spellId, requiredSpecialization);
            continue;
        }
        ...
        {
            sLog->outError("misc", "Skill specialization %u has too low additional create chance in `skill_extra_item_template`!", spellId);
            continue;
        }
        ...
        if (!additionalMaxNum)
        {
            sLog->outError("misc", "Skill specialization %u has 0 max number of extra items in `skill_extra_item_template`!", spellId);
            continue;
        }
    ...
    sLog->outInfo("server.loading", ">> Loaded %u spell specialization definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
