...
uint32 GroupMgr::GenerateNewGroupDbStoreId()
{
    ...
    {
        sLog->outError("misc", "Group storage ID overflow!! Can't continue, shutting down server. ");
...
uint32 GroupMgr::GenerateGroupId()
{
    ...
    {
        sLog->outError("misc", "Group guid overflow!! Can't continue, shutting down server. ");
        World::StopNow(ERROR_EXIT_CODE);
    }
    return NextGroupId++;
}
...
{
    {
        ...
        if (!result)
        {
            sLog->outInfo("server.loading", ">> Loaded 0 group definitions. DB table `groups` is empty!");
            return;
        }
        ...
        sLog->outInfo("server.loading", ">> Loaded %u group definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }

    sLog->outInfo("server.loading", "Loading Group members...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            sLog->outInfo("server.loading", ">> Loaded 0 group members. DB table `group_member` is empty!");
            return;
        }
        ...
        do
        {
            ...
            if (group)
                ...
                sLog->outError("misc", "GroupMgr::LoadGroups: Consistency failed, can't find group (storage id: %u)", fields[0].GetUInt32());

            ++count;
        }
        while (result->NextRow());

        sLog->outInfo("server.loading", ">> Loaded %u group members in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }

    sLog->outInfo("server.loading", "Loading Group instance saves...");
    {
        uint32 oldMSTime = getMSTime();
        //                                                   0           1        2              3             4             5            6
        ...
        if (!result)
        {
            sLog->outInfo("server.loading", ">> Loaded 0 group-instance saves. DB table `group_instance` is empty!");
            return;
        }
        ...
        do
        {
            ...
            {
                sLog->outError("sql.sql", "Incorrect entry in group_instance table : no dungeon map %d", fields[1].GetUInt16());
                continue;
            }
            ...
            {
                sLog->outError("sql.sql", "Wrong dungeon difficulty use in group_instance table: %d", diff + 1);
        ...
        sLog->outInfo("server.loading", ">> Loaded %u group-instance saves in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }
}
