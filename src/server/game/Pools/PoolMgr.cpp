...
void PoolGroup<Creature>::Spawn1Object(PoolObject* obj)
{
    if (CreatureData const* data = sObjectMgr->GetCreatureData(obj->guid))
    {
        ...
        if (!map->Instanceable() && map->IsGridLoaded(data->posX, data->posY))
        {
            ...
            //sLog->outDebug("pool", "Spawning creature %u", guid);
...
void PoolGroup<GameObject>::Spawn1Object(PoolObject* obj)
{
    if (GameObjectData const* data = sObjectMgr->GetGOData(obj->guid))
    {
        ...
        if (!map->Instanceable() && map->IsGridLoaded(data->posX, data->posY))
        {
            ...
            //sLog->outDebug("pool", "Spawning gameobject %u", guid);
...
void PoolGroup<Quest>::Spawn1Object(PoolObject* obj)
{
    ...
    for (PooledQuestRelation::iterator itr = qr.first; itr != qr.second; ++itr)
    {
        sLog->outDebug("pool", "PoolGroup<Quest>: Adding quest %u to creature %u", itr->first, itr->second);
    ...
    for (PooledQuestRelation::iterator itr = qr.first; itr != qr.second; ++itr)
    {
        sLog->outDebug("pool", "PoolGroup<Quest>: Adding quest %u to GO %u", itr->first, itr->second);
        questMap->insert(QuestRelations::value_type(itr->second, itr->first));
    }
}
...
{
    sLog->outDebug("pool", "PoolGroup<Quest>: Spawning pool %u", poolId);
    // load state from db
    if (!triggerFrom)
    {
    ...
    if (limit > newQuests.size() && !currentQuests.empty())
    {
        ...
        while
            (newQuests.size() < limit && !currentQuests.empty()); // failsafe
    }
...
void PoolMgr::LoadFromDB()
{
    // Pool templates
    {
        ...
        {
            mPoolTemplate.clear();
            sLog->outInfo("server.loading", ">> Loaded 0 object pools. DB table `pool_template` is empty.");
            return;
        }
        ...
        sLog->outInfo("server.loading", ">> Loaded %u objects pools in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }

    // Creatures

    sLog->outInfo("server.loading", "Loading Creatures Pooling Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            sLog->outInfo("server.loading", ">> Loaded 0 creatures in  pools. DB table `pool_creature` is empty.");
        }
        else
        {
            uint32 count = 0;
            do
            {
                ...
                if (!data)
                {
                    sLog->outError("sql.sql", "`pool_creature` has a non existing creature spawn (GUID: %u) defined for pool id (%u), skipped.", guid, pool_id);
                    continue;
                }
                if (pool_id > max_pool_id)
                {
                    sLog->outError("sql.sql", "`pool_creature` pool id (%u) is out of range compared to max pool id in `pool_template`, skipped.", pool_id);
                    continue;
                }
                if (chance < 0 || chance > 100)
                {
                    sLog->outError("sql.sql", "`pool_creature` has an invalid chance (%f) for creature guid (%u) in pool id (%u), skipped.", chance, guid, pool_id);
                    continue;
                }
                PoolTemplateData* pPoolTemplate = &mPoolTemplate[pool_id];
            ...
            sLog->outInfo("server.loading", ">> Loaded %u creatures in pools in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // Gameobjects

    sLog->outInfo("server.loading", "Loading Gameobject Pooling Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            sLog->outInfo("server.loading", ">> Loaded 0 gameobjects in  pools. DB table `pool_gameobject` is empty.");
        }
        else
        {
            uint32 count = 0;
            do
            {
                ...
                if (!data)
                {
                    sLog->outError("sql.sql", "`pool_gameobject` has a non existing gameobject spawn (GUID: %u) defined for pool id (%u), skipped.", guid, pool_id);
                    continue;
                }
                ...
                {
                    sLog->outError("sql.sql", "`pool_gameobject` has a not lootable gameobject spawn (GUID: %u, type: %u) defined for pool id (%u), skipped.", guid, goinfo->type, pool_id);
                    continue;
                }

                if (pool_id > max_pool_id)
                {
                    sLog->outError("sql.sql", "`pool_gameobject` pool id (%u) is out of range compared to max pool id in `pool_template`, skipped.", pool_id);
                    continue;
                }

                if (chance < 0 || chance > 100)
                {
                    sLog->outError("sql.sql", "`pool_gameobject` has an invalid chance (%f) for gameobject guid (%u) in pool id (%u), skipped.", chance, guid, pool_id);
                    continue;
                }
            ...
            sLog->outInfo("server.loading", ">> Loaded %u gameobject in pools in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // Pool of pools

    sLog->outInfo("server.loading", "Loading Mother Pooling Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            sLog->outInfo("server.loading", ">> Loaded 0 pools in pools");
        }
        else
        {
            uint32 count = 0;
            do
            {
                ...
                {
                    sLog->outError("sql.sql", "`pool_pool` mother_pool id (%u) is out of range compared to max pool id in `pool_template`, skipped.", mother_pool_id);
                    continue;
                }
                if (child_pool_id > max_pool_id)
                {
                    sLog->outError("sql.sql", "`pool_pool` included pool_id (%u) is out of range compared to max pool id in `pool_template`, skipped.", child_pool_id);
                    continue;
                }
                if (mother_pool_id == child_pool_id)
                {
                    sLog->outError("sql.sql", "`pool_pool` pool_id (%u) includes itself, dead-lock detected, skipped.", child_pool_id);
                    continue;
                }
                if (chance < 0 || chance > 100)
                {
                    sLog->outError("sql.sql", "`pool_pool` has an invalid chance (%f) for pool id (%u) in mother pool id (%u), skipped.", chance, child_pool_id, mother_pool_id);
                    continue;
                }
            ...
            for (uint32 i=0; i < max_pool_id; ++i)
            {
                ...
                for (SearchMap::iterator poolItr = mPoolSearchMap.find(i); poolItr != mPoolSearchMap.end(); poolItr = mPoolSearchMap.find(poolItr->second))
                {
                    ...
                    if (checkedPools.find(poolItr->second) != checkedPools.end())
                    {
                        ...
                        sLog->outError("sql.sql", "%s", ss.str().c_str());
            ...
            sLog->outInfo("server.loading", ">> Loaded %u pools in mother pools in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    sLog->outInfo("server.loading", "Loading Quest Pooling Data...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            sLog->outInfo("server.loading", ">> Loaded 0 quests in pools");
        }
        else
        {
            ...
            do
            {
                ...
                if (!quest)
                {
                    sLog->outError("sql.sql", "`pool_quest` has a non existing quest template (Entry: %u) defined for pool id (%u), skipped.", entry, pool_id);
                    continue;
                }

                if (pool_id > max_pool_id)
                {
                    sLog->outError("sql.sql", "`pool_quest` pool id (%u) is out of range compared to max pool id in `pool_template`, skipped.", pool_id);
                    continue;
                }

                if (!quest->IsDailyOrWeekly())
                {
                    sLog->outError("sql.sql", "`pool_quest` has an quest (%u) which is not daily or weekly in pool id (%u), use ExclusiveGroup instead, skipped.", entry, pool_id);
                    continue;
                }
                ...
                {
                    sLog->outError("sql.sql", "`pool_quest` quest %u is %s but pool (%u) is specified for %s, mixing not allowed, skipped.",
                ...
                {
                    sLog->outError("sql.sql", "`pool_quest` lists entry (%u) as member of pool (%u) but is not started anywhere, skipped.", entry, pool_id);
                    continue;
                }
            ...
            sLog->outInfo("server.loading", ">> Loaded %u quests in pools in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }

    // The initialize method will spawn all pools not in an event and not in another pool, this is why there is 2 left joins with 2 null checks
    sLog->outInfo("server.loading", "Starting objects pooling system...");
    {
        uint32 oldMSTime = getMSTime();
        ...
        if (!result)
        {
            sLog->outInfo("server.loading", ">> Pool handling system initialized, 0 pools spawned.");
        }
        else
        {
            uint32 count = 0;
            do
            {
                ...
                if (!CheckPool(pool_entry))
                {
                    if (pool_pool_id)
                        ...
                        sLog->outError("sql.sql", "Pool Id %u has no equal chance pooled entites defined and explicit chance sum is not 100. This broken pool is a child pool of Id %u and cannot be safely removed.", pool_entry, fields[2].GetUInt32());
                    else
                        sLog->outError("sql.sql", "Pool Id %u has no equal chance pooled entites defined and explicit chance sum is not 100. The pool will not be spawned.", pool_entry);
                    continue;
                }
            ...
            sLog->outDebug("pool", "Pool handling system initialized, %u pools spawned in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
