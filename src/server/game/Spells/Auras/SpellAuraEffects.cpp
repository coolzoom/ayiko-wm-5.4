                ...
                case SPELLFAMILY_GENERIC:
                    switch (GetId())
                    {
                        ...
                        case 81262:
                        {
                            ...
                            targets.SetDst(caster->GetPositionX(), caster->GetPositionY(), caster->GetPositionZ(), caster->GetOrientation());
                            int32 bp = GetBase()->GetEffect(EFFECT_0)->GetAmount();
                            values.AddSpellMod(SPELLVALUE_BASE_POINT0, bp);
                            GetCaster()->CastSpell(targets, sSpellMgr->GetSpellInfo(81269), &values, TRIGGERED_FULL_MASK);
                            break;
                        }
                        default:
                            break;
...
void AuraEffect::HandleModStealth(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        //if (target->GetTypeId() == TYPEID_PLAYER)
            //target->SetByteFlag(PLAYER_BYTES2, 3, PLAYER_FIELD_BYTE2_STEALTH); // TODO : Check PLAYER_FIELD_AURA_VISION
...
void AuraEffect::HandlePeriodicTriggerSpellAuraTick(Unit* target, Unit* caster) const
{
    ...
    else
    {
        ...
        if (!c || !caster || !sScriptMgr->OnDummyEffect(caster, GetId(), SpellEffIndex(GetEffIndex()), target->ToCreature()) ||
            ...
            sLog->outDebug("spells", "AuraEffect::HandlePeriodicTriggerSpellAuraTick: Spell %u has non-existent spell %u in EffectTriggered[%d] and is therefor not triggered.", GetId(), triggerSpellId, GetEffIndex());
    }
}
...
void AuraEffect::HandleRaidProcFromChargeAuraProc(AuraApplication* aurApp, ProcEventInfo& /*eventInfo*/)
{
    ...
    switch (GetId())
    {
        ...
        case 43593:            // Cold Stare
            ...
            break;
        default:
            sLog->outDebug("spells", "AuraEffect::HandleRaidProcFromChargeAuraProc: received not handled spell: %u", GetId());
            return;
    }
...
void AuraEffect::HandleRaidProcFromChargeWithValueAuraProc(AuraApplication* aurApp, ProcEventInfo& /*eventInfo*/)
{
    ...
    {
        sLog->outDebug("spells", "AuraEffect::HandleRaidProcFromChargeWithValueAuraProc: received not handled spell: %u", GetId());
        return;
    }
