...
class ScriptCallGuard final
{
public:
    template <typename HookType>
    ScriptCallGuard(ScriptType *script, HookType hook)
        : script_(script)
    {
        script_->_PrepareScriptCall(hook);
    }

    template <typename HookType, typename P1>
    ScriptCallGuard(ScriptType *script, HookType hook, P1 &&param1)
        : script_(script)
    {
        script_->_PrepareScriptCall(hook, param1);
    }

    ScriptCallGuard(ScriptCallGuard &&other)
        : script_(other.script_)
    {
        other.script_ = nullptr;
    }

    ScriptCallGuard & operator=(ScriptCallGuard &&other)
    {
        script_ = other.script_;
        other.script_ = nullptr;
...
template <typename ScriptType, typename HookType>
inline ScriptCallGuard<ScriptType>
makeScriptCallGuard(ScriptType *script, HookType hook)
{
    return ScriptCallGuard<ScriptType>(script, hook);
}

template <typename ScriptType, typename HookType, typename P1>
inline ScriptCallGuard<ScriptType>
makeScriptCallGuard(ScriptType *script, HookType hook, P1 &&param1)
{
    return ScriptCallGuard<ScriptType>(script, hook, param1);
}
