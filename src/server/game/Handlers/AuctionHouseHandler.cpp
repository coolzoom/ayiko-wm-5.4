...
void WorldSession::HandleAuctionHelloOpcode(WorldPacket& recvData)
{
    ...
    if (!unit)
    {
        sLog->outDebug("network", "WORLD: HandleAuctionHelloOpcode - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
...
void WorldSession::HandleAuctionSellItem(WorldPacket & recvData)
{
    ...
    if (!creature)
    {
        sLog->outDebug("network", "WORLD: HandleAuctionSellItem - Unit (GUID: %u) not found or you can't interact with him.", GUID_LOPART(auctioneer));
        return;
    }
    ...
    {
        sLog->outDebug("network", "WORLD: HandleAuctionSellItem - Unit (GUID: %u) has wrong faction.", GUID_LOPART(auctioneer));
        return;
    }
    ...
    for (uint32 i = 0; i < itemsCount; ++i)
    {
        ...
        if (itemsCount == 1 && item->GetCount() == count[i])
        {
            ...
            sLog->outInfo("network", "CMSG_AUCTION_SELL_ITEM: Player %s (guid %d) is selling item %s entry"
                          " %u (guid %d) to auctioneer %u with count %u with initial bid " UI64FMTD " with buyout " UI64FMTD
                          " and with time %u (in sec) in auctionhouse %u",
                          _player->GetName(), _player->GetGUIDLow(), item->GetTemplate()->Name1.c_str(), item->GetEntry(),
                          item->GetGUIDLow(), AH->auctioneer, item->GetCount(), bid, buyout, auctionTime, AH->GetHouseId());
        ...
        else // Required stack size of auction does not match to current item stack size, clone item and set correct stack size
        {
            ...
            if (!newItem)
            {
                sLog->outError("network", "CMSG_AUCTION_SELL_ITEM: Could not create clone of item %u", item->GetEntry());
                SendAuctionCommandResult(NULL, AUCTION_SELL_ITEM, ERR_AUCTION_DATABASE_ERROR);
                return;
            }
            ...
            sLog->outInfo("network", "CMSG_AUCTION_SELL_ITEM: Player %s (guid %d) is selling item %s entry %u (guid %d) to auctioneer"
                          " %u with count %u with initial bid " UI64FMTD " with buyout " UI64FMTD " and with time %u (in sec) in auctionhouse %u",
...
{
    sLog->outDebug("network", "WORLD: Received CMSG_AUCTION_PLACE_BID");
    ...
    if (!creature)
    {
        sLog->outDebug("network", "WORLD: HandleAuctionPlaceBid - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(auctioneer)));
        return;
    }
...
{
    sLog->outDebug("network", "WORLD: Received CMSG_AUCTION_REMOVE_ITEM");
    ...
    if (!creature)
    {
        sLog->outDebug("network", "WORLD: HandleAuctionRemoveItem - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(auctioneer)));
        return;
    }
    ...
    if (auction && auction->owner == player->GetGUIDLow())
    {
        ...
        {
            sLog->outError("network", "Auction id: %u got non existing item (item guid : %u)!", auction->Id, auction->itemGUIDLow);
    ...
    else
    {
        ...
        sLog->outError("network", "CHEATER: %u tried to cancel auction (id: %u) of another player or auction is NULL", player->GetGUIDLow(), auctionId);
        return;
    }
...
{
    sLog->outDebug("network", "WORLD: Received CMSG_AUCTION_LIST_BIDDER_ITEMS");
    ...
    if (!creature)
    {
        sLog->outDebug("network", "WORLD: HandleAuctionListBidderItems - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
...
{
    sLog->outDebug("network", "WORLD: Received CMSG_AUCTION_LIST_OWNER_ITEMS");
    ...
    if (!creature)
    {
        sLog->outDebug("network", "WORLD: HandleAuctionListOwnerItems - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
...
{
    sLog->outDebug("network", "WORLD: Received CMSG_AUCTION_LIST_ITEMS");
    ...
    if (!creature)
    {
        sLog->outDebug("network", "WORLD: HandleAuctionListItems - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
    ...
    //sLog->outDebug("misc", "Auctionhouse search (GUID: %u TypeId: %u)",, list from: %u, searchedname: %s, levelmin: %u, levelmax: %u, auctionSlotID: %u, auctionMainCategory: %u, auctionSubCategory: %u, quality: %u, usable: %u",
...
{
    sLog->outDebug("network", "WORLD: Received CMSG_AUCTION_LIST_PENDING_SALES");
