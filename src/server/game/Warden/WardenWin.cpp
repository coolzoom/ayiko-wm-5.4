...
void WardenWin::Init(WorldSession* session, BigNumber *k)
{
    ...
    sLog->outDebug("warden", "Server side warden for client %u initializing...", session->GetAccountId());
    sLog->outDebug("warden", "C->S Key: %s", ByteArrayToHexStr(_inputKey, 16).c_str());
    sLog->outDebug("warden", "S->C Key: %s", ByteArrayToHexStr(_outputKey, 16).c_str());
    sLog->outDebug("warden", "  Seed: %s", ByteArrayToHexStr(_seed, 16).c_str());
    sLog->outDebug("warden", "Loading Module...");

    _module = GetModuleForClient();

    sLog->outDebug("warden", "Module Key: %s", ByteArrayToHexStr(_module->Key, 16).c_str());
    sLog->outDebug("warden", "Module ID: %s", ByteArrayToHexStr(_module->Id, 16).c_str());
    RequestModule();
}
...
void WardenWin::HandleHashResult(ByteBuffer &buff)
{
    ...
    {
        sLog->outWarn("warden", "%s failed hash reply. Action: %s", _session->GetPlayerName(false).c_str(), Penalty().c_str());
        return;
    }
...
void WardenWin::RequestData()
{
    ...
    sLog->outDebug("warden", "%s", stream.str().c_str());
}
...
void WardenWin::HandleData(ByteBuffer &buff)
{
    ...
    {
        buff.rpos(buff.wpos());
        sLog->outWarn("warden", "%s failed checksum. Action: %s", _session->GetPlayerName(false).c_str(), Penalty().c_str());
        return;
    }
    ...
    // TIMING_CHECK
    {
        ...
        if (result == 0x00)
        {
            sLog->outWarn("warden", "%s failed timing check. Action: %s", _session->GetPlayerName(false).c_str(), Penalty().c_str());
            return;
        }
        ...
        sLog->outDebug("warden", "RequestTicks %u", _serverTicks);    // At request
    ...
    for (std::list<uint16>::iterator itr = _currentChecks.begin(); itr != _currentChecks.end(); ++itr)
    {
        ...
        switch (type)
        {
            case MEM_CHECK:
            {
                ...
                if (Mem_Result != 0)
                {
                    sLog->outDebug("warden", "RESULT MEM_CHECK not 0x00, CheckId %u account Id %u", *itr, _session->GetAccountId());
                    checkFailed = *itr;
                    continue;
                }
                ...
                {
                    sLog->outDebug("warden", "RESULT MEM_CHECK fail CheckId %u account Id %u", *itr, _session->GetAccountId());
                    checkFailed = *itr;
                ...
                buff.rpos(buff.rpos() + rd->Length);
                sLog->outDebug("warden", "RESULT MEM_CHECK passed CheckId %u account Id %u", *itr, _session->GetAccountId());
                break;
            }
            case PAGE_CHECK_A:
            case PAGE_CHECK_B:
            ...
            case MODULE_CHECK:
            {
                ...
                if (memcmp(buff.contents() + buff.rpos(), &byte, sizeof(uint8)) != 0)
                {
                    if (type == PAGE_CHECK_A || type == PAGE_CHECK_B)
                        sLog->outDebug("warden", "RESULT PAGE_CHECK fail, CheckId %u account Id %u", *itr, _session->GetAccountId());
                    if (type == MODULE_CHECK)
                        sLog->outDebug("warden", "RESULT MODULE_CHECK fail, CheckId %u account Id %u", *itr, _session->GetAccountId());
                    if (type == DRIVER_CHECK)
                        sLog->outDebug("warden", "RESULT DRIVER_CHECK fail, CheckId %u account Id %u", *itr, _session->GetAccountId());
                    checkFailed = *itr;
                ...
                buff.rpos(buff.rpos() + 1);
                if (type == PAGE_CHECK_A || type == PAGE_CHECK_B)
                    sLog->outDebug("warden", "RESULT PAGE_CHECK passed CheckId %u account Id %u", *itr, _session->GetAccountId());
                else if (type == MODULE_CHECK)
                    sLog->outDebug("warden", "RESULT MODULE_CHECK passed CheckId %u account Id %u", *itr, _session->GetAccountId());
                else if (type == DRIVER_CHECK)
                    sLog->outDebug("warden", "RESULT DRIVER_CHECK passed CheckId %u account Id %u", *itr, _session->GetAccountId());
                break;
            }
            case LUA_STR_CHECK:
            {
                ...
                if (Lua_Result != 0)
                {
                    sLog->outDebug("warden", "RESULT LUA_STR_CHECK fail, CheckId %u account Id %u", *itr, _session->GetAccountId());
                    checkFailed = *itr;
                    continue;
                }
                ...
                sLog->outDebug("warden", "RESULT LUA_STR_CHECK passed, CheckId %u account Id %u", *itr, _session->GetAccountId());
                break;
            }
            case MPQ_CHECK:
            {
                ...
                if (Mpq_Result != 0)
                {
                    sLog->outDebug("warden", "RESULT MPQ_CHECK not 0x00 account id %u", _session->GetAccountId());
                    checkFailed = *itr;
                    continue;
                }
                ...
                {
                    sLog->outDebug("warden", "RESULT MPQ_CHECK fail, CheckId %u account Id %u", *itr, _session->GetAccountId());
                    checkFailed = *itr;
                ...
                buff.rpos(buff.rpos() + 20);                // 20 bytes SHA1
                sLog->outDebug("warden", "RESULT MPQ_CHECK passed, CheckId %u account Id %u", *itr, _session->GetAccountId());
                break;
            }
    ...
    if (checkFailed > 0)
    {
        ...
        sLog->outWarn("warden", "%s failed Warden check %u. Action: %s", _session->GetPlayerName(false).c_str(), checkFailed, Penalty(check).c_str());
    }
