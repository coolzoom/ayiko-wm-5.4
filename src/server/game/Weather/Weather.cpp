    ...
    : m_zone(zone), m_weatherChances(weatherChances)
{
    ...
    sLog->outInfo("misc", "WORLD: Starting weather system for zone %u (change every %u minutes).", m_zone, (uint32)(m_timer.GetInterval() / (MINUTE*IN_MILLISECONDS)));
}
...
bool Weather::ReGenerate()
{
    ...
    sLog->outInfo("misc", "Generating a change in %s weather for zone %u.", seasonName[season], m_zone);
...
bool Weather::UpdateWeather()
{
    ...
    sLog->outInfo("misc", "Change the weather of zone %u to %s.", m_zone, wthstr);
