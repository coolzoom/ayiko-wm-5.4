    ...
    switch (m_deathState)
    {
        ...
        case ALIVE:
        {
            ...
            if (isControlled())
            {
                ...
                {
                    sLog->outError(LOG_FILTER_PETS, "Pet %u is not pet of owner %s, removed", GetEntry(), GetOwner()->GetName());
                    Remove(PET_REMOVE_DISMISS, PET_REMOVE_FLAG_RESET_CURRENT);
                    return;
                }
            }
...
bool Guardian::InitStatsForLevel(uint8 petlevel)
{
    ...
    if (isPet() && GetOwner()->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        else if (GetOwner()->getClass() == CLASS_HUNTER)
        {
            ...
            sLog->outError(LOG_FILTER_PETS, "Unknown type pet %u is summoned by player class %u", GetEntry(), GetOwner()->getClass());
    }
