...
{
    sLog->outDebug("bg.battleground", "bg_av HandleKillUnit %i", unit->GetEntry());
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;
    ...
    */
    ...
    else if (entry == BG_AV_CreatureInfo[AV_NPC_A_CAPTAIN][0])
    {
        if (!m_CaptainAlive[0])
        {
            sLog->outError("bg.battleground", "Killed a Captain twice, please report this bug, if you haven't done \".respawn\"");
            return;
        }
...
void BattlegroundAV::HandleQuestComplete(uint32 questid, Player* player)
{
    ...
    sLog->outDebug("bg.battleground", "BG_AV Quest %i completed", questid);
    switch (questid)
    {
        ...
        case AV_QUEST_H_SCRAPS2:
            ...
            {
                sLog->outDebug("bg.battleground", "BG_AV Quest %i completed starting with unit upgrading..", questid);
        ...
        case AV_QUEST_H_COMMANDER1:
            ...
            if (m_Team_QuestStatus[team][1] == 30)
                sLog->outDebug("bg.battleground", "BG_AV Quest %i completed (need to implement some events here", questid);
            break;
        ...
        case AV_QUEST_H_NEAR_MINE:
            ...
            if (m_Team_QuestStatus[team][5] == 28)
            {
                ...
                if (m_Team_QuestStatus[team][6] == 7)
                    sLog->outDebug("bg.battleground", "BG_AV Quest %i completed (need to implement some events here - ground assault ready", questid);
            }
            break;
        ...
        case AV_QUEST_H_RIDER_HIDE:
            ...
            if (m_Team_QuestStatus[team][7] == 25)
            {
                ...
                if (m_Team_QuestStatus[team][8] == 25)
                    sLog->outDebug("bg.battleground", "BG_AV Quest %i completed (need to implement some events here - rider assault ready", questid);
            }
            break;
        ...
        case AV_QUEST_H_RIDER_TAME:
            ...
            break;
        default:
            sLog->outDebug("bg.battleground", "BG_AV Quest %i completed but is not interesting at all", questid);
...
{
    sLog->outDebug("bg.battleground", "BG_AV: start spawning mine stuff");
...
void BattlegroundAV::RemovePlayer(Player* player, uint64 /*guid*/, uint32 /*team*/)
{
   if (!player)
    {
        sLog->outError("bg.battleground", "bg_AV no player at remove");
        return;
    }
...
void BattlegroundAV::HandleAreaTrigger(Player* Source, uint32 Trigger)
{
    ...
    switch (Trigger)
    {
        ...
        case 3331:
            ...
            break;
        default:
            sLog->outDebug("bg.battleground", "WARNING: Unhandled AreaTrigger in Battleground: %u", Trigger);
...
void BattlegroundAV::EventPlayerDestroyedPoint(BG_AV_Nodes node)
{
    ...
    sLog->outDebug("bg.battleground", "bg_av: player destroyed point node %i object %i", node, object);
    ...
    if (IsTower(node))
    {
        ...
        if (BgCreatures[AV_CPLACE_A_MARSHAL_SOUTH + tmp])
            ...
            sLog->outError("bg.battleground", "BG_AV: playerdestroyedpoint: marshal %i doesn't exist", AV_CPLACE_A_MARSHAL_SOUTH + tmp);
        //spawn destroyed aura
        for (uint8 i=0; i <= 9; i++)
...
//changing the owner results in setting respawntim to infinite for current creatures, spawning new mine owners creatures and changing the chest-objects so that the current owning team can use them
    ...
    if (!initial)
    {
        sLog->outDebug("bg.battleground", "bg_av depopulating mine %i (0=north, 1=south)", mine);
    ...
    sLog->outDebug("bg.battleground", "bg_av populating mine %i (0=north, 1=south)", mine);
    uint16 miner;
    ...
    else
    {
       ...
       //vermin
        sLog->outDebug("bg.battleground", "spawning vermin");
        if (team == ALLIANCE)
...
void BattlegroundAV::PopulateNode(BG_AV_Nodes node)
{
    ...
    if (m_Team_QuestStatus[team2][0] < 500)
        ...
        if (!AddSpiritGuide(node, BG_AV_CreaturePos[node][0], BG_AV_CreaturePos[node][1], BG_AV_CreaturePos[node][2], BG_AV_CreaturePos[node][3], owner))
            sLog->outError("bg.battleground", "AV: couldn't spawn spiritguide at node %i", node);
...
{
    sLog->outDebug("bg.battleground", "bg_AV getnodethroughobject %i", object);
    ...
    sLog->outError("bg.battleground", "BattlegroundAV: ERROR! GetPlace got a wrong object :(");
    ASSERT(false);
...
{ //this function is the counterpart to GetNodeThroughObject()
    sLog->outDebug("bg.battleground", "bg_AV GetObjectThroughNode %i", node);
    ...
    sLog->outError("bg.battleground", "BattlegroundAV: Error! GetPlaceNode couldn't resolve node %i", node);
    ASSERT(false);
    return 0;
}
...
void BattlegroundAV::EventPlayerClickedOnFlag(Player* source, GameObject* target_obj)
{
    ...
    sLog->outDebug("bg.battleground", "BG_AV using gameobject %i with type %i", target_obj->GetEntry(), object);
    if (object < 0)
        return;
...
void BattlegroundAV::EventPlayerDefendsPoint(Player* player, uint32 object)
{
    ...
    sLog->outDebug("bg.battleground", "player defends point object: %i node: %i", object, node);
    if (m_Nodes[node].PrevOwner != team)
    {
        sLog->outError("bg.battleground", "BG_AV: player defends point which doesn't belong to his team %i", node);
        return;
    }
...
void BattlegroundAV::EventPlayerAssaultsPoint(Player* player, uint32 object)
{
    ...
    uint32 team  = player->GetTeam();
    sLog->outDebug("bg.battleground", "bg_av: player assaults point object %i node %i", object, node);
...
uint8 BattlegroundAV::GetWorldStateType(uint8 state, uint16 team) //this is used for node worldstates and returns values which fit good into the worldstatesarray
{
    ...
    sLog->outError("bg.battleground", "BG_AV: should update a strange worldstate state:%i team:%i", state, team);
...
bool BattlegroundAV::SetupBattleground()
{
    ...
    {
        sLog->outError("sql.sql", "BatteGroundAV: Failed to spawn some object Battleground not created!1");
        return false;
    }
    ...
    for (uint8 i = BG_AV_NODES_FIRSTAID_STATION; i < BG_AV_NODES_MAX; ++i)
    {
        if (i <= BG_AV_NODES_FROSTWOLF_HUT)
        {
            ...
            {
                sLog->outError("bg.battleground", "BatteGroundAV: Failed to spawn some object Battleground not created!2");
                return false;
            }
        }
        else //towers
        {
            if (i <= BG_AV_NODES_STONEHEART_BUNKER) //alliance towers
            {
                ...
                {
                    sLog->outError("bg.battleground", "BatteGroundAV: Failed to spawn some object Battleground not created!3");
                    return false;
                }
            }
            else //horde towers
            {
                ...
                {
                    sLog->outError("bg.battleground", "BatteGroundAV: Failed to spawn some object Battleground not created!4");
                    return false;
                }
            }
            for (uint8 j=0; j <= 9; j++) //burning aura
            {
                ...
                {
                    sLog->outError("bg.battleground", "BatteGroundAV: Failed to spawn some object Battleground not created!5.%i", i);
                    return false;
                }
            }
        }
    ...
    for (uint8 i=0; i<2; i++) //burning aura for buildings
    {
        for (uint8 j=0; j <= 9; j++)
        {
            if (j<5)
            {
                ...
                {
                    sLog->outError("bg.battleground", "BatteGroundAV: Failed to spawn some object Battleground not created!6.%i", i);
                    return false;
                }
            }
            else
            {
                ...
                {
                    sLog->outError("bg.battleground", "BatteGroundAV: Failed to spawn some object Battleground not created!7.%i", i);
                    return false;
                }
            }
        }
    ...
    for (uint16 i= 0; i <= (BG_AV_OBJECT_MINE_SUPPLY_N_MAX-BG_AV_OBJECT_MINE_SUPPLY_N_MIN); i++)
    {
        ...
        {
            sLog->outError("bg.battleground", "BatteGroundAV: Failed to spawn some mine supplies Battleground not created!7.5.%i", i);
            return false;
        }
    }
    for (uint16 i= 0; i <= (BG_AV_OBJECT_MINE_SUPPLY_S_MAX-BG_AV_OBJECT_MINE_SUPPLY_S_MIN); i++)
    {
        ...
        {
            sLog->outError("bg.battleground", "BatteGroundAV: Failed to spawn some mine supplies Battleground not created!7.6.%i", i);
            return false;
        }
    }
    ...
    {
        sLog->outError("bg.battleground", "BatteGroundAV: Failed to spawn some object Battleground not created!8");
        return false;
    }
    for (uint8 i = 0; i < 4; i++)
    {
        ...
        {
            sLog->outError("bg.battleground", "BatteGroundAV: Failed to spawn some object Battleground not created!9.%i", i);
            return false;
        }
    }

    uint16 i;
    sLog->outDebug("bg.battleground", "Alterac Valley: entering state STATUS_WAIT_JOIN ...");
    // Initial Nodes
    ...
    //creatures
    sLog->outDebug("bg.battleground", "BG_AV start populating nodes");
    ...
    sLog->outDebug("bg.battleground", "BG_AV: start spawning static creatures");
    for (i = 0; i < AV_STATICCPLACE_MAX; i++)
        AddAVCreature(0, i + AV_CPLACE_MAX);
    //mainspiritguides:
    sLog->outDebug("bg.battleground", "BG_AV: start spawning spiritguides creatures");
    ...
    sLog->outDebug("bg.battleground", "BG_AV: start spawning marshal creatures");
...
const char* BattlegroundAV::GetNodeName(BG_AV_Nodes node)
{
    switch (node)
    {
        ...
        default:
            sLog->outError("bg.battleground", "tried to get name for node %u", node);
            break;
    }
...
void BattlegroundAV::AssaultNode(BG_AV_Nodes node, uint16 team)
{
    ...
    {
        sLog->outFatal("bg.battleground", "Assaulting team is TotalOwner of node");
        ASSERT (false);
    }
    if (m_Nodes[node].Owner == team)
    {
        sLog->outFatal("bg.battleground", "Assaulting team is owner of node");
        ASSERT (false);
    }
    if (m_Nodes[node].State == POINT_DESTROYED)
    {
        sLog->outFatal("bg.battleground", "Destroyed node is being assaulted");
        ASSERT (false);
    }
    if (m_Nodes[node].State == POINT_ASSAULTED && m_Nodes[node].TotalOwner) //only assault an assaulted node if no totalowner exists
    {
        sLog->outFatal("bg.battleground", "Assault on an not assaulted node with total owner");
        ASSERT (false);
    }
