...
void BattlegroundDG::_ChangeBanner(uint8 node, uint8 type)
{
    ...
    if (Creature* captPoint = GetBgMap()->GetCreature(BgCreatures[BG_DG_OBJECT_CAPT_POINT_START + node]))
    {
        ...
        switch (m_Nodes[node])
        {
            case BG_DG_NODE_STATUS_ALLY_CONTESTED:  addAura = BG_DG_CAPT_POINT_ALLIANCE_CONTEST; captPoint->setFaction(1997); break;
...
void BattlegroundDG::_postUpdateImpl_Flags(uint32 diff)
{
    ...
    for (int team = 0; team < BG_TEAMS_COUNT; ++team)
    {
        ...
        if (m_lastTick[team] > BG_DG_TickIntervals[points])
        {
            ...
            {
                RewardHonorToTeam(GetBonusHonorFromKill(10), (team == TEAM_ALLIANCE) ? ALLIANCE : HORDE);
...
void BattlegroundDG::EventPlayerCapturedFlag(Player* player)
{
    ...
    RewardHonorToTeam(GetBonusHonorFromKill(20), player->GetBGTeam());
    ...
    UpdateCartState(player->GetBGTeam(), 1);                  // flag state none
    ...
    _flagsTimer[GetTeamIndexByTeamId(player->GetBGTeam()) ? 0 : 1] = BG_DG_CART_RESPAWN_TIME;
}
...
void BattlegroundDG::EventPlayerDroppedFlag(Player* player)
{
    ...
    if (set)
    {
        ...
        UpdateCartState(player->GetBGTeam(), 1);
...
bool BattlegroundDG::SetupBattleground()
{
    ...
    if (!sg || !AddSpiritGuide(BG_DG_SPIRIT_NORTHERN_ALLIANCE, sg->x, sg->y, sg->z, sg->facing + M_PI, TEAM_ALLIANCE))
    {
    ...
    if (!sg || !AddSpiritGuide(BG_DG_SPIRIT_SOUTHERN_ALLIANCE, sg->x, sg->y, sg->z, sg->facing + M_PI, TEAM_ALLIANCE))
    {
    ...
    if (!sg || !AddSpiritGuide(BG_DG_SPIRIT_NORTHERN_HORDE, sg->x, sg->y, sg->z, sg->facing + M_PI, TEAM_HORDE))
    {
    ...
    if (!sg || !AddSpiritGuide(BG_DG_SPIRIT_SOUTHERN_HORDE, sg->x, sg->y, sg->z, sg->facing + M_PI, TEAM_HORDE))
    {
