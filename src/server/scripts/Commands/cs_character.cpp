...
public:
    ...
    static bool HandleCharacterRenameCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (newNameStr)
        {
            ...
            if (WorldSession* session = handler->GetSession())
            {
                ...
                {
                    sLog->outCommand(session->GetAccountId(), "GM %s (Account: %u) forced rename %s to player %s (Account: %u)",
                                     player->GetName().c_str(), session->GetAccountId(), newName.c_str(), playerOldName.c_str(), sObjectMgr->GetPlayerAccountIdByGUID(targetGuid));
                }
            }
            else
    ...
    */
    ...
    static bool HandlePDumpLoadCommand(ChatHandler* handler, char const* args)
    {
        ...
        uint32 length = std::ftell(fin);
        std::rewind(fin);

        char *dump = new char[length];
        if (std::fread(dump, length, sizeof(*dump), fin) == 1)
        {
            switch (PlayerDumpReader::LoadDump(dump, accountId, name).first)
            {
                case DUMP_SUCCESS:
                ...
                case DUMP_TOO_MANY_CHARS:
                    handler->PSendSysMessage(LANG_ACCOUNT_CHARACTER_LIST_FULL,
                                             accountName.c_str(), accountId);
                    handler->SetSentErrorMessage(true);
                    break;
                default:
