                    ...
                    case EVENT_OUTRO_2:
                        {
                            if(instance)
                            {
                                ...
                                instance->SetBossState(DATA_SNOWDRIFT_STATE, DONE);
                            }

                            Talk(TALK_OUTRO_2);
                            Movement::MoveSplineInit init(me);
                            init.MoveTo(SnowdriftPos[1].GetPositionX(), SnowdriftPos[1].GetPositionY(), SnowdriftPos[1].GetPositionZ());
            ...
            if(uint32 eventId = events.ExecuteEvent())
            {
                switch(eventId)
                {
                    ...
                    case EVENT_FIST_OF_FURY:
                        Talk(TALK_FISTS);
                        me->CastSpell(me, SPELL_FIST_OF_FURY, false);
                        events.ScheduleEvent(EVENT_FIST_OF_FURY, 15 * IN_MILLISECONDS);
                        break;
                    ...
                    case EVENT_CHECK_DEST:
                        if(me->GetVictim() && me->GetVictim()->HasAuraTypeWithCaster(SPELL_AURA_FIXATE_TARGET, me->GetGUID()))
                        {
...
public:
    ...
    {
        npc_spm_pandaren_refereeAI(Creature* creature) : BossAI(creature, DATA_SNOWDRIFT_STATE) {}
                    ...
                    case EVENT_CHECK_WIPE:
                        if(instance)
                        {
                            if(instance->IsWipe())
                            {
                                ...
                                instance->SetBossState(DATA_SNOWDRIFT_STATE, FAIL);
                            }
                            else
...
public:
    ...
    struct npc_snowdrift_noviceAI : public ScriptedAI
    {
        ...
        void InitializeAI() override
        {
            ...
            me->CastSpell(x, y, me->GetMap()->GetHeight(me->GetPhaseMask(), x, y, MAX_HEIGHT), SPELL_JUMP, false);
...
class at_before_snowdrift : public AreaTriggerScript
{
public:
    at_before_snowdrift() : AreaTriggerScript("at_before_snowdrift") {}
    ...
    bool OnTrigger(Player* player, AreaTriggerEntry const* trigger)
    {
        ...
        {
            if(instance->GetBossState(DATA_SNOWDRIFT_STATE) != IN_PROGRESS && instance->GetBossState(DATA_SNOWDRIFT_STATE) != DONE)
            {
                instance->SetData(DATA_SNOWDRIFT_STATE, IN_PROGRESS);
                instance->SetBossState(DATA_SNOWDRIFT_STATE, IN_PROGRESS);
...
public:
    ...
    {
        if(unit->GetDistance(caster) <= 1.5 && unit->GetTypeId() == TYPEID_PLAYER && !unit->ToPlayer()->HasAura(SPELL_BALL_OF_FIRE_AURA_BLUE) && !unit->ToPlayer()->HasAura(SPELL_BALL_OF_FIRE_AURA_RED))
            return false;
