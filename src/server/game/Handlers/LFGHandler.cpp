...
void WorldSession::HandleLfgLockInfoRequestOpcode(WorldPacket& recvData)
{
    ...
    sLog->outDebug(LOG_FILTER_NETWORKIO, "CMSG_LFD_LOCK_INFO_REQUEST [" UI64FMTD "]", uint64(guid));
    ...
    sLog->outDebug(LOG_FILTER_NETWORKIO, "SMSG_LFG_PLAYER_INFO [" UI64FMTD "]", uint64(guid));
...
{
    sLog->outDebug(LOG_FILTER_LFG, "CMSG_LFG_GET_STATUS %s", GetPlayer()->GetName());
...
{
    sLog->outDebug(LOG_FILTER_NETWORKIO, "SMSG_LFG_ROLE_CHOSEN [" UI64FMTD "] guid: [" UI64FMTD "] roles: %u", GetPlayer()->GetGUID(), uint64(guid), roles);
