...
public:
    ...
    ChatCommand* GetCommands() const
    {
        static ChatCommand rbacAccountCommandTable[] =
        {
            ...
            {       "check", rbac::RBAC_PERM_COMMAND_RBAC_ACC_PERM_CHECK,  true, &HandleRBACPermCheckCommand,    "", NULL },
    ...
    static RBACCommandData* ReadParams(ChatHandler* handler, char const* args, bool checkParams = true)
    {
        ...
        if (checkParams && handler->HasLowerSecurityAccount(session, accountId, true))
            return NULL;

        RBACCommandData * const data = new RBACCommandData();
        rbac::RBACData * const rdata = session ? session->GetRBACData() : NULL;
    ...
    static bool HandleRBACPermListCommand(ChatHandler* handler, char const* args)
    {
        ...
        handler->PSendSysMessage(LANG_RBAC_LIST_HEADER_BY_SEC_LEVEL, command->rbac->GetId(), command->rbac->GetSecurityLevel());
    ...
    static bool HandleRBACPermCheckCommand(ChatHandler* handler, char const* args)
    {
        RBACCommandData* command = ReadParams(handler, args, false);
        ...
        if (command->rbac->HasPermission(command->id))
        {
            rbac::RBACPermission const* permission = sAccountMgr->GetRBACPermission(command->id);
            handler->PSendSysMessage(LANG_RBAC_LIST_ELEMENT, permission->GetId(), permission->GetName().c_str());
        }
