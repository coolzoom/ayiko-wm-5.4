...
Player* Map::_GetScriptPlayerSourceOrTarget(Object* source, Object* target, ScriptInfo const &scriptInfo) const
{
    Player* player = NULL;
    if (!source && !target)
...
Creature* Map::_GetScriptCreatureSourceOrTarget(Object* source, Object* target, ScriptInfo const &scriptInfo, bool bReverse) const
{
...
Unit* Map::_GetScriptUnit(Object* obj, bool isSource, ScriptInfo const &scriptInfo) const
{
    Unit* unit = NULL;
    if (!obj)
...
Player* Map::_GetScriptPlayer(Object* obj, bool isSource, ScriptInfo const &scriptInfo) const
{
    Player* player = NULL;
    if (!obj)
        TC_LOG_ERROR("scripts", "%s %s object is NULL.", scriptInfo.GetDebugInfo().c_str(), isSource ? "source" : "target");
...
Creature* Map::_GetScriptCreature(Object* obj, bool isSource, ScriptInfo const &scriptInfo) const
{
    Creature* creature = NULL;
    if (!obj)
        TC_LOG_ERROR("scripts", "%s %s object is NULL.", scriptInfo.GetDebugInfo().c_str(), isSource ? "source" : "target");
...
WorldObject* Map::_GetScriptWorldObject(Object* obj, bool isSource, ScriptInfo const &scriptInfo) const
{
...
void Map::_ScriptProcessDoor(Object* source, Object* target, ScriptInfo const &scriptInfo) const
{
    bool bOpen = false;
    ...
    while (!m_scriptSchedule.empty() && (iter->first <= sWorld->GetGameTime()))
    {
        ...
        switch (step.script.command)
        {
            case SCRIPT_COMMAND_TALK:
                ...
                {
                    TC_LOG_ERROR("scripts", "%s invalid chat type (%u), script type %u specified, skipping.", step.script.GetDebugInfo().c_str(), step.script.Talk.ChatType, step.script.type);
                    break;
                }
            ...
            case SCRIPT_COMMAND_MOVE_TO:
                ...
                if (Creature* cSource = _GetScriptCreatureSourceOrTarget(source, target, step.script))
                {
                    ...
                    {
                        float speed = unit->GetDistance(step.script.MoveTo.DestX, step.script.MoveTo.DestY, step.script.MoveTo.DestZ) / (step.script.MoveTo.TravelTime * 0.001f);
