...
void WorldPacket::Compress(z_stream* compressionStream)
{
    ...
    {
        sLog->outError("network", "Packet with opcode 0x%04X is already compressed!", uncompressedOpcode);
        return;
    }
    ...
    *this << uint32(size);
    ...
    sLog->outInfo("network", "Successfully compressed opcode %u (len %u) to %u (len %u)", uncompressedOpcode, size, opcode, destsize);
}
...
void WorldPacket::Compress(void* dst, uint32 *dst_size, const void* src, int src_size)
{
    ...
    if (z_res != Z_OK)
    {
        sLog->outError("network", "Can't compress packet (zlib: deflate) Error code: %i (%s, msg: %s)", z_res, zError(z_res), _compressionStream->msg);
        *dst_size = 0;
        return;
    }
