        ...
        class spell_blood_queen_vampiric_bite_SpellScript : public SpellScript
        {
            ...
            void OnCast()
            {
                ...
                if (GetCaster()->ToPlayer()->GetQuestStatus(QUEST_BLOOD_INFUSION) == QUEST_STATUS_INCOMPLETE)
                {
                    ...
                    {
                        Aura *aura = GetCaster()->GetAura(SPELL_GUSHING_WOUND);
                        if (aura != NULL)
                        {
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                ...
                if (InstanceScript* instance = GetCaster()->GetInstanceScript())
                {
                    ...
                    if(instance->GetData(DATA_BLOODQUEEN_TARGETS_PROC) == 1) // premier proc
                    {
                        ...
                        instance->SetData64(DATA_BLOODQUEEN_TARGETS, 0); // Met les joueurs dans un ordre al�atoire
                    ...
                    if(FirstProc) // Perte de donn�e normale : on arrondi � l'entier inf�rieur
