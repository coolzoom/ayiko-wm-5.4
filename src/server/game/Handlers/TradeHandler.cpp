...
void WorldSession::SendUpdateTrade(bool trader_data /*= true*/)
{
    ...
    data << uint32(TRADE_STATUS_BEGIN_TRADE);               // unk values but necessary, TradeStatus ?
    ...
    for (uint8 i = 0; i < TRADE_SLOT_COUNT; ++i)
    {
        ...
        data.WriteBitSeq<3, 0, 6>(giftCreatorGuid);
        bool notWrapped = data.WriteBit(!item->HasFlag(ITEM_FIELD_FLAGS, ITEM_FLAG_WRAPPED));
        data.WriteBitSeq<1, 2, 7>(giftCreatorGuid);

        if (notWrapped)
        {
            data.WriteBitSeq<6, 3, 5, 1, 4, 2>(creatorGuid);
            data.WriteBit(item->GetTemplate()->LockID != 0);
            data.WriteBitSeq<7, 0>(creatorGuid);
            ...
            itemData << uint32(0);
            itemData << uint32(0);
            itemData.WriteByteSeq<6>(creatorGuid);
            itemData << uint32(0);
            itemData.WriteByteSeq<3>(creatorGuid);
            itemData << uint32(0);
            itemData.WriteByteSeq<1>(creatorGuid);
            itemData << uint32(0);
            itemData.WriteByteSeq<0, 7, 4, 5>(creatorGuid);

            itemData << uint32(0);
            itemData << uint32(0);
        }

        data.WriteBitSeq<4, 5>(giftCreatorGuid);

        itemData.WriteByteSeq<3, 7, 4>(giftCreatorGuid);
        itemData << uint8(i);
        itemData.WriteByteSeq<6>(giftCreatorGuid);
        itemData << uint32(item->GetTemplate()->ItemId);
        itemData.WriteByteSeq<0, 1, 2, 5>(giftCreatorGuid);
...
void WorldSession::HandleInitiateTradeOpcode(WorldPacket& recvPacket)
{
    ...
    recvPacket.ReadBitSeq<7, 4, 2, 6, 0, 1, 3, 5>(guid);
    recvPacket.ReadByteSeq<2, 5, 0, 3, 7, 1, 6, 4>(guid);
    ...
    // Is need update statuts enum ?
    data.WriteBits(TRADE_STATUS_BEGIN_TRADE, 5);

    ObjectGuid playerGuid = _player->GetGUID();
    // WTB StartBitStream...

    data.WriteBitSeq<2, 4, 1, 0, 6, 3, 7, 5>(playerGuid);
    data.WriteBit(0); // unk bit, usually 0
    data.WriteByteSeq<3, 1, 5, 7, 4, 0, 6, 2>(playerGuid);
