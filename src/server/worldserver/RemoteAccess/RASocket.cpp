...
int RASocket::open(void *)
{
    ...
    {
        sLog->outError("server.worldserver", "RASocket::open: peer().get_remote_addr error is %s", ACE_OS::strerror(errno));
        return -1;
    }

    sLog->outDebug("commands.ra", "Incoming connection from %s", remote_addr.get_host_addr());
...
{
    sLog->outDebug("commands.ra", "Closing connection");
    peer().close_reader();
    wait();
    destroy();
    return 0;
...
int RASocket::recv_line(std::string& out_line)
{
    ...
    {
        sLog->outDebug("commands.ra", "Recv error %s", ACE_OS::strerror(errno));
        return -1;
    }
...
int RASocket::process_command(const std::string& command)
{
    ...
    sLog->outDebug("commands.ra", "Got command: %s", command.c_str());
...
int RASocket::check_access_level(const std::string& user)
{
    ...
    if (!result)
    {
        sLog->outDebug("commands.ra", "User %s does not exist in database", user.c_str());
        return -1;
    }
    ...
    {
        sLog->outDebug("commands.ra", "User %s has no privilege to login", user.c_str());
        return -1;
    }
    else if (fields[2].GetInt32() != -1)
    {
        sLog->outDebug("commands.ra", "User %s has to be assigned on all realms (with RealmID = '-1')", user.c_str());
        return -1;
    }
...
int RASocket::check_password(const std::string& user, const std::string& pass)
{
    ...
    if (!result)
    {
        sLog->outDebug("commands.ra", "Wrong password for user: %s", user.c_str());
        return -1;
    }
...
int RASocket::authenticate()
{
    ...
    sLog->outDebug("commands.ra", "Login attempt for user: %s", user.c_str());
    ...
    sLog->outDebug("commands.ra", "User login: %s", user.c_str());
...
int RASocket::subnegotiate()
{
    ...
    if (n >= 1024)
    {
        sLog->outDebug("commands.ra", "RASocket::subnegotiate: allocated buffer 1024 bytes was too small for negotiation packet, size: %u", uint32(n));
        return -1;
    }
    ...
    for (uint8 i = 0; i < n; )
    {
        ...
        if (iac == 0xFF)   // "Interpret as Command" (IAC)
        {
            ...
            uint8 param = buf[++i];
            ss << uint32(param);
            sLog->outDebug("commands.ra", ss.str().c_str());
        }
        ++i;
    }
    #endif
...
void RASocket::zprint(void* callbackArg, const char * szText)
{
    ...
    {
        sLog->outDebug("commands.ra", "Failed to enqueue message, queue is full or closed. Error is %s", ACE_OS::strerror(errno));
        mb->release();
    }
}
...
void RASocket::commandFinished(void* callbackArg, bool /*success*/)
{
    ...
    if (socket->putq(mb) == -1)
    {
        ...
        sLog->outDebug("commands.ra", "Failed to enqueue command end message. Error is %s", ACE_OS::strerror(errno));
        mb->release();
    }
}
