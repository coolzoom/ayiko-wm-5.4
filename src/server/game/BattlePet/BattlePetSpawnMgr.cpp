...
void BattlePetSpawnMgr::Initialise()
{
    ...
    do
    {
        ...
        {
            if (auto areaInfo = sAreaStore.LookupEntry(i))
            {
...
void BattlePetSpawnMgr::OnAddToOrRemoveFromMap(Creature* creature, bool remove)
{
    if (!creature)
        return;
    ...
    remove ? m_battlePetMapPools[mapId][zoneId].OnRemoveFromZone(creature): m_battlePetMapPools[mapId][zoneId].OnAddToZone(creature);
}
...
void BattlePetSpawnZoneMgr::OnRespawn(Creature* creature, BattlePet* battlePet)
{
    ...
    creature->SetFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_PETBATTLE);
    creature->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_NPC);
    creature->SetUInt32Value(UNIT_FIELD_WILD_BATTLE_PET_LEVEL, battlePet->GetLevel());
}
...
void BattlePetSpawnZoneMgr::SpawnCreature(uint64 guid, BattlePetSpawnTemplate* spawnTemplate)
{
    ...
    if (!replacementCreature->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_UNIT), creature->GetMap(), creature->GetPhaseMask(),
        speciesEntry->NpcId, 0, TEAM_NONE, creature->m_positionX, creature->m_positionY, creature->m_positionZ, creature->m_orientation))
    {
    ...
    replacementCreature->SetFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_PETBATTLE);
...
void BattlePetSpawnZoneMgr::RemoveCreature(uint64 guid, BattlePetSpawnTemplate* spawnTemplate)
{
    ...
    auto battlePet = spawnTemplate->WildBattlePetInfo[replacementCreature->GetGUID()];
    if (battlePet)
    {
