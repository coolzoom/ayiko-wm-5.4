...
Creature::~Creature()
{
    ...
    //if (m_uint32Values)
    //    sLog->outError("entities.unit", "Deconstruct Creature Entry = %u", GetEntry());
}
...
bool Creature::InitEntry(uint32 Entry, uint32 /*team*/, const CreatureData* data)
{
    ...
    if (!normalInfo)
    {
        sLog->outError("sql.sql", "Creature::InitEntry creature entry %u does not exist.", Entry);
        return false;
    }
    ...
    {
        sLog->outError("sql.sql", "Creature (Entry: %u) has no model defined in table `creature_template`, can't load. ", Entry);
        return false;
    }
...
void Creature::Update(uint32 diff)
{
    ...
    switch (m_deathState)
    {
        case JUST_RESPAWNED:
            ...
            sLog->outError("entities.unit", "Creature (GUID: %u Entry: %u) in wrong state: JUST_RESPAWNED (4)", GetGUIDLow(), GetEntry());
            break;
        case JUST_DIED:
            // Must not be called, see Creature::setDeathState JUST_DIED -> CORPSE promoting.
            sLog->outError("entities.unit", "Creature (GUID: %u Entry: %u) in wrong state: JUST_DEAD (1)", GetGUIDLow(), GetEntry());
            break;
        case DEAD:
        {
        ...
        case CORPSE:
        {
            ...
            {
                RemoveCorpse(false);
                sLog->outDebug("entities.unit", "Removing corpse... %u ", GetUInt32Value(OBJECT_FIELD_ENTRY));
            }
            break;
        }
        case ALIVE:
...
bool Creature::Create(uint32 guidlow, Map* map, uint32 phaseMask, uint32 Entry, uint32 vehId, uint32 team, float x, float y, float z, float ang, const CreatureData* data)
{
    ...
    if (!cinfo)
    {
        sLog->outError("sql.sql", "Creature::Create(): creature template (guidlow: %u, entry: %u) does not exist.", guidlow, Entry);
        return false;
    }
    ...
    if (!IsPositionValid())
    {
        sLog->outError("entities.unit", "Creature::Create(): given coordinates for creature (guidlow %d, entry %d) are not valid (X: %f, Y: %f, Z: %f, O: %f)", guidlow, Entry, x, y, z, ang);
        return false;
    }
...
void Creature::SaveToDB()
{
    ...
    if (!data)
    {
        sLog->outError("entities.unit", "Creature::SaveToDB failed, cannot get creature data!");
        return;
    }
...
bool Creature::CreateFromProto(uint32 guidlow, uint32 Entry, uint32 vehId, uint32 team, const CreatureData* data)
{
    ...
    if (!cinfo)
    {
        sLog->outError("sql.sql", "Creature::CreateFromProto(): creature template (guidlow: %u, entry: %u) does not exist.", guidlow, Entry);
        return false;
    }
...
bool Creature::LoadCreatureFromDB(uint32 guid, Map* map, bool addToMap)
{
    ...
    if (!data)
    {
        sLog->outError("sql.sql", "Creature (GUID: %u) not found in table `creature`, can't load. ", guid);
        return false;
    }
...
void Creature::DeleteFromDB()
{
    if (!m_DBTableGuid)
    {
        sLog->outError("entities.unit", "Trying to delete not saved creature! LowGUID: %u, Entry: %u", GetGUIDLow(), GetEntry());
        return;
    }
...
void Creature::Respawn(bool force)
{
    ...
    if (getDeathState() == DEAD)
    {
        ...
        sLog->outDebug("entities.unit", "Respawning creature %s (GuidLow: %u, Full GUID: " UI64FMTD " Entry: %u)", GetName(), GetGUIDLow(), GetGUID(), GetEntry());
        m_respawnTime = 0;
...
SpellInfo const* Creature::reachWithSpellAttack(Unit* victim)
{
    ...
    for (uint32 i=0; i < CREATURE_MAX_SPELLS; ++i)
    {
        ...
        if (!spellInfo)
        {
            sLog->outError("entities.unit", "WORLD: unknown spell id %i", m_spells[i]);
            continue;
        }
...
Unit* Creature::SelectNearestTargetInAttackDistance(float dist) const
{
    ...
    {
        sLog->outError("entities.unit", "Creature (GUID: %u Entry: %u) SelectNearestTargetInAttackDistance called with dist > MAX_VISIBILITY_DISTANCE. Distance set to ATTACK_DISTANCE.", GetGUIDLow(), GetEntry());
...
void Creature::SendAIReaction(AiReaction reactionType)
{
    ...
    sLog->outDebug("network", "WORLD: Sent SMSG_AI_REACTION, type %u.", reactionType);
}
...
bool Creature::LoadCreaturesAddon(bool reload)
{
    ...
    if (!cainfo->auras.empty())
    {
        for (std::vector<uint32>::const_iterator itr = cainfo->auras.begin(); itr != cainfo->auras.end(); ++itr)
        {
            ...
            {
                sLog->outError("sql.sql", "Creature (GUID: %u Entry: %u) has wrong spell %u defined in `auras` field.", GetGUIDLow(), GetEntry(), *itr);
                continue;
            }
            ...
            {
                if (!reload)
                    sLog->outError("sql.sql", "Creature (GUID: %u Entry: %u) has duplicate aura (spell %u) in `auras` field.", GetGUIDLow(), GetEntry(), *itr);

                continue;
            }

            AddAura(*itr, this);
            sLog->outDebug("entities.unit", "Spell: %u added to creature (GUID: %u Entry: %u)", *itr, GetGUIDLow(), GetEntry());
        }
    }
...
void Creature::SetInCombatWithZone()
{
    ...
    {
        sLog->outError("entities.unit", "Creature entry %u call SetInCombatWithZone but creature cannot have threat list.", GetEntry());
        return;
    }
    ...
    if (!map->IsDungeon())
    {
        sLog->outError("entities.unit", "Creature entry %u call SetInCombatWithZone for map (id: %u) that isn't an instance.", GetEntry(), map->GetId());
        return;
    }
...
void Creature::SetPosition(float x, float y, float z, float o)
{
    ...
    {
        sLog->outDebug("entities.unit", "Creature::SetPosition(%f, %f, %f) .. bad coordinates!", x, y, z);
        return;
    }
