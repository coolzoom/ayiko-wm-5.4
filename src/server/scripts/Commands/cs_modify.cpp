...
public:
    ...
    static bool HandleModifyEnergyCommand(ChatHandler* handler, const char* args)
    {
        ...
        sLog->outDebug("misc", handler->GetTrinityString(LANG_CURRENT_ENERGY), target->GetMaxPower(POWER_ENERGY));
...
// strtoull doesn't exist on WIN
        ...
        if (addmoney < 0)
        {
            ...
            sLog->outDebug("misc", handler->GetTrinityString(LANG_CURRENT_MONEY), moneyuser, addmoney, newmoney);
            if (newmoney <= 0)
            {
            ...
            {
                if (static_cast<uint64>(newmoney) > MAX_MONEY_AMOUNT)
                ...
                target->SetMoney(static_cast<uint64>(newmoney));
            }
        }
        else
        {
            ...
            if (static_cast<uint64>(addmoney) >= MAX_MONEY_AMOUNT)
                target->SetMoney(MAX_MONEY_AMOUNT);
            else
                target->ModifyMoney(addmoney);
        }

        sLog->outDebug("misc", handler->GetTrinityString(LANG_NEW_MONEY), moneyuser, uint32(addmoney), target->GetMoney());
        return true;
    }
