...
void PlayerMenu::SendPointOfInterest(uint32 poiId) const
{
    ...
    if (!poi)
    {
        sLog->outError("sql.sql", "Request to send non-existing POI (Id: %u), ignored.", poiId);
        return;
    }
...
void PlayerMenu::SendQuestGiverQuestList(QEmote eEmote, const std::string& Title, uint64 npcGUID)
{
    ...
    _session->SendPacket(&data);
    sLog->outDebug("network", "WORLD: Sent SMSG_QUESTGIVER_QUEST_LIST NPC Guid=%u", GUID_LOPART(npcGUID));
}
...
void PlayerMenu::SendQuestGiverStatus(uint32 questStatus, uint64 npcGUID) const
{
    ...
    _session->SendPacket(&data);
    sLog->outDebug("network", "WORLD: Sent SMSG_QUESTGIVER_STATUS NPC Guid=%u, status=%u", GUID_LOPART(npcGUID), questStatus);
}
...
void PlayerMenu::SendQuestGiverQuestDetails(Quest const* quest, uint64 npcGUID, bool /*activateAccept*/) const
{
    ...
    */
    ...
    sLog->outDebug("network", "WORLD: Sent SMSG_QUESTGIVER_QUEST_DETAILS NPCGuid=%u, questid=%u", GUID_LOPART(npcGUID), quest->GetQuestId());
}
...
void PlayerMenu::SendQuestQueryResponse(Quest const* quest) const
{
    ...
    _session->SendPacket(&data);
    sLog->outDebug("network", "WORLD: Sent SMSG_QUEST_QUERY_RESPONSE questid=%u", quest->GetQuestId());
}
...
void PlayerMenu::SendQuestGiverOfferReward(Quest const* quest, uint64 npcGUID, bool /*enableNext*/) const
{
    ...
    */
    ...
    _session->SendPacket(&data);
    sLog->outDebug("network", "WORLD: Sent SMSG_QUESTGIVER_OFFER_REWARD NPCGuid=%u, questid=%u", GUID_LOPART(npcGUID), quest->GetQuestId());
}
...
void PlayerMenu::SendQuestGiverRequestItems(Quest const* quest, uint64 npcGUID, bool canComplete, bool closeOnCancel) const
{
    ...
    _session->SendPacket(&data);
    sLog->outDebug("network", "WORLD: Sent SMSG_QUESTGIVER_REQUEST_ITEMS NPCGuid=%u, questid=%u", GUID_LOPART(npcGUID), quest->GetQuestId());
}
