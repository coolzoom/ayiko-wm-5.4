...
{
    m_UpdateTimer = 0;
    //sLog->outDebug("outdoorpvp", "Instantiating OutdoorPvPMgr");
}

void OutdoorPvPMgr::Die()
{
    //sLog->outDebug("outdoorpvp", "Deleting OutdoorPvPMgr");
...
void OutdoorPvPMgr::InitOutdoorPvP()
{
    ...
    if (!result)
    {
        sLog->outError("server.loading", ">> Loaded 0 outdoor PvP definitions. DB table `outdoorpvp_template` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            sLog->outError("sql.sql", "Invalid OutdoorPvPTypes value %u in outdoorpvp_template; skipped.", typeId);
            continue;
        }
    ...
    for (uint8 i = 1; i < MAX_OUTDOORPVP_TYPES; ++i)
    {
        ...
        {
            sLog->outError("sql.sql", "Could not initialize OutdoorPvP object for type ID %u; no entry in database.", uint32(i));
            continue;
        }
        ...
        if (!pvp)
        {
            sLog->outError("outdoorpvp", "Could not initialize OutdoorPvP object for type ID %u; got NULL pointer from script.", uint32(i));
            continue;
        }

        if (!pvp->SetupOutdoorPvP())
        {
            sLog->outError("outdoorpvp", "Could not initialize OutdoorPvP object for type ID %u; SetupOutdoorPvP failed.", uint32(i));
            delete pvp;
            continue;
        }
    ...
    sLog->outInfo("server.loading", ">> Loaded %u outdoor PvP definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void OutdoorPvPMgr::HandlePlayerEnterZone(Player* player, uint32 zoneid)
{
    ...
    sLog->outDebug("outdoorpvp", "Player %u entered outdoorpvp id %u", player->GetGUIDLow(), itr->second->GetTypeId());
}
...
void OutdoorPvPMgr::HandlePlayerLeaveZone(Player* player, uint32 zoneid)
{
    ...
    sLog->outDebug("outdoorpvp", "Player %u left outdoorpvp id %u", player->GetGUIDLow(), itr->second->GetTypeId());
}
