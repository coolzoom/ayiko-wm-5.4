...
void WaypointMgr::Load()
{
    ...
    if (!result)
    {
        sLog->outError("server.loading", ">> Loaded 0 waypoints. DB table `waypoint_data` is empty!");
    ...
    sLog->outInfo("server.loading", ">> Loaded %u waypoints in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
