    ...
    struct boss_gu_cloudstrikeAI : public BossAI
    {
        ...
        void EnterCombat(Unit* who) override
        {
            if(!azurePhase)
            {
                ...
                events.ScheduleEvent(EVENT_INVOKE_LIGHTNING, urand(5, 10) * IN_MILLISECONDS);
            }
        }
            ...
            if(uint32 eventId = events.ExecuteEvent())
            {
                switch(eventId)
                {
                    ...
                    case EVENT_STATIC_FIELD:
                        ...
                        events.ScheduleEvent(EVENT_STATIC_FIELD, urand(13, 16) * IN_MILLISECONDS);
                        break;
                }
            }
...
public:
    ...
    struct npc_azure_serpentAI : public ScriptedAI
    {
        ...
        void DoAction(const int32 action) override
        {
            if(action == ACTION_AZURE_SERPENT_P_2)
            {
                events.Reset();
                Talk(TALK_PHASE_2);
                Movement::MoveSplineInit init(me);
        ...
        void UpdateAI(const uint32 diff) override
        {
            ...
            switch(events.ExecuteEvent())
            {
                ...
                case EVENT_MAGNETIC_SHROUD:
                    ...
                    events.ScheduleEvent(EVENT_MAGNETIC_SHROUD, urand(12, 16) * IN_MILLISECONDS);
                    break;
                case EVENT_LIGHTNING_BREATH:
                    ...
                    events.ScheduleEvent(EVENT_LIGHTNING_BREATH, urand(8, 12) * IN_MILLISECONDS);
                    break;
                case EVENT_SWITCH_PHASE:
                    ...
                    events.ScheduleEvent(EVENT_MAGNETIC_SHROUD, urand(8, 12) * IN_MILLISECONDS);
                    events.ScheduleEvent(EVENT_LIGHTNING_BREATH, urand(2.5, 7.5) * IN_MILLISECONDS);
                    break;
            }
...
public:
    ...
    {
        if(auto const pInstance = player->GetInstanceScript())
           if(auto const gu = pInstance->instance->GetCreature(pInstance->GetData64(DATA_GU_CLOUDSTRIKE)))
             if(gu->IsAIEnabled)
