...
void BattlegroundSSM::Reset()
{
    ...
    m_HonorTics = (isBGWeekend) ? BG_SSM_SSMWeekendHonorTicks : BG_SSM_NotSSMWeekendHonorTicks;
...
bool BattlegroundSSM::SetupBattleground()
{
    ...
    if (!sg || !AddSpiritGuide(SSM_SPIRIT_ALLIANCE, sg->x, sg->y, sg->z, 2.138462f, TEAM_ALLIANCE))
    {
    ...
    if (!sg || !AddSpiritGuide(SSM_SPIRIT_HORDE, sg->x, sg->y, sg->z, 5.570653f, TEAM_HORDE))
    {
        TC_LOG_ERROR("sql.sql", "BatteGroundSM: Failed to spawn spirit guide! Battleground not created!");
        return false;
    }
...
{
    SSMBattlegroundGaveyards bggy = player->GetBGTeam() == ALLIANCE ? SSM_GRAVEYARD_MAIN_ALLIANCE : SSM_GRAVEYARD_MAIN_HORDE;
