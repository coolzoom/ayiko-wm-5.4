...
public:
    ...
    {
        boss_spirit_kings_controllerAI(Creature* creature) : BossAI(creature, DATA_SPIRIT_KINGS), summons(creature)
        {
        ...
        void DoAction(const int32 action)
        {
            ...
            switch (action)
            {
                case ACTION_ENTER_COMBAT:
                {
                    ...
                    if (Creature* incomingSpirit = pInstance->instance->GetCreature(pInstance->GetData64(spiritKingsEntry[1])))
                ...
                case ACTION_SAY_INTRO_FOR_NEXT_SPIRIT_IF_ANY:
                {
                    ...
                    if (Creature* incomingSpirit = pInstance->instance->GetCreature(pInstance->GetData64(spiritKingsEntry[vanquishedCount + 1])))
                    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (events.ExecuteEvent() == EVENT_CHECK_WIPE)
            {
                if (pInstance->IsWipe())
                {
                    for (auto itr : spiritKingsEntry)
                        if (Creature* spirit = pInstance->instance->GetCreature(pInstance->GetData64(itr)))
                            spirit->AI()->EnterEvadeMode();

                        if (pInstance)
                        {
                            pInstance->SetBossState(DATA_SPIRIT_KINGS, FAIL);
                            pInstance->SetData(DATA_SPIRIT_KINGS, FAIL);
                            pInstance->DoRemoveAurasDueToSpellOnPlayers(SPELL_MADDENING_SHOUT);
                        }
...
public:
    ...
    struct boss_spirit_kingsAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            if (me->GetEntry() == NPC_QIANG)
            {
                ...
                if (pInstance)
                {
                    ...
                    pInstance->SetData(DATA_SPIRIT_KINGS, NOT_STARTED);
                }
            }
        ...
        void EnterCombat(Unit* /*attacker*/)
        {
            if (!aggroDone)
            {
                ...
                switch (me->GetEntry())
                {
                    ...
                    case NPC_MENG:
                        Talk(MENG_AGGRO);
                        afterEvents.ScheduleEvent(EVENT_MADDENING_SHOUT, 21000);
            ...
            if (me->HealthBelowPctDamaged(5, damage) && !vanquished)
            {
                ...
                switch (me->GetEntry())
                {
                    ...
                    case NPC_MENG:
                        ...
                        afterEvents.ScheduleEvent(EVENT_MADDENING_SHOUT, 35000);
        ...
        void UpdateAI(const uint32 diff)
        {
            if (uint32 eventId = afterEvents.ExecuteEvent())
            {
                switch (eventId)
                {
