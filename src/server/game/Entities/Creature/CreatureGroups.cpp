...
void FormationMgr::AddCreatureToGroup(uint32 groupId, Creature* member)
{
    ...
    {
        sLog->outDebug("entities.unit", "Group found: %u, inserting creature GUID: %u, Group InstanceID %u", groupId, member->GetGUIDLow(), member->GetInstanceId());
...
{
    sLog->outDebug("entities.unit", "Deleting member pointer to GUID: %u from group %u", group->GetId(), member->GetDBTableGUIDLow());
    ...
    if (group->isEmpty())
    {
        ...
        sLog->outDebug("entities.unit", "Deleting group with InstanceID %u", member->GetInstanceId());
...
void FormationMgr::LoadCreatureFormations()
{
    ...
    if (!result)
    {
        sLog->outError("server.loading", ">>  Loaded 0 creatures in formations. DB table `creature_formations` is empty!");
        return;
    }
    ...
    do
    {
        ...
        // Check data correctness
        {
            ...
            {
                sLog->outError("sql.sql", "creature_formations table leader guid %u incorrect (not exist)", group_member->leaderGUID);
                delete group_member;
                continue;
            }
            ...
            {
                sLog->outError("sql.sql", "creature_formations table member guid %u incorrect (not exist)", memberGUID);
                delete group_member;
                continue;
            }
        }
    ...
    sLog->outInfo("server.loading", ">> Loaded %u creatures in formations in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}

void CreatureGroup::AddMember(Creature* member)
{
    sLog->outDebug("entities.unit", "CreatureGroup::AddMember: Adding unit GUID: %u.", member->GetGUIDLow());
...
void CreatureGroup::MemberAttackStart(Creature* member, Unit* target)
{
    ...
    for (CreatureGroupMemberType::iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
    {
        if (m_leader)                                   // Avoid crash if leader was killed and reset.
            sLog->outDebug("entities.unit", "GROUP ATTACK: group instance id %u calls member instid %u", m_leader->GetInstanceId(), member->GetInstanceId());
...
void CreatureGroup::FormationReset(bool dismiss)
{
    for (CreatureGroupMemberType::iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
    {
        if (itr->first != m_leader && itr->first->isAlive())
        {
            ...
            sLog->outDebug("entities.unit", "Set %s movement for member GUID: %u", dismiss ? "default" : "idle", itr->first->GetGUIDLow());
        }
    }
    m_Formed = !dismiss;
}
