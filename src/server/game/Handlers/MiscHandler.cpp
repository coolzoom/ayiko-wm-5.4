...
{
    sLog->outDebug("network", "WORLD: Recvd CMSG_REPOP_REQUEST Message");
    ...
    {
        sLog->outDebug("network", "HandleRepopRequestOpcode: got request after player %s(%d) was killed and before he was updated", GetPlayer()->GetName(), GetPlayer()->GetGUIDLow());
...
{
    sLog->outDebug("network", "WORLD: CMSG_GOSSIP_SELECT_OPTION");
    ...
    if (IS_CRE_OR_VEH_GUID(guid))
    {
        ...
        if (!unit)
        {
            sLog->outDebug("network", "WORLD: HandleGossipSelectOptionOpcode - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(guid)));
            return;
        }
    }
    else if (IS_GAMEOBJECT_GUID(guid))
    {
        ...
        if (!go)
        {
            sLog->outDebug("network", "WORLD: HandleGossipSelectOptionOpcode - GameObject (GUID: %u) not found.", uint32(GUID_LOPART(guid)));
            return;
        }
    }
    else
    {
        sLog->outDebug("network", "WORLD: HandleGossipSelectOptionOpcode - unsupported GUID type for highguid %u. lowpart %u.", uint32(GUID_HIPART(guid)), uint32(GUID_LOPART(guid)));
        return;
    }
    ...
    {
        sLog->outDebug("network", "WORLD: HandleGossipSelectOptionOpcode - Script reloaded while in use, ignoring and set new scipt id");
        if (unit)
...
{
    sLog->outDebug("network", "WORLD: Recvd CMSG_WHO Message");
    ...
    for (uint32 i = 0; i < str_count; ++i)
    {
        ...
        wstrToLower(str[i]);
        sLog->outDebug("network", "String %u: %s", i, temp.c_str());
    }
    ...
    for (uint32 i = 0; i < zones_count; ++i)
    {
        ...
        zoneids[i] = temp;
        sLog->outDebug("network", "Zone %u: %u", i, zoneids[i]);
    }
    if (unkLen3 > 0)
    {
    ...
    sLog->outDebug("network", "Minlvl %u, maxlvl %u, name %s, guild %s, racemask %u, classmask %u, zones %u, strings %u", level_min, level_max, player_name.c_str(), guild_name.c_str(), racemask, classmask, zones_count, str_count);
    ...
    sLog->outDebug("network", "WORLD: Send SMSG_WHO Message");
}
void WorldSession::HandleLogoutRequestOpcode(WorldPacket& /*recvData*/)
{
    sLog->outDebug("network", "WORLD: Recvd CMSG_LOGOUT_REQUEST Message, security - %u", GetSecurity());
...
{
    sLog->outDebug("network", "WORLD: Recvd CMSG_PLAYER_LOGOUT Message");
}

void WorldSession::HandleLogoutCancelOpcode(WorldPacket& /*recvData*/)
{
    sLog->outDebug("network", "WORLD: Recvd CMSG_LOGOUT_CANCEL Message");
    ...
    sLog->outDebug("network", "WORLD: Sent SMSG_LOGOUT_CANCEL_ACK Message");
}
...
void WorldSession::HandleZoneUpdateOpcode(WorldPacket& recvData)
{
    ...
    sLog->outDebug("network", "WORLD: Recvd ZONE_UPDATE: %u", newZone);
...
{
    // sLog->outDebug("network", "WORLD: Received CMSG_STANDSTATECHANGE"); -- too many spam in log at lags/debug stop
    uint32 animstate;
    recvData >> animstate;
...
void WorldSession::HandleContactListOpcode(WorldPacket& recvData)
{
    ...
    sLog->outDebug("network", "WORLD: Received CMSG_CONTACT_LIST");
    _player->GetSocial()->SendSocialList(_player);
}

void WorldSession::HandleAddFriendOpcode(WorldPacket& recvData)
{
    sLog->outDebug("network", "WORLD: Received CMSG_ADD_FRIEND");
    ...
    sLog->outDebug("network", "WORLD: %s asked to add friend : '%s'", GetPlayer()->GetName(), friendName.c_str());
...
void WorldSession::HandleAddFriendOpcodeCallBack(PreparedQueryResult result, std::string friendNote)
{
            ...
            if (friendGuid)
            {
                ...
                else
                {
                    ...
                    if (!GetPlayer()->GetSocial()->AddToSocialList(GUID_LOPART(friendGuid), false))
                    {
                        ...
                        sLog->outDebug("network", "WORLD: %s's friend list is full.", GetPlayer()->GetName());
                    }
                }
    ...
    sLog->outDebug("network", "WORLD: Sent (SMSG_FRIEND_STATUS)");
}
...
void WorldSession::HandleDelFriendOpcode(WorldPacket& recvData)
{
    ...
    sLog->outDebug("network", "WORLD: Received CMSG_DEL_FRIEND");
    ...
    sLog->outDebug("network", "WORLD: Sent motd (SMSG_FRIEND_STATUS)");
}

void WorldSession::HandleAddIgnoreOpcode(WorldPacket& recvData)
{
    sLog->outDebug("network", "WORLD: Received CMSG_ADD_IGNORE");
    ...
    sLog->outDebug("network", "WORLD: %s asked to Ignore: '%s'", GetPlayer()->GetName(), ignoreName.c_str());
...
void WorldSession::HandleDelIgnoreOpcode(WorldPacket& recvData)
{
    ...
    sLog->outDebug("network", "WORLD: Received CMSG_DEL_IGNORE");
...
{
    sLog->outDebug("network", "CMSG_SET_CONTACT_NOTES");
    uint64 guid;
    std::string note;
    recvData >> guid >> note;
...
void WorldSession::HandleBugOpcode(WorldPacket& recvData)
{
    ...
    if (suggestion == 0)
        sLog->outDebug("network", "WORLD: Received CMSG_BUG [Bug Report]");
    else
        sLog->outDebug("network", "WORLD: Received CMSG_BUG [Suggestion]");

    sLog->outDebug("network", "%s", type.c_str());
    sLog->outDebug("network", "%s", content.c_str());
...
{
    sLog->outDebug("network", "WORLD: Received CMSG_RECLAIM_CORPSE");
...
{
    sLog->outDebug("network", "WORLD: Received CMSG_RESURRECT_RESPONSE");
...
void WorldSession::HandleAreaTriggerOpcode(WorldPacket& recvData)
{
    ...
    sLog->outDebug("network", "CMSG_AREATRIGGER. Trigger ID: %u", triggerId);

    Player* player = GetPlayer();
    if (player->isInFlight())
    {
        sLog->outDebug("network", "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) in flight, ignore Area Trigger ID:%u",
    ...
    if (!atEntry)
    {
        sLog->outDebug("network", "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) send unknown (by DBC) Area Trigger ID:%u",
            player->GetName(), player->GetGUIDLow(), triggerId);
        return;
    }
    ...
    {
        sLog->outDebug("network", "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) too far (trigger map: %u player map: %u), ignore Area Trigger ID: %u",
    ...
    if (atEntry->radius > 0)
    {
        ...
        {
            sLog->outDebug("network", "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) too far (radius: %f distance: %f), ignore Area Trigger ID: %u",
    ...
    else
    {
        ...
        {
            sLog->outDebug("network", "HandleAreaTriggerOpcode: Player '%s' (GUID: %u) too far (1/2 box X: %f 1/2 box Y: %f 1/2 box Z: %f rotatedPlayerX: %f rotatedPlayerY: %f dZ:%f), ignore Area Trigger ID: %u",
...
{
    sLog->outDebug("network", "WORLD: Received CMSG_UPDATE_ACCOUNT_DATA");
    ...
    sLog->outDebug("network", "UAD: type %u, time %u, decompressedSize %u", type, timestamp, decompressedSize);
    ...
    if (decompressedSize > 0xFFFF)
    {
        ...
        sLog->outError("network", "UAD: Account data packet too big, size %u", decompressedSize);
        return;
    }
    ...
    {
        recvData.rfinish();                   // unnneded warning spam in this case
        sLog->outError("network", "UAD: Failed to decompress account data");
        return;
    }
...
{
    sLog->outDebug("network", "WORLD: Received CMSG_REQUEST_ACCOUNT_DATA");

    uint32 type = recvData.ReadBits(3);
    recvData.FlushBits();

    sLog->outDebug("network", "RAD: type %u", type);
    ...
    {
        sLog->outDebug("network", "RAD: Failed to compress account data");
        return;
    }
...
{
    sLog->outDebug("network", "WORLD: Received CMSG_SET_ACTION_BUTTON");
    uint8 button;
    ...
    sLog->outInfo("network", "BUTTON: %u ACTION: %u TYPE: %u", button, action, type);
    if (!packetData)
    {
        sLog->outInfo("network", "MISC: Remove action from button %u", button);
    ...
    else
    {
        switch (type)
        {
            ...
            case ACTION_BUTTON_CMACRO:
                sLog->outInfo("network", "MISC: Added Macro %u into button %u", action, button);
                break;
            case ACTION_BUTTON_EQSET:
                sLog->outInfo("network", "MISC: Added EquipmentSet %u into button %u", action, button);
                break;
            case ACTION_BUTTON_SPELL:
                sLog->outInfo("network", "MISC: Added Spell %u into button %u", action, button);
                break;
            case ACTION_BUTTON_SUB_BUTTON:
                sLog->outInfo("network", "MISC: Added sub buttons %u into button %u", action, button);
                break;
            case ACTION_BUTTON_ITEM:
                sLog->outInfo("network", "MISC: Added Item %u into button %u", action, button);
                break;
            default:
                sLog->outError("network", "MISC: Unknown action button type %u for action %u into button %u for player %s (GUID: %u)", type, action, button, _player->GetName(), _player->GetGUIDLow());
                return;
        }
...
{
    sLog->outDebug("network", "WORLD: Received CMSG_COMPLETE_CINEMATIC");
}

void WorldSession::HandleNextCinematicCamera(WorldPacket& /*recvData*/)
{
    sLog->outDebug("network", "WORLD: Received CMSG_NEXT_CINEMATIC_CAMERA");
}

void WorldSession::HandleMoveTimeSkippedOpcode(WorldPacket& recvData)
{
    sLog->outDebug("network", "WORLD: Received CMSG_MOVE_TIME_SKIPPED");
...
{
    sLog->outDebug("network", "WORLD: CMSG_MOVE_FEATHER_FALL_ACK");
...
void WorldSession::HandleSetActionBarToggles(WorldPacket& recvData)
{
    ...
    {
        if (actionBar != 0)
            sLog->outError("network", "WorldSession::HandleSetActionBarToggles in not logged state with value: %u, ignored", uint32(actionBar));
        return;
    }
...
void WorldSession::HandleInspectOpcode(WorldPacket& recvData)
{
    ...
    sLog->outDebug("network", "WORLD: Received CMSG_INSPECT");
    ...
    if (!player)
    {
        sLog->outDebug("network", "CMSG_INSPECT: No player found from GUID: " UI64FMTD, uint64(playerGuid));
        return;
    }
...
void WorldSession::HandleInspectHonorStatsOpcode(WorldPacket& recvData)
{
    ...
    if (!player)
    {
        //sLog->outDebug("network", "CMSG_INSPECT_HONOR_STATS: No player found from GUID: " UI64FMTD, guid);
        return;
    }
...
void WorldSession::HandleInspectRatedBGStatsOpcode(WorldPacket& recvData)
{
    ...
    if (!player)
    {
        //sLog->outDebug("network", "CMSG_REQUEST_INSPECT_RATED_BG_STATS: No player found from GUID: " UI64FMTD, guid);
        return;
    }
...
void WorldSession::HandleWorldTeleportOpcode(WorldPacket& recvData)
{
    ...
    sLog->outDebug("network", "WORLD: Received CMSG_WORLD_TELEPORT");

    if (GetPlayer()->isInFlight())
    {
        sLog->outDebug("network", "Player '%s' (GUID: %u) in flight, ignore worldport command.", GetPlayer()->GetName(), GetPlayer()->GetGUIDLow());
        return;
    }

    sLog->outDebug("network", "CMSG_WORLD_TELEPORT: Player = %s, Time = %u, map = %u, x = %f, y = %f, z = %f, o = %f", GetPlayer()->GetName(), time, mapid, PositionX, PositionY, PositionZ, Orientation);
...
{
    sLog->outDebug("network", "Received opcode CMSG_WHOIS");
    ...
    sLog->outDebug("network", "Received whois command from player %s for character %s", GetPlayer()->GetName(), charname.c_str());
}

void WorldSession::HandleComplainOpcode(WorldPacket& /*recvData*/)
{
    sLog->outDebug("network", "WORLD: CMSG_COMPLAIN");
...
{
    sLog->outDebug("network", "CMSG_REALM_SPLIT");
...
{
    sLog->outDebug("network", "CMSG_REALM_QUERY_NAME");
...
{
    sLog->outDebug("network", "WORLD: CMSG_FAR_SIGHT");

    bool apply = recvData.ReadBit();

    if (apply)
    {
        sLog->outDebug("network", "Player %u set vision to self", _player->GetGUIDLow());
        _player->SetSeer(_player);
    }
    else
    {
        sLog->outDebug("network", "Added FarSight " UI64FMTD " to player %u", _player->GetUInt64Value(PLAYER_FARSIGHT), _player->GetGUIDLow());
        if (WorldObject* target = _player->GetViewpoint())
            _player->SetSeer(target);
        else
            sLog->outError("network", "Player %s requests non-existing seer " UI64FMTD, _player->GetName(), _player->GetUInt64Value(PLAYER_FARSIGHT));
    }
...
{
    sLog->outDebug("network", "CMSG_SET_TITLE");
...
{
    sLog->outDebug("network", "CMSG_TIME_SYNC_RESP");

    uint32 counter, clientTicks;
    recvData >> counter >> clientTicks;

    if (counter != _player->m_timeSyncCounter - 1)
        sLog->outDebug("network", "Wrong time sync counter from player %s (cheater?)", _player->GetName());

    sLog->outDebug("network", "Time sync received: counter %u, client ticks %u, time since last sync %u", counter, clientTicks, clientTicks - _player->m_timeSyncClient);

    uint32 ourTicks = clientTicks + (getMSTime() - _player->m_timeSyncServer);

    // diff should be small
    sLog->outDebug("network", "Our ticks: %u, diff %u, latency %u", ourTicks, ourTicks - clientTicks, GetLatency());
...
{
    sLog->outDebug("network", "WORLD: CMSG_RESET_INSTANCES");
...
{
    sLog->outDebug("network", "MSG_SET_DUNGEON_DIFFICULTY");
    ...
    {
        sLog->outError("network", "WorldSession::HandleSetDungeonDifficultyOpcode: player %d sent an invalid instance mode %d!", _player->GetGUIDLow(), mode);
        return;
    }
    ...
    {
        sLog->outError("network", "WorldSession::HandleSetDungeonDifficultyOpcode: player (Name: %s, GUID: %u) tried to reset the instance while player is inside!", _player->GetName(), _player->GetGUIDLow());
        return;
    }
    ...
    if (group)
    {
        if (group->IsLeader(_player->GetGUID()))
        {
            for (GroupReference* itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
            {
                ...
                {
                    sLog->outError("network", "WorldSession::HandleSetDungeonDifficultyOpcode: player %d tried to reset the instance while group member (Name: %s, GUID: %u) is inside!", _player->GetGUIDLow(), groupGuy->GetName(), groupGuy->GetGUIDLow());
                    return;
                }
            }
...
{
    sLog->outDebug("network", "MSG_SET_RAID_DIFFICULTY");
    ...
    {
        sLog->outError("network", "WorldSession::HandleSetRaidDifficultyOpcode: player %d sent an invalid instance mode %d!", _player->GetGUIDLow(), mode);
        return;
    }
    ...
    if (map && map->IsDungeon())
    {
        sLog->outError("network", "WorldSession::HandleSetRaidDifficultyOpcode: player %d tried to reset the instance while inside!", _player->GetGUIDLow());
        return;
    }
...
{
    sLog->outDebug("network", "WORLD: CMSG_CANCEL_MOUNT_AURA");
...
void WorldSession::HandleSetTaxiBenchmarkOpcode(WorldPacket& recvData)
{
    ...
    sLog->outDebug("network", "Client used \"/timetest %d\" command", mode);
}
...
{
    // empty opcode
    sLog->outDebug("network", "WORLD: CMSG_WORLD_STATE_UI_TIMER_UPDATE");
...
{
    // empty opcode
    sLog->outDebug("network", "WORLD: CMSG_READY_FOR_ACCOUNT_DATA_TIMES");
...
{
    sLog->outDebug("network", "WORLD: CMSG_AREA_SPIRIT_HEALER_QUERY");
...
{
    sLog->outDebug("network", "WORLD: CMSG_AREA_SPIRIT_HEALER_QUEUE");
...
void WorldSession::HandleInstanceLockResponse(WorldPacket& recvPacket)
{
    ...
    {
        sLog->outInfo("network", "InstanceLockResponse: Player %s (guid %u) tried to bind himself/teleport to graveyard without a pending bind!", _player->GetName(), _player->GetGUIDLow());
        return;
    }
    ...
    for (uint32 i = 0; i < count; ++i)
    {
        ...
        switch (type)
        {
            ...
            case DB2_REPLY_BROADCAST_TEXT:
                ...
                break;
            default:
                sLog->outError("network", "CMSG_REQUEST_HOTFIX: Received unknown hotfix type: %u", type);
                recvPacket.rfinish();
                return;
        }
    }
...
{
    sLog->outDebug("network", "WORLD: CMSG_UPDATE_MISSILE_TRAJECTORY");
...
void WorldSession::HandleObjectUpdateFailedOpcode(WorldPacket& recvPacket)
{
    ...
    sLog->outError("network", "Object update failed for object " UI64FMTD " (%s) for player %s (%u)", uint64(guid), obj ? obj->GetName() : "object-not-found", GetPlayerName().c_str(), GetGuidLow());
}
