    ...
    struct boss_shade_of_akamaAI : public ScriptedAI
    {
        ...
        void MoveInLineOfSight(Unit* /*who*/)
        {
            if (!GridSearcherSucceeded)
            {
                ...
                } else sLog->outError("scripts", "SD2 ERROR: No Channelers are stored in the list. This encounter will not work properly");
            }
        }
        ...
        void IncrementDeathCount(uint64 guid = 0)               // If guid is set, will remove it from list of sorcerer
        {
            ...
            sLog->outDebug("scripts", "Increasing Death Count for Shade of Akama encounter");
            ++DeathCount;
            me->RemoveAuraFromStack(SPELL_SHADE_SOUL_CHANNEL_2);
            if (guid)
            {
                if (Sorcerers.empty())
                    sLog->outError("scripts", "SD2 ERROR: Shade of Akama - attempt to remove guid " UI64FMTD " from Sorcerers list but list is already empty", guid);
        ...
        void FindChannelers()
        {
            ...
            if (!ChannelerList.empty())
            {
                for (std::list<Creature*>::const_iterator itr = ChannelerList.begin(); itr != ChannelerList.end(); ++itr)
                {
                    ...
                    sLog->outDebug("scripts", "Shade of Akama Grid Search found channeler " UI64FMTD ". Adding to list", (*itr)->GetGUID());
                }
            }
            else sLog->outError("scripts", "SD2 ERROR: Grid Search was unable to find any channelers. Shade of Akama encounter will be buggy");
        }
        ...
        void SetSelectableChannelers()
        {
            if (Channelers.empty())
            {
                sLog->outError("scripts", "SD2 ERROR: Channeler List is empty, Shade of Akama encounter will be buggy");
                return;
            }
...
void mob_ashtongue_channeler::mob_ashtongue_channelerAI::JustDied(Unit* /*killer*/)
{
    ...
    else sLog->outError("scripts", "SD2 ERROR: Channeler dead but unable to increment DeathCount for Shade of Akama.");
}
...
void mob_ashtongue_sorcerer::mob_ashtongue_sorcererAI::JustDied(Unit* /*killer*/)
{
    ...
    else sLog->outError("scripts", "SD2 ERROR: Sorcerer dead but unable to increment DeathCount for Shade of Akama.");
}
