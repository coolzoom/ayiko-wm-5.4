...
void Guild::SendCommandResult(WorldSession* session, GuildCommandType type, GuildCommandError errCode, const std::string& param)
{
    ...
    sLog->outDebug("guild", "WORLD: Sent (SMSG_GUILD_COMMAND_RESULT)");
}
...
void Guild::SendSaveEmblemResult(WorldSession* session, GuildEmblemError errCode)
{
    ...
    sLog->outDebug("guild", "WORLD: Sent (SMSG_PLAYER_SAVE_GUILD_EMBLEM)");
}
...
inline void Guild::LogHolder::WritePacket(WorldPacket& data, bool hasCashFlow /* = false */) const
{
    ByteBuffer buffer;
...
bool Guild::BankTab::LoadItemFromDB(Field* fields)
{
    ...
    {
        sLog->outError("guild", "Invalid slot for item (GUID: %u, id: %u) in guild bank, skipped.", itemGuid, itemEntry);
        return false;
    }
    ...
    if (!proto)
    {
        sLog->outError("guild", "Unknown item (GUID: %u, id: %u) in guild bank, skipped.", itemGuid, itemEntry);
        return false;
    }
    ...
    {
        sLog->outError("guild", "Item (GUID %u, id: %u) not found in item_instance, deleting from guild bank!", itemGuid, itemEntry);
...
bool Guild::Member::LoadFromDB(Field* fields)
{
    ...
    if (!m_zoneId)
    {
        sLog->outError("guild", "Player (GUID: %u) has broken zone-data", GUID_LOPART(m_guid));
...
bool Guild::Member::CheckStats() const
{
    if (m_level < 1)
    {
        sLog->outError("guild", "Player (GUID: %u) has a broken data in field `characters`.`level`, deleting him from guild!", GUID_LOPART(m_guid));
        return false;
    }
    if (m_class < CLASS_WARRIOR || m_class >= MAX_CLASSES)
    {
        sLog->outError("guild", "Player (GUID: %u) has a broken data in field `characters`.`class`, deleting him from guild!", GUID_LOPART(m_guid));
        return false;
    }
    return true;
}
...
Item* Guild::BankMoveItemData::StoreItem(SQLTransaction& trans, Item* pItem)
{
    ...
    for (ItemPosCountVec::const_iterator itr = m_vec.begin(); itr != m_vec.end(); )
    {
        ...
        sLog->outDebug("guild", "GUILD STORAGE: StoreItem tab = %u, slot = %u, item = %u, count = %u",
...
void Guild::BankMoveItemData::LogAction(MoveItemData* pFrom) const
{
    ...
    {
        sLog->outCommand(m_pPlayer->GetSession()->GetAccountId(), "GM %s (Account: %u) deposit item: %s (Entry: %u Count: %u) to guild bank (Guild ID: %u)",
                         m_pPlayer->GetName(), m_pPlayer->GetSession()->GetAccountId(), pFrom->GetItem()->GetTemplate()->Name1.c_str(),
                         pFrom->GetItem()->GetEntry(), pFrom->GetItem()->GetCount(), m_pGuild->GetId());
    }}
...
{
    sLog->outDebug("guild", "GUILD STORAGE: CanStore() tab = %u, slot = %u, item = %u, count = %u",
        m_container, m_slotId, pItem->GetEntry(), pItem->GetCount());
...
bool Guild::Create(Player* pLeader, const std::string& name)
{
    ...
    sLog->outDebug("guild", "GUILD: creating guild [%s] for leader %s (%u)",
...
void Guild::HandleRoster(WorldSession* session /*= NULL*/)
{
    ...
    sLog->outDebug("guild", "WORLD: Sent (SMSG_GUILD_ROSTER)");
}
...
void Guild::HandleQuery(WorldSession* session)
{
    ...
    sLog->outDebug("guild", "WORLD: Sent (SMSG_GUILD_QUERY_RESPONSE)");
}
...
void Guild::HandleSetRankInfo(WorldSession* session, uint32 rankId, const std::string& name, uint32 rights, uint32 moneyPerDay, GuildBankRightsAndSlotsVec rightsAndSlots)
{
    ...
    {
        sLog->outDebug("guild", "WORLD: Changed RankName to '%s', rights to 0x%08X", name.c_str(), rights);
...
void Guild::HandleInviteMember(WorldSession* session, const std::string& name)
{
    ...
    sLog->outDebug("guild", "Player %s invited %s to join his Guild", player->GetName(), name.c_str());
    ...
    sLog->outDebug("guild", "WORLD: Sent (SMSG_GUILD_INVITE)");
}
...
void Guild::HandleDisband(WorldSession* session)
{
    ...
    {
        Disband();
        sLog->outDebug("guild", "WORLD: Guild Successfully Disbanded");
        delete this;
    }
}
...
void Guild::HandleGuildPartyRequest(WorldSession* session)
{
    ...
    session->SendPacket(&data);
    sLog->outDebug("network", "WORLD: Sent (SMSG_GUILD_PARTY_STATE_RESPONSE)");
}
...
void Guild::SendEventLog(WorldSession* session) const
{
    ...
    session->SendPacket(&data);
    sLog->outDebug("guild", "WORLD: Sent (SMSG_GUILD_EVENT_LOG_QUERY_RESULT)");
}
...
void Guild::SendBankLog(WorldSession* session, uint8 tabId) const
{
    ...
    if (tabId < GetPurchasedTabsSize() || tabId == GUILD_BANK_MAX_TABS)
    {
        ...
        session->SendPacket(&data);
        sLog->outDebug("guild", "WORLD: Sent (SMSG_GUILD_BANK_LOG_QUERY_RESULT) for tab %u", tabId);
    }
}
...
void Guild::SendBankList(WorldSession* session, uint8 tabId, bool withContent, bool withTabInfo) const
{
    ...
    sLog->outDebug("guild", "WORLD: Sent (SMSG_GUILD_BANK_LIST)");
}
...
void Guild::SendPermissions(WorldSession* session) const
{
    ...
    sLog->outDebug("guild", "WORLD: Sent (SMSG_GUILD_PERMISSIONS_QUERY_RESULTS)");
}
...
void Guild::SendMoneyInfo(WorldSession* session) const
{
    ...
    session->SendPacket(&data);
    sLog->outDebug("guild", "WORLD: Sent SMSG_GUILD_BANK_MONEY_WITHDRAWN");
}
...
void Guild::SendLoginInfo(WorldSession* session)
{
    ...
    */
    ...
    if (m_motd.size() > 0)
        data.append(m_motd.c_str(), m_motd.size());
    session->SendPacket(&data);
    sLog->outDebug("guild", "WORLD: Sent SMSG_GUILD_SEND_MOTD");
    ...
    if (dbTabId < GetPurchasedTabsSize() || isMoneyTab)
    {
        ...
        if (pLog->CanInsert())
        {
            ...
            if (BankEventLogEntry::IsMoneyEvent(eventType))
            {
                if (!isMoneyTab)
                {
                    sLog->outError("guild", "GuildBankEventLog ERROR: MoneyEvent(LogGuid: %u, Guild: %u) does not belong to money tab (%u), ignoring...", guid, m_id, dbTabId);
                    return false;
                }
            }
            else if (isMoneyTab)
            {
                sLog->outError("guild", "GuildBankEventLog ERROR: non-money event (LogGuid: %u, Guild: %u) belongs to money tab, ignoring...", guid, m_id);
                return false;
            }
...
bool Guild::LoadBankTabFromDB(Field* fields)
{
    ...
    if (tabId >= GetPurchasedTabsSize())
    {
        sLog->outError("guild", "Invalid tab (tabId: %u) in guild bank, skipped.", tabId);
        return false;
    }
...
bool Guild::LoadBankItemFromDB(Field* fields)
{
    ...
    if (tabId >= GetPurchasedTabsSize())
    {
        sLog->outError("guild", "Invalid tab for item (GUID: %u, id: #%u) in guild bank, skipped.",
...
bool Guild::Validate()
{
    ...
    {
        sLog->outError("guild", "Guild %u has invalid number of ranks, creating new...", m_id);
        broken_ranks = true;
    }
    else
    {
        for (uint8 rankId = 0; rankId < _GetRanksSize(); ++rankId)
        {
            ...
            {
                sLog->outError("guild", "Guild %u has broken rank id %u, creating default set of ranks...", m_id, rankId);
                broken_ranks = true;
            }
        }
    }
...
void Guild::_MoveItems(MoveItemData* pSrc, MoveItemData* pDest, uint32 splitedAmount)
{
    ...
    {
        sLog->outFatal("misc", "Guild::SwapItems: Player %s(GUIDLow: %u) tried to move item %u from tab %u slot %u to tab %u slot %u, but item %u has a stack of zero!",
...
void Guild::_BroadcastEvent(GuildEvents guildEvent, uint64 guid, const char* param1, const char* param2, const char* param3) const
{
    ...
    sLog->outDebug("guild", "WORLD: Sent SMSG_GUILD_EVENT");
}
...
void Guild::SendGuildRanksUpdate(uint64 setterGuid, uint64 targetGuid, uint32 rank)
{
    ...
    sLog->outDebug("network", "WORLD: Sent SMSG_GUILD_RANKS_UPDATE");
}
